                               ...
                               GtkSnapshot *snapshot)
{
  ...
  /*g_message ("%s BOUNDS: %f, %f, %f, %f", __FUNCTION__,*/
  ...
  /*GtkAllocation alloc;*/
  /*gtk_widget_get_allocation (self->test_widget, &alloc);*/
  /*g_message ("%s ALLOCATION: %d, %d, %d, %d", __FUNCTION__,*/
             /*alloc.x, alloc.y, alloc.width, alloc.height);*/


#if 0
  const graphene_point_t pick_point = { child_bounds.origin.x + (child_bounds.size.width / 2.0f),
                                        child_bounds.origin.y + child_bounds.size.height * (3.0/4.0) };


  g_message (">>>>>>>>>>>>>>>>>>>>>>>>>>>> %f, %f", pick_point.x, pick_point.y);
  GtkWidget *picked = gtk_widget_pick (widget, pick_point.x, pick_point.y);
  g_message ("<<<<<<<<<<<<<<<<<<<<<<<<<<<<");
  ...
  gtk_snapshot_append_color (snapshot, &(GdkRGBA){0, 0, 1, 1},
                             &GRAPHENE_RECT_INIT (pick_point.x, pick_point.y, 5, 5));
#endif


  /*width = TEST_WIDGET_MIN_SIZE;*/
  /*height = TEST_WIDGET_MIN_SIZE;*/

  graphene_rect_t self_bounds;
  gtk_widget_compute_bounds (self->test_widget, self->test_widget, &self_bounds);
  /*g_message ("Self bounds: %f, %f, %f, %f",*/
             /*self_bounds.origin.x,*/
             /*self_bounds.origin.y,*/
             /*self_bounds.size.width,*/
             /*self_bounds.size.height);*/
  ...
  for (x = 0; x < G_N_ELEMENTS (points); x ++)
    {
      ...
      /*g_message ("%d:      Start: %f/%f", x, points[x].coords.x, points[x].coords.y);*/
      gtk_widget_translate_coordinates (self->test_widget, widget,
                                        points[x].coords.x, points[x].coords.y,
                                        &px, &py);
      /*g_message ("%d: Translated: %d/%d", x, px, py);*/

      gtk_snapshot_append_color (snapshot, &points[x].color,
                                 &GRAPHENE_RECT_INIT (px, py,
                                                      4,
                                                      4));
      gtk_widget_translate_coordinates (widget, self->test_widget, px, py, &px, &py);
      /*g_message ("%d:        End: %d/%d", x, px, py);*/
    }
  ...
  gtk_snapshot_append_color (snapshot, &black,
                             &GRAPHENE_RECT_INIT (child_bounds.origin.x + child_bounds.size.width - 1,
                                                  child_bounds.origin.y,
                                                  1,
                                                  child_bounds.size.height));
...
main (int argc, char **argv)
{
  ...
  /*graphene_matrix_init_scale (&transform, 1, 2, 1);*/
  /*graphene_matrix_init_rotate (&transform,*/
                               /*45.0,*/
                               /*graphene_vec3_z_axis ());*/
  /*gtk_widget_set_transform (test_widget, &transform);*/
