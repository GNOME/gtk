  ...
  if (!window)
    {
      ...
      gtk_label_set_markup (GTK_LABEL (label),
			    "French (FranÅ√Åßais) Bonjour, Salut\n"
			    "Korean (ÅÌïúÅÍÅ∏Ä)   ÅÏïàÅÎÖïÅÌïòÅÏÑÅ∏ÅÏöî, ÅÏïàÅÎÖïÅÌïòÅÏãÅ≠ÅÎãàÅÍÅπå\n"
			    "Russian (Å–Å†Å—ÉÅ—ÅÅ—ÅÅ–Å∫Å–Å∏Å–Åπ) Å–óÅ–Å¥Å—ÄÅ–Å∞Å–Å≤Å—ÅÅ—ÇÅ–Å≤Å—ÉÅ–ÅπÅ—ÇÅ–Åµ!\n"
			    "Chinese (Simplified) <span lang=\"zh-cn\">ÅÂÖÉÅÊÅ∞î	ÅÂÅºÄÅÂèë</span>\n"
			    "Chinese (Traditional) <span lang=\"zh-tw\">ÅÂÖÉÅÊÅ∞Å£	ÅÈñãÅÁôÅº</span>\n"
			    "Japanese <span lang=\"ja\">ÅÂÖÉÅÊÅ∞ó	ÅÈñãÅÁôÅ∫</span>");
      gtk_label_set_justify (GTK_LABEL (label), GTK_JUSTIFY_LEFT);
      gtk_container_add (GTK_CONTAINER (frame), label);
      gtk_box_pack_start (GTK_BOX (vbox), frame, FALSE, FALSE, 0);

      frame = gtk_frame_new ("Bidirection Label");
      label = gtk_label_new ("Arabic	ÅÿÅßÅŸÑÅÿÅ≥ÅŸÑÅÿÅßÅŸÖ ÅÿÅπÅŸÑÅŸäÅŸÉÅŸÖ\n"
			     "Hebrew	Å◊Å©Å◊úÅ◊ïÅ◊ù");
      ...
      label = gtk_label_new ("This label is underlined!\n"
			     "This one is underlined (Å„ÅìÅ„ÇìÅ„ÅÅ´Å„ÅÅ°Å„ÅÅØ) in quite a funky fashion");
      gtk_label_set_justify (GTK_LABEL (label), GTK_JUSTIFY_LEFT);
  ...
  if (!window)
    {
      ...
      gtk_box_pack_start (GTK_BOX (GTK_DIALOG (window)->vbox), button, FALSE, FALSE, 0);
      button = gtk_button_new_with_mnemonic ("Button 3 (_Å—Ñ)");
      gtk_box_pack_start (GTK_BOX (GTK_DIALOG (window)->vbox), button, FALSE, FALSE, 0);
      button = gtk_button_new_with_mnemonic ("Button 4 (_Å–Å§)");
      gtk_box_pack_start (GTK_BOX (GTK_DIALOG (window)->vbox), button, FALSE, FALSE, 0);
  ...
  if (!window)
    {
      ...
      entry = gtk_entry_new ();
      gtk_entry_set_text (GTK_ENTRY (entry), "hello world ÅÿÅßÅŸÑÅÿÅ≥ÅŸÑÅÿÅßÅŸÖ ÅÿÅπÅŸÑÅŸäÅŸÉÅŸÖ");
