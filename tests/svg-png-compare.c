      ...
      while (1)
        {
          ...
          if (g_str_has_suffix (g_file_peek_path (child), ".svg"))
  ...
  for (unsigned int i = 0; i < files->len; i++)
    {
      ...
      path2 = g_strdup (path);
      path2[strlen (path) - 3] = 'p';
      path2[strlen (path) - 2] = 'n';
      path2[strlen (path) - 1] = 'g';

      svg = GDK_PAINTABLE (gdk_texture_new_from_filename (path2, NULL));
      if (svg)
        {
