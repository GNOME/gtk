          ...
          gpointer      user_data)
{
  ...
  g_print ("received item %p, %s, at %g %g\n",
           item, gtk_label_get_label (GTK_LABEL (item)),
           data->x, data->y);

  gtk_fixed_move (GTK_FIXED (data->canvas), item, data->x, data->y);
...
item_drag_motion (GtkDropTarget *dest,
                  ...
                  int            y)
{
  g_print ("item drag-motion: %s\n", gtk_drop_target_find_mimetype (dest));
  ...
  for (i = 0; i < 4; i++)
    {
      ...
      item = canvas_item_new (i);
      gtk_fixed_put (GTK_FIXED (canvas), item, 50 * i, 50 * i);
    }
