...
main (int argc, char *argv[])
{
  ...
  gtk_center_box_set_start_widget (box, GTK_WIDGET (child));

  child = gtk_label_new ("Center");
  gtk_label_set_ellipsize (child, PANGO_ELLIPSIZE_END);
  gtk_center_box_set_center_widget (box, GTK_WIDGET (child));

  child = gtk_label_new ("End");
  gtk_label_set_ellipsize (child, PANGO_ELLIPSIZE_END);
  gtk_center_box_set_end_widget (box, GTK_WIDGET (child));
