...
void get_dialog_response(GtkDialog *dialog,
                         gint arg1,
			 gpointer data)
{
  GtkEntry *entry; 
...
void changed(GtkEntry *entry, gpointer *data)
{
  GtkEntry *other_entry = GTK_ENTRY(data);
  printf("%s | %s\n",gtk_entry_get_text(entry),gtk_entry_get_text(other_entry));
}

void make_selection_dialog(GdkScreen *screen, 
			   GtkWidget *entry,
			   GtkWidget* other_entry)
{
  ...
  g_object_connect(clear,"signal::clicked", clear_entry, double_entry ,NULL);
...
main (int argc, char *argv[])
{
  ...
  dialog_label = gtk_label_new("Please enter the name of\nthe second display\n");
  ...
  gtk_signal_connect(GTK_OBJECT (dialog), "response",
                     GTK_SIGNAL_FUNC (get_dialog_response), 
                     display_entry);
  ...
  while(!correct_second_display)
  {
    ...
    if(dpy2)
    {
      ...
      char *error_msg = g_new(char, sizeof(char)*(strlen(screen2_name)+50));
      sprintf(error_msg,
