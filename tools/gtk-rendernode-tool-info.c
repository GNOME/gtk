...
count_nodes (GskRenderNodeReplay *replay,
             GskRenderNode       *node,
             gpointer             data)
{
...
file_info (const char *filename)
{
  ...
  gsk_render_node_replay_set_node_filter (replay, count_nodes, &count, NULL);
  
  gsk_render_node_replay_foreach_node (replay, node);
  ...
  gsk_render_node_replay_free (replay);
