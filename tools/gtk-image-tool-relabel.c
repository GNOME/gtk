               ...
               GdkColorState   *color_state)
{
  ...
  gdk_texture_downloader_set_format (downloader, gdk_texture_get_format (orig));
  gdk_texture_downloader_set_color_state (downloader, gdk_texture_get_color_state (orig));

  bytes = gdk_texture_downloader_download_bytes (downloader, &stride);

  texture = gdk_memory_texture_new_with_color_state (gdk_texture_get_width (orig),
                                                     gdk_texture_get_height (orig),
                                                     gdk_texture_get_format (orig),
                                                     color_state,
                                                     bytes,
                                                     stride);
            ...
            const char ***argv)
{
  ...
  g_option_context_set_summary (context, _("Change the color state of the image without conversion."));
