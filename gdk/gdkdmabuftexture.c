...
gdk_dmabuf_texture_invoke_callback (gpointer data)
{
  ...
  if (display->egl_downloader &&
                                      ...
                                      download->stride))
    {
      GDK_DISPLAY_DEBUG (display, DMABUF,
                         "Downloading dmabuf texture using %s",
                         G_OBJECT_TYPE_NAME (display->egl_downloader));
  ...
  if (display->vk_downloader &&
                                      ...
                                      download->stride))
    {
      GDK_DISPLAY_DEBUG (display, DMABUF,
                         "Downloading dmabuf texture using %s",
                         G_OBJECT_TYPE_NAME (display->vk_downloader));
      g_atomic_int_set (&download->spinlock, 1);
      return FALSE;
    }

#ifdef HAVE_DMABUF
  gdk_dmabuf_download_mmap (GDK_TEXTURE (download->texture),
                            download->format,
                            download->color_state,
                            download->data,
                            download->stride);
  GDK_DISPLAY_DEBUG (display, DMABUF, "Downloading dmabuf texture using mmap");
#endif
