 ...
 */
static guint32
gdk_absolute_event_interpolation_newest_event_time_before (GdkAbsoluteEventInterpolation  *interpolator,
                                                           guint32                         interval)
{
  ...
  if (interpolator->event_history->len == 0)
    {
      /* TODO maybe use an assertion here instead of just a warning */
      g_warning ("Returning '0' as the newest event time before interval, since the event history is empty");
      return 0;
    }
  ...
  /* Since we are looking for a timestamp larger than that of the oldest
                                      ...
                                      GdkEvent               *event)
{
  ...
  /* The newest event in the event history holds the current accumulated uninterpolated properties */
 ...
 */
guint32
gdk_event_interpolation_newest_event_time_before (GdkEventInterpolation  *interpolator,
                                                  guint32                 interval)
{
  return gdk_absolute_event_interpolation_newest_event_time_before (interpolator->absolute_interpolator,
                                                                    interval);
}
