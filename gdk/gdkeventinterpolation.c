 ...
 * of pinch events etc. 'Discrete' properties are those with a discrete rather
                                            ...
                                            GdkEvent               *event)
{
  ...
  gdk_event_get_relative_values_for_interpolation (event, values);

  for (i = 0; i < values->len; i++)
    {
      dv = &g_array_index (values, gdouble, i);
      *dv = 0;
    }
  ...
  dummy_event_time = gdk_event_get_time (event) - EVENT_HISTORY_DUMMY_POLLING_INTERVAL;
                                      ...
                                      GdkEvent               *event)
{
  ...
  /* Currently only precise scroll events are supported */
  g_return_if_fail (gdk_event_get_event_type (event) == GDK_SCROLL);
  g_return_if_fail (event->scroll.direction == GDK_SCROLL_SMOOTH);
  ...
  /* The latest event in the event history holds the current accumulated uninterpolated properties */
  latest_absolute_event =
    gdk_absolute_event_interpolation_get_latest_event (interpolator->absolute_interpolator);

  gdk_event_get_relative_values_for_interpolation (latest_absolute_event, acc_uninterpolated_values);
  ...
  /* Convert relative properties to absolute ones */
  for (i = 0; i < acc_uninterpolated_values->len; i++)
    {
      av = &g_array_index (acc_uninterpolated_values, gdouble, i);
      dv = &g_array_index (new_values, gdouble, i);
      *av += *dv;
    }
                                           ...
                                           gint64                  frame_time)
{
  ...
  for (i = 0; i < acc_interpolated_values->len; i++)
    {
      av = &g_array_index (acc_interpolated_values, gdouble, i);
      dv = &g_array_index (new_values, gdouble, i);
