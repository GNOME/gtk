 ...
 * milliseconds passed since the oldest event in the event history.
 *
 * For example, if the timestamps of the events in the history are
 * [4, 8, 12, 16, 20], then calling this function with interval = 9 will return
 * '12'. That's because the oldest event has a timestamp of '4'. Adding '9' we
 * get '13'. The most recent event with a timestamp lower than '13' is the 3rd
 * event, which has a timestamp of '12'.
 *
 * It's the responsibility of the caller to check if the history is empty
 * before calling this function.
 */
                                                           ...
                                                           guint32                         interval)
{
  ...
  /* The first element in the array is the oldest */
  oldest_elem = g_ptr_array_index (interpolator->event_history, 0);
  cutoff_point = gdk_event_get_time (oldest_elem) + interval;
