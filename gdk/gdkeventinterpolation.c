...
gdk_absolute_event_interpolation_latest_event_time (GdkAbsoluteEventInterpolation  *interpolator)
{
  ...
  if (interpolator->event_history->len == 0)
    {
      GDK_NOTE (EVENTS, g_message ("Interpolation using current time since event history is empty"));
                                                    ...
                                                    gint64                          frame_time)
{
  ...
  if (interpolator->event_history->len == 0)
    {
      ...
      g_warning ("Can't interpolate event, history is empty");
      return NULL;
    }
  ...
  g_warning ("Can't interpolate event, frame time earlier than first history element");
