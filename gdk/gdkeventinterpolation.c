 ...
 * consecutive input events.
 */
#define RELATIVE_EVENT_HISTORY_DUMMY_POLLING_INTERVAL 12
 ...
 *
 * Absolute interpolation is also used internally to interpolate relative
 * events, or events with both absolute and relative properties. In that case
 * the relative properties are converted to absolute ones by accumulating them
 * before adding them to the history. This is done by the relative event
 * interpolator.
 */
...
gdk_absolute_event_interpolation_history_push (GdkAbsoluteEventInterpolation  *interpolator,
                                               GdkEvent                       *event)
{
  /* Only support precise scroll events for now */
 ...
 */
static GdkEvent *
gdk_absolute_event_interpolation_get_latest_event (GdkAbsoluteEventInterpolation  *interpolator)
{
 ...
 */
static guint32
gdk_absolute_event_interpolation_latest_event_time (GdkAbsoluteEventInterpolation  *interpolator)
{
  ...
  if (interpolator->event_history->len == 0)
    {
      ...
      g_warning ("Returning '0' as the latest event time, since the event history is empty");
      return 0;
    }

  latest_elem = g_ptr_array_index (interpolator->event_history, interpolator->event_history->len - 1);

  return gdk_event_get_time (latest_elem);
}
 ...
 */
static gint
gdk_absolute_event_interpolation_latest_event_before (GdkAbsoluteEventInterpolation  *interpolator,
                                                      guint32 timestamp)
{
  gint i;
  GdkEvent *elem = NULL;
...
/*
 * Returns the timestamp of the most recent event to occur before 'interval'
 * milliseconds passed since the start of the current event history.
 *
 * It's the responsibility of the caller to check if the history is empty
 * before calling this function.
 */
static guint32
gdk_absolute_event_interpolation_latest_event_time_before (GdkAbsoluteEventInterpolation  *interpolator,
                                                           guint32                         interval)
{
  guint32 cutoff_point;
  GdkEvent *first_elem;
  GdkEvent *requested_elem;
  gint i;

  if (interpolator->event_history->len == 0)
    {
      /* TODO maybe use an assertion here instead of just a warning */
      g_warning ("Returning '0' as the latest event time before interval, since the event history is empty");
      return 0;
    }

  /* Treat the first element as time '0' */
  first_elem = g_ptr_array_index (interpolator->event_history, 0);
  cutoff_point = gdk_event_get_time (first_elem) + interval;

  /* Find the first timestamp equal to or lower than the cutoff */
  i = gdk_absolute_event_interpolation_latest_event_before (interpolator,
                                                            cutoff_point);

  /* Since we are looking for a timestamp larger than that of the earliest
     event in history, we should find *some* event. */
  g_assert (i >= 0);

  requested_elem = g_ptr_array_index (interpolator->event_history, i);
  return gdk_event_get_time (requested_elem);
}
 ...
 */
static void
gdk_absolute_event_interpolation_set_stop_event (GdkAbsoluteEventInterpolation  *interpolator,
                                                 GdkEvent                       *event)
{
  /* Only support precise scroll events for now */
  g_return_if_fail (gdk_event_get_event_type (event) == GDK_SCROLL);
 ...
 */
static GdkEvent *
gdk_absolute_event_interpolation_get_stop_event (GdkAbsoluteEventInterpolation  *interpolator)
{
 ...
 *
 * Stop events are *not* reset as part of the history reset, since once a stop
 * event has been received it's illegal to receive any more events for the same
 * gesture history.
 */
static void
 ...
 * latest event. However if a later event is then added, calling with the same
                                                    ...
                                                    gint64                          frame_time)
{
  ...
  if (i >= 0)
    ...
    {
      /* The interpolation point lies before the earliest event. This is a
  ...
  if (gdk_event_get_time (elem) == interpolation_point)
    ...
    {
      /* We have 2 consecutive events with different time stamps */
      GdkEvent *first_elem;
      ...
      /* the scroll deltas actually hold accumulated deltas */
      gdk_event_get_scroll_deltas (first_elem, &first_elem_x, &first_elem_y);
      gdk_event_get_scroll_deltas (second_elem, &second_elem_x, &second_elem_y);
      ...
      /* update the interpolated properties */
      interpolated_elem->scroll.delta_x = interpolated_x;
      interpolated_elem->scroll.delta_y = interpolated_y;
 ...
 *
 * The relative events interpolator uses an absolute events interpolator for
 * history bookkeeping and for doing the actual interpolation. Relative events
 * are converted to absolute ones before saving in the event history (by
 * accumulating relative properties). Interpolated events are converted back to
 * relative ones by calculating the delta from the accumulated interpolated
 * properties.
 */
...
/*
 * Allocate a relative events interpolator.
 *
 * Should be freed with gdk_event_interpolation_free().
 */
                                            ...
                                            GdkEvent               *event)
{
  ...
  dummy_event->scroll.delta_x = 0;
  dummy_event->scroll.delta_y = 0;

  dummy_event->scroll.time = gdk_event_get_time (event) -
    RELATIVE_EVENT_HISTORY_DUMMY_POLLING_INTERVAL;
 ...
 * them before adding them to the history.
 */
void
gdk_event_interpolation_history_push (GdkEventInterpolation  *interpolator,
                                      GdkEvent               *event)
{
  GdkEvent* latest_absolute_event;
  gdouble uninterpolated_x;
  gdouble uninterpolated_y;
  gdouble delta_x;
  gdouble delta_y;
  ...
  if (!gdk_absolute_event_interpolation_history_length (interpolator->absolute_interpolator))
    {
      gdk_event_interpolation_history_push_dummy (interpolator, event);
    }

  /* The latest event in the absolute event history holds the accumulated uninterpolated properties */
  latest_absolute_event =
    gdk_absolute_event_interpolation_get_latest_event (interpolator->absolute_interpolator);

  gdk_event_get_scroll_deltas (latest_absolute_event, &uninterpolated_x, &uninterpolated_y);
  ...
  saved_event->scroll.delta_x = uninterpolated_x + delta_x;
  saved_event->scroll.delta_y = uninterpolated_y + delta_y;
                                           ...
                                           gint64                  frame_time)
{
  ...
  gdk_event_get_scroll_deltas (interpolated_event, &interpolated_x, &interpolated_y);

  /* TODO there is no gdk_event_set_scroll_deltas */
 ...
 */
guint32
gdk_event_interpolation_latest_event_time (GdkEventInterpolation  *interpolator)
{
  return gdk_absolute_event_interpolation_latest_event_time (interpolator->absolute_interpolator);
}
...
{
  GdkEvent *saved_event = gdk_event_copy (event);
  gdk_absolute_event_interpolation_set_stop_event (interpolator->absolute_interpolator,
                                                   saved_event);
}
...
{
  return gdk_absolute_event_interpolation_get_stop_event (interpolator->absolute_interpolator);
}

/*
 * Returns the timestamp of the most recent event to occur before 'interval' milliseconds
 * passed since the start of the current event history.
 *
 * It's the responsibility of the caller to check if the history is empty
 * before calling this function.
 */
guint32
gdk_event_interpolation_latest_event_time_before (GdkEventInterpolation  *interpolator,
                                                  guint32                 interval)
{
  return gdk_absolute_event_interpolation_latest_event_time_before (interpolator->absolute_interpolator,
                                                                    interval);
}
