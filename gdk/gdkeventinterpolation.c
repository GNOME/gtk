...
gdk_absolute_event_interpolation_latest_event_time (GdkAbsoluteEventInterpolation  *interpolator)
{
  ...
  if (interpolator->event_history->len == 0)
    {
      g_warning ("Interpolation using current time since event history is empty");
                                                           ...
                                                           guint32                         interval)
{
  ...
  if (interpolator->event_history->len == 0)
    {
      g_warning ("Interpolation using current time instead of latest since event history is empty");
      return GDK_CURRENT_TIME;
    }
...
void
gdk_absolute_event_interpolation_received_stop (GdkAbsoluteEventInterpolation  *interpolator,
                                                GdkEvent                       *event)
{
  /* Only support precise scroll events for now */
  g_return_if_fail (gdk_event_get_event_type (event) == GDK_SCROLL);
...
/*
 * Return the average time between consecutive events.
 *
...
struct _GdkRelativeEventInterpolation
{
  ...
  /*
   * The absolute position of the point where the gesture started.
   */
  gdouble fixed_x;
  gdouble fixed_y;
                                                    ...
                                                    GdkEvent                       *event)
{
   ...
   * more dummy events will have to be created when the first real event
   * arrives. Alternatively we can start with linear, since we'll have 2 data
   * points once the first event is received. Then with each additional event
   * we can use more and more sophisticated methods.
   */
  if (!gdk_absolute_event_interpolation_history_length (interpolator->absolute_interpolator))
    {
      ...
      gdk_event_get_coords (event, &first_x, &first_y);

      interpolator->fixed_x = first_x;
      interpolator->fixed_y = first_y;

      /* We know the delta so we can calculate where the input device has been
      ...
      gdk_event_set_coords (dummy_event, first_x, first_y);

      interpolator->latest_uninterpolated_x = first_x;
      interpolator->latest_uninterpolated_y = first_y;

      interpolator->latest_interpolated_x = first_x;
      interpolator->latest_interpolated_y = first_y;

      /* TODO there is no gdk_event_set_scroll_deltas */
      dummy_event->scroll.delta_x = 0;
      dummy_event->scroll.delta_y = 0;

      dummy_event->scroll.time = gdk_event_get_time (event) - RELATIVE_EVENT_HISTORY_DUMMY_POLLING_INTERVAL;
...
{
  interpolator->fixed_x = 0;
  interpolator->fixed_y = 0;
                                                    ...
                                                    gint64                          frame_time)
{
  ...
  /* Update the event absolute location to where the gesture began */
  gdk_event_set_coords(interpolated_event, interpolator->fixed_x, interpolator->fixed_y);
...
guint32
gdk_relative_event_interpolation_latest_event_time (GdkRelativeEventInterpolation  *interpolator)
{
  return gdk_absolute_event_interpolation_latest_event_time (interpolator->absolute_interpolator);
}

void
gdk_relative_event_interpolation_received_stop (GdkRelativeEventInterpolation  *interpolator,
                                                GdkEvent *event)
{
  GdkEvent *saved_event = gdk_event_copy (event);
  gdk_absolute_event_interpolation_received_stop (interpolator->absolute_interpolator, saved_event);
}

GdkEvent *
gdk_relative_event_interpolation_get_stop_event (GdkRelativeEventInterpolation  *interpolator)
{
  return gdk_absolute_event_interpolation_get_stop_event (interpolator->absolute_interpolator);
}

guint32
gdk_relative_event_interpolation_latest_event_time_before (GdkRelativeEventInterpolation  *interpolator,
                                                           guint32                         interval)
{
...
guint32
gdk_relative_event_interpolation_average_event_interval (GdkRelativeEventInterpolation  *interpolator)
{
