          ...
          if (priv->freeze_count == 0)
            {
               ...
               *
               * As mentioned above, this function is not scheduled at precise
               * regular times. Experimentally it is scheduled at offsets of
               * about 500us on average from the "precise" time, and sometimes
               * even 1500us away. For high refresh rate displays - 240Hz and
               * above - this is on the order of half a frame interval. In
               * order not to reset the frame time due to false positives on
               * these displays, a minimum of 2000us is used for the threshold.
               */
              reset_threshold = MAX ((frame_interval / 2), 2000);

              if (frame_time_error >= reset_threshold)
