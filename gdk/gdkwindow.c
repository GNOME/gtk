...
gdk_window_init (GdkWindow *window)
{
  ...
  window->event_interpolators = g_ptr_array_new_with_free_func ((GDestroyNotify) gdk_event_interpolation_free);
...
gdk_window_finalize (GObject *object)
{
  ...
  if (window->event_interpolators)
    {
      gdk_window_stop_interpolation_callback (window);
      g_ptr_array_unref (window->event_interpolators);
      window->event_interpolators = NULL;
    }
                                                   ...
                                                   gint64     frame_duration_ms)
{
  ...
  GdkEventInterpolation * const animating_interpolator =
    g_ptr_array_index (window->event_interpolators, 0);
  ...
  if (send_stop)
    {
      ...
      g_ptr_array_remove_index (window->event_interpolators, 0);

      /* Is another gesture waiting in line? */
      if (window->event_interpolators->len)
        {
    ...
    case GDK_SCROLL:
      ...
      if (event_win->interpolate_scroll_events &&
          source_event->scroll.direction == GDK_SCROLL_SMOOTH)
        {
          ...
          if (event_win->event_interpolators->len == 0)
            {
          ...
          else
            {
              /* Scrolling is already in progress. We always push events to the
                 latest interpolator */
              accumulating_interpolator = g_ptr_array_index (event_win->event_interpolators,
                                                             event_win->event_interpolators->len - 1);
          ...
          if (new_gesture_started)
            {
              ...
              g_ptr_array_add (event_win->event_interpolators,
