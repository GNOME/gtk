...
gdk_window_init (GdkWindow *window)
{
  ...
  window->relative_interpolators = g_ptr_array_new_with_free_func ((GDestroyNotify) gdk_event_interpolation_free);
  window->interpolation_tick_id = 0;
  window->interpolation_overlapped_gesture_started = FALSE;
  window->interpolation_last_event_received_time = 0;
...
/*
 * Update the interpolation offset for overlapped gestures. See
 * gdk_window_update_interpolation_offset for details.
 *
 * If an overlapped gesture just started outputting interpolated events, we
 * need at most a single display frame worth of events. Otherwise we'll get a
 * 'jump', since too many events will be coalesced when calculating the first
 * interpolated event.
 */
static void
gdk_window_update_interpolation_offset_overlapped (GdkWindow *window,
                                                   gint64     frame_time,
                                                   gint64     frame_duration_ms)
{
  ...
  /* Usually the new gesture will already contain some events. However it's
     possibe that the new gesture only contains a 'stop' event, or maybe
     just a 'start' event was received while the rest of the events were
     momentarily stalled. If there are no events in the new gesture then we
  ...
  if (gdk_event_interpolation_history_length (animating_interpolator) > 0)
    {
      /* frame_duration_ms should be valid here because we don't stop the
         callback between overlapped gestures */
      guint32 timestamp_offset_from_latest_event = (frame_time / 1000) -
        gdk_event_interpolation_latest_event_time_before (animating_interpolator,
                                                          frame_duration_ms);
 ...
 * timestamp of the latest event. We update it to the minimal value that
                                        ...
                                        gint64     frame_time)
{
  ...
  GdkEventInterpolation * const animating_interpolator =
    g_ptr_array_index (window->relative_interpolators, 0);
  guint32 timestamp_offset_from_latest_event = 0;

  /* Even though window->interpolation_last_frame_time might not be valid here,
     we only use frame_duration_ms when it is. */
  gint64 frame_duration_ms = MAX ((frame_time - window->interpolation_last_frame_time) / 1000, 1);

  /* The offset is calculated somewhat differently for overlapped gestures. */
  if (window->interpolation_overlapped_gesture_started)
    {
      gdk_window_update_interpolation_offset_overlapped (window, frame_time, frame_duration_ms);
      return;
    }
  ...
  if (gdk_event_interpolation_history_length (animating_interpolator) > 0)
    {
      timestamp_offset_from_latest_event = (frame_time / 1000) -
        gdk_event_interpolation_latest_event_time (animating_interpolator);
    }
...
static gboolean
gdk_window_interpolation_all_existing_events_emitted (GdkEventInterpolation *animating_interpolator,
                                                      gint64                 adjusted_interpolation_point)
{
  /* If the timestamp of the interpolated event is equal to or larger than the
     timestamp of the latest event in the history, then all events in the
     history buffer have been interpolated. */
  gint32 interpolated_event_time = (adjusted_interpolation_point / 1000);
  gint32 latest_uninterpolated_event_time =
    gdk_event_interpolation_latest_event_time (animating_interpolator);

  return (interpolated_event_time >= latest_uninterpolated_event_time);
}

/*
 * Use a heuristic to detect if we should stop the interpolation callback. For
 * example, some versions of XWayland don't emit a 'scroll stop' signal, so
 * without this heuristic the callback will continue firing indefinitely.
 * Furthermore, some event types don't have a stop signal. It's also possible
 * that the user simply stopped moving his fingers on the touchpad, without
 * lifting them up.
 *
 ...
 * stopped. That can occur 'legitimately' if the user stops moving his fingers
 * but doesn't lift them up from the touchpad. However it's ok to stop the
 * callback in this situation as well since once the fingers start moving again
 * a new callback will be started.
 *
 * In any case, as long as there are uninterpolated events in the history
 * buffer we don't stop the callback.
 */
static gboolean
gdk_window_interpolation_callback_should_stop (GdkWindow *window,
                                               gint64     frame_time)
{
  /* We always emit events from the oldest (first) interpolator */
  GdkEventInterpolation * const animating_interpolator =
  ...
  /* Estimate the interpolation point of the previous frame. It's ok to err
     as long as we get an earlier rather than a later point, since we shouldn't
  ...
  if (gdk_event_interpolation_history_length (animating_interpolator) > 0 &&
      !gdk_window_interpolation_all_existing_events_emitted (animating_interpolator,
                                                             previous_interpolation_point))
    {
      /* There are some uninterpolated events in the event history. */
      return FALSE;
    }
  ...
  /* The grace period takes into account both frame and event duration */
  ...
  timestamp_offset_from_latest_event = g_get_monotonic_time () -
    window->interpolation_last_event_received_time;
 ...
 */
static void
gdk_window_interpolation_handle_stop_event (GdkWindow *window,
                                            GdkEvent  *stop_event,
                                            gint64     adjusted_interpolation_point)
{
  /* We always emit events from the oldest (first) interpolator */
  ...
  if (gdk_event_interpolation_history_length (animating_interpolator) == 0)
    ...
    {
      /* If all events in the history buffer have been interpolated then we can
         emit a stop event. */
      send_stop = gdk_window_interpolation_all_existing_events_emitted (animating_interpolator,
                                        ...
                                        GdkWindow     *window)
{
  ...
  if (!stop_event &&
      gdk_window_interpolation_callback_should_stop (window,
                                                     frame_time))
    {
      /* Reset the history in order to prevent 'jumps' when we start receiving
         events again. */
  ...
  if (stop_event)
    {
      gdk_window_interpolation_handle_stop_event (window, stop_event, adjusted_interpolation_point);
    }
    ...
    case GDK_SCROLL:
      ...
      if (event_win->interpolate_scroll_events &&
          source_event->scroll.direction == GDK_SCROLL_SMOOTH)
        {
          ...
          event_win->interpolation_last_event_received_time = g_get_monotonic_time ();
