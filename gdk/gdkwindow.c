...
gdk_window_init (GdkWindow *window)
{
  ...
  window->relative_interpolators = g_ptr_array_new_with_free_func ((GDestroyNotify) gdk_relative_event_interpolation_free);
  window->interpolation_tick_id = 0;
  window->interpolation_overlapped_gesture_started = FALSE;
  window->interpolation_last_event_time = 0;

  /* FIXME make that configurable */
  window->interpolate_scroll_events = TRUE;
}
...
gdk_window_finalize (GObject *object)
{
  ...
  if (window->relative_interpolators)
    {
      gdk_window_stop_interpolation_callback (window);
      g_ptr_array_unref (window->relative_interpolators);
      window->relative_interpolators = NULL;
    }
 ...
 * between the earliest and latest events in the event history. While we could
 ...
 * event in the event history. We maintain a 'constant' offset from the frame
 * time to compensate for that. This offset is the effective latency. That
 * offset should be constant since, as explained above, the interpolation point
 * should move in lockstep with the frame time.
 *
 * The higher the offset the bigger the latency, so we would like the get the
 * minimal fixed offset that guarantees we'll always get an adjusted
 ...
 * changes. The offset is reset whenever a new gesture starts, so that should
 ...
 */
static void gdk_window_update_interpolation_offset (GdkWindow *window,
                                                    gint64     frame_time)
{
  /* We always emit events from the oldest (first) interpolator */
  GdkRelativeEventInterpolation * const animating_interpolator =
    g_ptr_array_index (window->relative_interpolators, 0);
  ...
  /* If an overlapped gesture just started, we need at most a single display
     frame worth of events. Otherwise we'll get jumps. */
  if (window->interpolation_overlapped_gesture_started)
    {
      /* frame_duration_ms should be valid here because we don't stop the
         callback between consecutive gestures */
      timestamp_offset_from_latest_event = (frame_time / 1000) -
        gdk_relative_event_interpolation_latest_event_time_before (animating_interpolator,
                                                                   frame_duration_ms);

      /* hard-set the time offset and the time offset target to the same value */
      window->interpolation_time_offset = timestamp_offset_from_latest_event;
      window->interpolation_time_offset_target = timestamp_offset_from_latest_event;
  ...
  /*
   * Calculate the time delta between the upcoming frame and the latest event
   ...
   */
  if (gdk_relative_event_interpolation_history_length (animating_interpolator) > 0)
    {
      timestamp_offset_from_latest_event = (frame_time / 1000) -
        gdk_relative_event_interpolation_latest_event_time (animating_interpolator);
    }
  else
    {
      /* If there are no events in the history then don't change the current
         offset. */
      timestamp_offset_from_latest_event = 0;
    }

  /* Adjust the target offset if it's too small */
  if (window->interpolation_time_offset_target < timestamp_offset_from_latest_event)
    window->interpolation_time_offset_target = timestamp_offset_from_latest_event;
  ...
  /* Update the actual interpolation time offset. In order to get smooth
     animation, window->interpolation_time_offset should stay relatively
     constant. */
      ...
      if (window->interpolation_last_frame_time == 0)
      ...
      else
        {
          /* Gesture animation is in progress, "ease" updating the offset to prevent back-jumps.
             ...
             cause us to ask for a value in time earlier than the one in the last callback. So
...
/*
 * Use a heuristic to detect if we should stop the interpolation callback. Some
 * versions of XWayland don't emit a 'scroll stop' signal, so without this
 * heuristic the callback will continue firing indefinitely.
 *
 ...
 * passed without receiving an input event it means that scrolling has stopped.
 * That can occur 'legitimately' if the user stops moving his fingers but
 * doesn't lift them up from the touchpad. However it's ok to stop the callback
 * in this situation as well since once the fingers start moving again a new
 * callback will be started.
 */
static gboolean gdk_window_interpolation_callback_should_stop (GdkWindow *window,
                                                               gint64     frame_time)
{
  /* We always emit events from the oldest (first) interpolator */
  GdkRelativeEventInterpolation * const animating_interpolator =
  ...
  if (window->interpolation_last_frame_time == 0)
    {
      /* First callback for this gesture, no need to stop the callback yet */
      return FALSE;
    }

  /* frame_time is in us while event time is in ms */
  display_frame_duration = frame_time - window->interpolation_last_frame_time;
  event_frame_duration = gdk_relative_event_interpolation_average_event_interval (animating_interpolator) * 1000;

  grace_period = MAX (display_frame_duration * INTERPOLATION_DISPLAY_GRACE,
                      event_frame_duration * INTERPOLATION_EVENT_GRACE);

  /* Calculate the time delta between the upcoming frame and the time the
     latest event was received. frame_time can sometimes get wacky - it
     might become larger than expected - so use g_get_monotonic_time() */
  timestamp_offset_from_latest_event = g_get_monotonic_time () - window->interpolation_last_event_time;

  return (timestamp_offset_from_latest_event > grace_period);
}
...
{
  /* We always emit events from the oldest (first) interpolator */
  GdkRelativeEventInterpolation * animating_interpolator =
    g_ptr_array_index (window->relative_interpolators, 0);
  gint64 adjusted_interpolation_point;
  gint64 frame_time = gdk_frame_clock_get_frame_time (frame_clock);
  GdkEvent *stop_event = NULL;

  /* Get the stop event, if it was set */
  stop_event = gdk_relative_event_interpolation_get_stop_event (animating_interpolator);

  /* Guard against a missing scroll stop event. This can happen due to bugs, e.g.
     XWayland currently doesn't emit stop events, or simply because the user stopped
     moving his fingers on the touchpad */
  if (!stop_event && gdk_window_interpolation_callback_should_stop (window, frame_time))
    {
      gdk_window_stop_interpolation_callback (window);
      return;
    }
  ...
  /* It's possible for the event history to be empty, but only if a stop event
     was received. That can happen if the user didn't move his fingers for a
     while, triggering the gdk_window_interpolation_callback_should_stop()
     check above, then later lifted his fingers without moving them. */
  if (gdk_relative_event_interpolation_history_length (animating_interpolator) > 0)
    {
      /* Synthesize an interpolated event */
      GdkEvent *interpolated_event =
        gdk_relative_event_interpolation_interpolate_event (animating_interpolator,
                                                            adjusted_interpolation_point);

      /* We might not get an event. That can happen, for example, if
         adjusted_interpolation_point is earlier than the timestamp of
         the oldest event. */
    ...
    {
      g_warning ("Can't interpolate event, history is empty even though no stop event was received");
    }
  ...
  if (stop_event)
    {
      ...
      if (gdk_relative_event_interpolation_history_length (animating_interpolator) == 0)
        ...
        {
          /* If the timestamp of the just-emitted event is larger than the
             latest event in the history, then we are done. */
          gint32 interpolated_event_time = (adjusted_interpolation_point / 1000);
          gint32 latest_uninterpolated_event_time =
            gdk_relative_event_interpolation_latest_event_time (animating_interpolator);

          if (interpolated_event_time >= latest_uninterpolated_event_time)
            {
              send_stop = TRUE;
            }
        }
      ...
      if (send_stop)
        {
          /* Set the event time to be 1ms after the previous event, just in case. */
          stop_event->scroll.time = (adjusted_interpolation_point / 1000) + 1;

          /* We are done with the current interpolator */
          g_ptr_array_remove_index (window->relative_interpolators, 0);

          /* Is another gesture waiting in line? */
          if (window->relative_interpolators->len)
            {
              window->interpolation_overlapped_gesture_started = TRUE;
            }
          else
            {
              /* Last gesture finished, stop the animation callback */
              gdk_window_stop_interpolation_callback (window);
            }

          /* Emit the stop event */
          _gdk_event_emit (stop_event);
          gdk_event_free (stop_event);
        }
    }
}
      ...
      if (window->relative_interpolators->len)
        {
          ...
          gdk_relative_event_interpolation_history_reset (animating_interpolator);
        }
		    ...
		    gulong serial)
{
      ...
      _gdk_synthesize_crossing_events (display,
           ...
           (((evmask & GDK_SMOOTH_SCROLL_MASK) == 0 &&
            ...
            ((evmask & GDK_SMOOTH_SCROLL_MASK) != 0 &&
             source_event->scroll.direction != GDK_SCROLL_SMOOTH &&
             gdk_event_get_pointer_emulated (source_event))))
    return (event_win->interpolate_scroll_events ? 1 : 0);
  ...
  switch (type)
    ...
    case GDK_SCROLL:
      ...
      if (event_win->interpolate_scroll_events &&
          source_event->scroll.direction == GDK_SCROLL_SMOOTH)
        {
          GdkRelativeEventInterpolation * accumulating_interpolator;
          gboolean new_gesture_started = FALSE;

          event_win->interpolation_last_event_time = g_get_monotonic_time ();

          /* Start the animation if it is not already in progress */
          gdk_window_start_interpolation_callback (event_win);

          if (event_win->relative_interpolators->len == 0)
            {
              /* No scrolling is currently in progress */
              new_gesture_started = TRUE;
            }
          else
            {
              /* Scrolling is already in progress. We always push events to the
                 latest interpolator */
              accumulating_interpolator = g_ptr_array_index (event_win->relative_interpolators,
                                                             event_win->relative_interpolators->len - 1);

              if (gdk_relative_event_interpolation_get_stop_event (accumulating_interpolator))
                {
          ...
          if (new_gesture_started)
            {
              accumulating_interpolator = gdk_relative_event_interpolation_new ();
              g_ptr_array_add (event_win->relative_interpolators,
          ...
          if (gdk_event_is_scroll_stop_event (event))
            {
              gdk_relative_event_interpolation_received_stop (accumulating_interpolator,
                                                              event);
            }
          else
            {
              gdk_relative_event_interpolation_history_push (accumulating_interpolator,
                                                             event);
            }
