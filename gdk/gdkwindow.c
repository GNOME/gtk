...
gdk_window_init (GdkWindow *window)
{
  ...
  window->interpolation_start_event_sent = FALSE;
...
/*
 * Update the interpolation offset for overlapped gestures. Overlapped gestures
 * are gestures that were started while a previous gesture was still being
 * interpolated.
 *
 * Usually the new gesture will already contain some data events. However it's
 * possibe that the new gesture only contains a 'stop' event - for example a
 * scroll stop. It's also possible that we only received a 'start' event, while
 * the rest of the events were momentarily stalled.
 *
 * If there are no 'data' events in the new gesture then we don't change the
 * offset.
 *
 * See gdk_window_update_interpolation_offset for details about the
                                                   ...
                                                   gint64     frame_duration_ms)
{
  ...
  if (gdk_event_interpolation_history_length (animating_interpolator) > 0)
    {
      /*
       * A new gesture is about to emit its first interpolated event. We need
       * at most a single display frame worth of events. Otherwise we'll get a
       * 'jump', since too many events will be coalesced when calculating the
       * first interpolated event.
       *
       * reference_event_time will contain the timestamp of the most recent
       * event to occur no later than a single frame duration since the start
       * of the new gesture.
       *
       * frame_duration_ms should contain a valid value here, because we don't
       * stop the callback between overlapped gestures.
       */
      const guint32 reference_event_time =
        gdk_event_interpolation_newest_event_time_before (animating_interpolator,
                                                          frame_duration_ms);

      /* frame_time is in us, reference_event_time in ms */
      guint32 timestamp_offset_from_reference_event = (frame_time / 1000) -
                                                      reference_event_time;

      /* Hard-set the time offset and the time offset target to the same value */
      window->interpolation_time_offset = timestamp_offset_from_reference_event;
      window->interpolation_time_offset_target = timestamp_offset_from_reference_event;

      /* The overlapped gesture is now ongoing, reset the flag */
      window->interpolation_overlapped_gesture_started = FALSE;
    }
  else if (gdk_event_interpolation_get_stop_event (animating_interpolator))
    {
      /* The overlapped gesture ended without any data events */
      window->interpolation_overlapped_gesture_started = FALSE;
    }
}
  ...
  if (gdk_event_interpolation_history_length (animating_interpolator) > 0)
    {
      timestamp_offset_from_newest_event = (frame_time / 1000) -
        gdk_event_interpolation_newest_event_time (animating_interpolator);
    }
                                                      ...
                                                      gint64                 adjusted_interpolation_point)
{
  /* If the timestamp of the interpolated event is equal to or larger than the
     timestamp of the newest event in the history, then all events in the
     history buffer have been interpolated. */
  guint32 interpolated_event_time = (adjusted_interpolation_point / 1000);
  guint32 newest_uninterpolated_event_time =
    gdk_event_interpolation_newest_event_time (animating_interpolator);

  return (interpolated_event_time >= newest_uninterpolated_event_time);
}
  ...
  if (send_stop)
    {
      ...
      /* Send a start event if one wasn't sent yet */
      if (start_event && !window->interpolation_start_event_sent)
        {
          /* Set the start event time to be the same as the stop event */
          gdk_event_set_time (start_event, gdk_event_get_time (stop_event));
      ...
      /* Reset interpolation_start_event_sent for the next gesture */
      window->interpolation_start_event_sent = FALSE;
    }
}
                                        ...
                                        GdkWindow     *window)
{
  ...
  /* Get the start and stop events (any of them might be null if not set) */
          ...
          if (start_event && !window->interpolation_start_event_sent)
            {
              ...
              window->interpolation_start_event_sent = TRUE;
            }
  ...
  if (stop_event)
    {
      gdk_window_interpolation_handle_stop_event (window, start_event, stop_event, adjusted_interpolation_point);
    }
                                    ...
                                    GdkEvent  *event)
{
  ...
  if (window->event_interpolators->len == 0)
    {
  ...
  else
    {
      ...
      accumulating_interpolator = g_ptr_array_index (window->event_interpolators,
                                                     window->event_interpolators->len - 1);
  ...
  if (new_gesture_started)
    {
      ...
      g_ptr_array_add (window->event_interpolators,
      ...
      _gdk_synthesize_crossing_events (display,
           ...
           (((evmask & GDK_SMOOTH_SCROLL_MASK) == 0 &&
            ...
            ((evmask & GDK_SMOOTH_SCROLL_MASK) != 0 &&
             source_event->scroll.direction != GDK_SCROLL_SMOOTH &&
             gdk_event_get_pointer_emulated (source_event))))
    return (event_win->interpolate_events ? 1 : 0);
