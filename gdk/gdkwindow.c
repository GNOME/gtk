 ...
 * Experimentally 40ms seems like a good hard limit.
 */
 ...
 * interpolation point must move in lockstep to the frame time. That's
 * because we want to calculate where the input device is located at the time
 * the frame is displayed as opposed to when the event was received.
 *
 * There is no point in trying to interpolate an event at the upcoming frame
 * time, since the upcoming frame time we will always be later then the latest
 ...
 */
static void update_interpolation_offset (GdkWindow *window,
                                         gint64     frame_time)
{
      ...
      if (window->interpolation_last_frame_time == 0)
      ...
      else
        {
          /* gesture animation in progress, "ease" updating the offset to prevent back-jumps.
             ...
             cause us to ask for a value in time earlier than the one in the last callback */
          gint64 frame_duration = frame_time - window->interpolation_last_frame_time;
          guint32 delta_to_target = window->interpolation_time_offset_target - window->interpolation_time_offset;
          window->interpolation_time_offset += MIN(delta_to_target, (frame_duration + 1)/ 2);
        }
    }
                                        ...
                                        GdkWindow     *window)
{
  ...
  update_interpolation_offset (window, frame_time);
