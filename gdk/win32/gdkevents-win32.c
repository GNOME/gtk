                                 ...
                                 gboolean   *return_val)
{
  ...
  /* for WM_[SYS]KEYDOWN and WM_[SYS]KEYUP */
  GdkWin32Surface *impl;
  GdkModifierType state;
  guint keyval;
  guint16 keycode;
  ...
  BYTE key_state[256]; /* also used for WM_IME_COMPOSITION */
  GArray *translation;
  MSG msg2;
  int level = 0;
  ...
  switch (msg->message)
    ...
    case WM_INPUTLANGCHANGE:
      ...
      display_win32->input_locale_items->input_locale = (HKL) msg->lParam;
      _gdk_win32_keymap_set_active_layout (win32_keymap, display_win32->input_locale_items->input_locale);
      display_win32->input_locale_items->keymap_serial++;
      GDK_NOTE (EVENTS,
                g_print (" cs:%lu hkl:%p%s",
                         (gulong) msg->wParam,
                         (gpointer) msg->lParam,
                         display_win32->input_locale_items->notifcation_sink->input_locale_is_ime ? " (IME)" : ""));
      ...
      event = gdk_key_event_new (GDK_KEY_PRESS,
                                 surface,
                                 display_win32->device_manager->core_keyboard,
                                 _gdk_win32_get_next_tick (msg->time),
                                 0,
                                 0,
                                 FALSE,
    ...
    case WM_SYSKEYDOWN:
      ...
      if ((msg->wParam != VK_F10 && msg->wParam != VK_MENU) && !(HIWORD (msg->lParam) & KF_ALTDOWN))
        break;

      /* Let the system handle Alt-Tab, Alt-Space and Alt-F4 unless
       * the keyboard is grabbed.
       */
      keyboard_grab = _gdk_display_get_last_device_grab (display, display_win32->device_manager->core_keyboard);
      if (!keyboard_grab && (msg->wParam == VK_TAB || msg->wParam == VK_SPACE || msg->wParam == VK_F4))
        break;

      /* Continue to code common with WM_KEYUP and WM_KEYDOWN */
      G_GNUC_FALLTHROUGH;
    ...
    case WM_KEYDOWN:
      ...
      if (msg->wParam == VK_PROCESSKEY || display_win32->event_record->in_ime_composition)
        break;
      ...
      if (msg->message == WM_KEYDOWN && msg->wParam == VK_SHIFT)
        {
          ...
          if (display_win32->event_record->both_shift_pressed[0] == 0)
            display_win32->event_record->both_shift_pressed[0] = pressed_shift;
          else if (display_win32->event_record->both_shift_pressed[0] != pressed_shift)
            display_win32->event_record->both_shift_pressed[1] = pressed_shift;
        }

      if (msg->message == WM_KEYUP && msg->wParam == VK_SHIFT)
        {
          if (display_win32->event_record->both_shift_pressed[0] != 0 &&
              display_win32->event_record->both_shift_pressed[1] != 0)
            {
              int tmp_retval;
              MSG fake_release = *msg;
              int pressed_shift = msg->lParam & 0xffffff;

              if (display_win32->event_record->both_shift_pressed[0] == pressed_shift)
                fake_release.lParam = display_win32->event_record->both_shift_pressed[1];
              else
                fake_release.lParam = display_win32->event_record->both_shift_pressed[0];

              display_win32->event_record->both_shift_pressed[0] = display_win32->event_record->both_shift_pressed[1] = 0;
    ...
    case WM_IME_STARTCOMPOSITION:
      display_win32->event_record->in_ime_composition = TRUE;
      break;

    case WM_IME_ENDCOMPOSITION:
      display_win32->event_record->in_ime_composition = FALSE;
      break;
    ...
    case WM_IME_COMPOSITION:
      ...
      for (int i = 0; i < ccount; i++)
        {
          GdkEvent *ime_composition_event;
          ...
          translated.level = 0;
          ime_composition_event = gdk_key_event_new (GDK_KEY_PRESS,
                                                     surface,
                                                     display_win32->device_manager->core_keyboard,
                                                     _gdk_win32_get_next_tick (msg->time),
                                                     0,
          ...
          ime_composition_event = gdk_key_event_new (GDK_KEY_RELEASE,
                                                     surface,
                                                     display_win32->device_manager->core_keyboard,
                                                     _gdk_win32_get_next_tick (msg->time),
                                                     0,
		     ...
		     int *ret_valp)
{
  ...
  if (surface == NULL || surface_status == GDK_WIN32_SURFACE_CREATED)
    {
      if (surface_status == GDK_WIN32_SURFACE_EXITED)
        exit (msg->wParam);
      else
        return FALSE;
    }
...
#define return GOTO_DONE_INSTEAD
  ...
  switch (msg->message)
    ...
    case WM_IME_ENDCOMPOSITION:
    case WM_IME_COMPOSITION:
      event = gdk_win32_handle_keyboard_event (display, surface, msg, &return_val);

      if (event != NULL)
        _gdk_win32_append_event (event);

      if (msg->message == WM_SYSCHAR && return_val)
        goto done;
