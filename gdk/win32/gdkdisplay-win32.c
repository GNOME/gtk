                            ...
                            GError          **error)
{
  ...
  res = gdk_win32_display_create_d3d_devices (self, 0, DXGI_ADAPTER_FLAG_SOFTWARE, &self->d3d11_device, &self->d3d12_device, &errors[0]) ||
        ...
        gdk_win32_display_create_d3d_devices (self, DXGI_ADAPTER_FLAG_SOFTWARE, 0, &self->d3d11_device, &self->d3d12_device, &errors[2]) ||
        gdk_win32_display_create_d3d_devices (self, DXGI_ADAPTER_FLAG_SOFTWARE, 0, &self->d3d11_device, NULL, &errors[3]);
...
gdk_win32_display_dispose (GObject *object)
{
  ...
  if (self->dummy_context_wgl.hglrc != NULL)
    {
      wglMakeCurrent (NULL, NULL);
      wglDeleteContext (self->dummy_context_wgl.hglrc);
      self->dummy_context_wgl.hglrc = NULL;
    }
  ...
  if (self->shcore_funcs.hshcore != NULL)
    {
      FreeLibrary (self->shcore_funcs.hshcore);
      self->shcore_funcs.hshcore = NULL;
    }
