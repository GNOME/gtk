  ...
  if (context_win32->egl_context != NULL)
    {
      ...
      /* XXX/WIP: Deal with cleanup!
      if (impl->egl_dummy_surface != EGL_NO_SURFACE)
        eglDestroySurface (display_win32->egl_disp, impl->egl_dummy_surface);
      if (impl->egl_surface != EGL_NO_SURFACE)
        eglDestroySurface (display_win32->egl_disp, impl->egl_surface);
...
#ifdef GDK_ENABLE_WIN32_EGL
  egl_disp = eglGetDisplay (display_win32->hdc_egl_temp);
