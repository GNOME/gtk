...
_gdk_win32_display_init_gl (GdkDisplay *display)
{
  ...
  /* TODO: for GTK+ master, see if we are using any envvar to force EGL, or when using automatic fallback */
  disable_wgl = g_getenv ("GDK_GL_EGL") != NULL;
#endif
                              ...
                              GError **error)
{
  ...
  /* TODO: for GTK+ master, see if we are using any envvar to force EGL, or when using automatic fallback */
  use_es = (g_getenv ("GDK_GL_EGL") != NULL ||
                                   ...
                                   gint *minor)
{
  ...
  if (!_gdk_win32_display_init_gl (display) || !display_win32->have_wgl
#ifdef GDK_WIN32_ENABLE_EGL
      || !display_win32->have_egl
#endif
     )
