...
_get_system_font_name (HDC hdc)
{
  ...
  /* The pango font fallback list got fixed during 1.43, before
   * that using anything but "Segoe UI" would lead to a poor glyph coverage */
  if (pango_version_check (1, 43, 0) != NULL &&
      g_ascii_strcasecmp (font_desc_string, "Segoe UI") != 0)
    {
