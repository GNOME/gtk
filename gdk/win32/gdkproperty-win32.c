                        ...
                        GValue      *value)
{
  ...
  if (strcmp ("gtk-double-click-time", name) == 0)
  ...
  else if (strcmp ("gtk-font-name", name) == 0)
    {
      ...
      /* Tha pango font fallback list got fixed during 1.43, before
       * that using the system font would lead to a poor coverage on Win8+ */
      if (g_win32_check_windows_version (6, 2, 0, G_WIN32_OS_ANY) &&
          pango_version_check (1, 43, 0) != NULL)
        return FALSE;

      font_name = _get_system_font_name (_gdk_display_hdc);
      if (font_name)
        {
          GDK_NOTE(MISC, g_print("gdk_screen_get_setting(\"%s\") : %s\n", name, font_name));
