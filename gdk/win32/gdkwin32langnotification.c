...
struct _GdkWin32LPNSink
{
  ITfLanguageProfileNotifySink itf_lpn_sink;

  gint ref_count;
};

typedef struct _GdkWin32LPNSink GdkWin32LPNSink;
...
static GdkWin32LPNSink *langchangenotify = NULL;
static GdkWin32ALPNSink *actlangchangenotify = NULL;
static ITfSource *itf_source = NULL;
static DWORD langchangenotify_id = 0;
static DWORD actlangchangenotify_id = 0;

static ULONG STDMETHODCALLTYPE
lpn_sink_addref (ITfLanguageProfileNotifySink *This)
{
  GdkWin32LPNSink *lpn_sink = (GdkWin32LPNSink *) This;
  int ref_count = ++lpn_sink->ref_count;

  return ref_count;
}

static HRESULT STDMETHODCALLTYPE
lpn_sink_queryinterface (ITfLanguageProfileNotifySink *This,
                         REFIID                        riid,
                         LPVOID                       *ppvObject)
{
  *ppvObject = NULL;
  ...
  if (IsEqualGUID (riid, &IID_IUnknown))
    {
      ITfLanguageProfileNotifySink_AddRef (This);
      *ppvObject = This;
      return S_OK;
    }
  else if (IsEqualGUID (riid, &IID_ITfLanguageProfileNotifySink))
    {
      ITfLanguageProfileNotifySink_AddRef (This);
      *ppvObject = This;
      return S_OK;
...
static ULONG STDMETHODCALLTYPE
lpn_sink_release (ITfLanguageProfileNotifySink *This)
{
  GdkWin32LPNSink *lpn_sink = (GdkWin32LPNSink *) This;
  int ref_count = --lpn_sink->ref_count;
...
static HRESULT STDMETHODCALLTYPE
lpn_sink_on_language_change (ITfLanguageProfileNotifySink *This,
                             LANGID                        langid,
                             WINBOOL                      *pfAccept)
{
  g_warning ("language change");
  return S_OK;
}

static HRESULT STDMETHODCALLTYPE
lpn_sink_on_language_changed (ITfLanguageProfileNotifySink* This)
{
  g_warning ("language changed");
  return S_OK;
}

static ITfLanguageProfileNotifySinkVtbl lpn_sink_vtbl = {
  lpn_sink_queryinterface,
  lpn_sink_addref,
  lpn_sink_release,
  lpn_sink_on_language_change,
  lpn_sink_on_language_changed,
};

static GdkWin32LPNSink *
lpn_sink_new ()
{
  GdkWin32LPNSink *result;

  result = g_new0 (GdkWin32LPNSink, 1);
  result->itf_lpn_sink.lpVtbl = &lpn_sink_vtbl;
  result->ref_count = 0;

  ITfLanguageProfileNotifySink_AddRef (&result->itf_lpn_sink);
                          ...
                          LPVOID                             *ppvObject)
{
  *ppvObject = NULL;
  ...
  else if (IsEqualGUID (riid, &IID_ITfActiveLanguageProfileNotifySink))
    {
      ITfActiveLanguageProfileNotifySink_AddRef (This);
      *ppvObject = This;
      return S_OK;
...
{
  HRESULT hr;
  /*ITfInputProcessorProfiles *itf_ipp;*/
...
*/
  ...
  if (!SUCCEEDED (hr))
    return;
/*
  hr = ITfInputProcessorProfiles_QueryInterface (itf_ipp, &IID_ITfSource, (VOID **) &itf_source);
  ITfInputProcessorProfiles_Release (itf_ipp);
*/
  ...
  if (!SUCCEEDED (hr))
    return;
/*
  langchangenotify = lpn_sink_new ();

  hr = ITfSource_AdviseSink (itf_source,
                             &IID_ITfLanguageProfileNotifySink,
                             (IUnknown *) langchangenotify,
                             &langchangenotify_id);

  if (!SUCCEEDED (hr))
    {
      ITfLanguageProfileNotifySink_Release (&langchangenotify->itf_lpn_sink);
      langchangenotify = NULL;
