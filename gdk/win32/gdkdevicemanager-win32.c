          ...
          for (j = 0; j < cursors_count; j++)
            {
              ...
              winpointer_fill_event_all (device_manager,
                                         device,
                                         msg,
                                         info[i-1],
                                         &evt_all);
              winpointer_event_all_to_touch_event (evt_all,
                                                   &evt);
              winpointer_event_finalize (device_manager,
                                         device,
                                         evt);
            }
        }
    ...
    case WM_POINTERENTER:
      {
        ...
        winpointer_fill_event_all (device_manager, device, msg, info[i-1], &evt_all);
        gdk_input_winpointer_fill_touch_event (evt);
        winpointer_event_finalize (device_manager, device, evt);
      }
    case WM_POINTERLEAVE:
      {
        GdkEvent *evt = gdk_event_new (GDK_PROXIMITY_OUT);
        GdkWinpointerEventAll evt_all;
...
G_GNUC_END_IGNORE_DEPRECATIONS;
  ...
  if (type == PT_PEN)
    {
      GdkDeviceWinpointer *device;
      GdkDeviceWinpointer *device_eraser;
      ...
      /* check GdkWindow mask and all parent GdkWindows masks */
      target_window = window;
      while (TRUE)
        {
          /*TODO: also device_eraser */
          if ((gdk_window_get_device_events (target_window, GDK_DEVICE (device)) & mask_test) != 0 ||
              (gdk_window_get_events (target_window) & mask_test) != 0)
            {
              break;
            }

          if (target_window->parent == gdk_get_defualt_root_window () ||
              target_window->parent == NULL)
            {
              return FALSE;
            }
      ...
      gdk_winpointer_input_pen_event (msg, target_window);
    }
  else if (type == PT_TOUCH)
    {
      ...
      gdk_winpointer_input_touch_event (msg, target_window);
    }
