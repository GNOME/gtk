                       ...
                       POINTER_INFO *info)
{
  ...
  axes = g_new0 (gdouble, 1);
{
  POINTER_PEN_INFO *pen_info = (POINTER_PEN_INFO*) info;
  if (pen_info->penMask & TOUCH_MASK_PRESSURE)
    {
      axes[0] = (gdouble) pen_info->pressure;
    }
  else
    axes[0] = 512.0;
}
          ...
          for (j = cursor_count; j > 0; j--)
            {
              ...
              winpointer_make_event (device_manager, device, target_window, msg, info);
            }
        }
