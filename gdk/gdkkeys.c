                                          ...
                                          guint      keyval)
{
  ...
  offset = GPOINTER_TO_UINT (g_hash_table_lookup (keymap->cache, GUINT_TO_POINTER (keyval)));
  if (offset == 0)
    {
      GdkKeymapKey key;
      ...
      g_hash_table_insert (keymap->cache, GUINT_TO_POINTER (keyval), GUINT_TO_POINTER (offset));
    }

  return (GdkKeymapKey *)&g_array_index (keymap->cached_keys, GdkKeymapKey, offset);
}
