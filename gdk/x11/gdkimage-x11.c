  ...
  if (GDK_DISPLAY_IMPL_X11 (display)->gdk_use_xshm)
    {
	 ...
	 GDK_DISPLAY_IMPL_X11 (display)->have_shm_pixmaps = (res == 2);
    }
}
	  ...
	  if (GDK_DISPLAY_IMPL_X11 (screen_impl->display)->gdk_use_xshm)
	    {
	      ...
	      private->ximage = XShmCreateImage (screen_impl->xdisplay, 
						 xvisual, depth,
						 ZPixmap, NULL, x_shm_info,
						 width, height);
	      ...
	      if (x_shm_info->shmaddr == (char*) -1)
		{
	      ...
	      XShmAttach (GDK_SCREEN_XDISPLAY (private->screen), x_shm_info);
	      XSync (GDK_SCREEN_XDISPLAY (private->screen), False);
...
_gdk_x11_image_get_shm_pixmap (GdkImage *image)
{
  ...
  if (!private->shm_pixmap && image->type == GDK_IMAGE_SHARED && 
      GDK_DISPLAY_IMPL_X11 (display)->have_shm_pixmaps)
...
#define UNGRAB() G_STMT_START {			\
  ...
  if (image && image->type == GDK_IMAGE_SHARED)
      ...
      if (shm_pixmap)
	{
	   ...
	   */
	  GC xgc = XCreateGC (xdisplay, impl->xid, 0, NULL);
  ...
  if (private->ximage)		/* Deal with failure of creation */
	...
	case GDK_IMAGE_SHARED:
	  ...
	  if (private->shm_pixmap)
	    XFreePixmap (GDK_SCREEN_XDISPLAY (private->screen), 
			 private->shm_pixmap);
...
gint
_gdk_windowing_get_bits_for_depth (GdkDisplay *display, gint depth)
{
