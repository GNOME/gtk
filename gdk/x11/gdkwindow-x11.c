...
_gdk_windowing_window_init (GdkScreen * screen)
{
  ...
  g_assert (screen_impl->root_window == NULL);

  screen_impl->default_colormap = gdk_colormap_get_system_for_screen (screen);

  XGetGeometry (screen_impl->xdisplay, screen_impl->xroot_window,
		&screen_impl->xroot_window, &x, &y, &width, &height,
		&border_width, &depth);
  XGetWindowAttributes (screen_impl->xdisplay, screen_impl->xroot_window,
			&xattributes);

  screen_impl->root_window = g_object_new (GDK_TYPE_WINDOW, NULL);
  private = (GdkWindowObject *) screen_impl->root_window;
  impl = GDK_WINDOW_IMPL_X11 (private->impl);
  draw_impl = GDK_DRAWABLE_IMPL_X11 (private->impl);
  
  draw_impl->screen = screen;
  draw_impl->xid = screen_impl->xroot_window;
 ...
 * NOTE : to be multihead safe parent needs to be specified 
 * 
		...
		gint           attributes_mask)
{
  ...
  switch (private->window_type)
      ...
      if (GDK_WINDOW_TYPE (parent) != GDK_WINDOW_ROOT)
	{
	  g_warning (G_STRLOC "Toplevel windows must be created as children of"
		     "\nof a window of type "
		     "GDK_WINDOW_ROOT or GDK_WINDOW_FOREIGN");
  ...
  if (attributes->wclass == GDK_INPUT_OUTPUT)
	...
	{
	  if ((((GdkVisualPrivate *)gdk_visual_get_system_for_screen 
		(screen))->xvisual) ==  xvisual)
            {
	      draw_impl->colormap =
      ...
      private->bg_color.pixel = BlackPixel (xdisplay, screen_impl->screen_num);
      xattributes.background_pixel = private->bg_color.pixel;

      private->bg_pixmap = NULL;
      
      xattributes.border_pixel = BlackPixel (xdisplay, screen_impl->screen_num);
  ...
  switch (GDK_WINDOW_TYPE (private))
    ...
    case GDK_WINDOW_TEMP:
	{
	  ...
	  wm_window_protocols[0] = 
	    gdk_x11_get_xatom_by_name_for_display (screen_impl->display,
						   "WM_DELETE_WINDOW");
	  wm_window_protocols[1] = 
	    gdk_x11_get_xatom_by_name_for_display (screen_impl->display,
						   "WM_TAKE_FOCUS");
	  wm_window_protocols[2] = 
	    gdk_x11_get_xatom_by_name_for_display (screen_impl->display,
						   "_NET_WM_PING");
  ...
  XChangeProperty (xdisplay, 
		   ...
		   (guchar *) &GDK_DISPLAY_IMPL_X11 (screen_impl->display)->
		   leader_window,
		   1);
				    ...
				    GdkNativeWindow anid)
{
  ...
  gdk_error_trap_push ();
  result = XGetWindowAttributes (display_impl->xdisplay, anid, &attrs);
   ...
   *        the parent */
  gdk_error_trap_push ();
  result = XQueryTree (display_impl->xdisplay, anid, 
  ...
  if (i > 0)
    {
      XChangeProperty (GDK_WINDOW_XDISPLAY (window),
		       ...
		       (GDK_WINDOW_DISPLAY (window), "_NET_WM_STATE"),
  ...
  if (private->state & GDK_WINDOW_STATE_STICKY)
    {
      ...
      XChangeProperty (GDK_WINDOW_XDISPLAY (window),
                       GDK_WINDOW_XID (window),
		       gdk_x11_get_xatom_by_name_for_display 
		       (GDK_WINDOW_DISPLAY (window), "_NET_WM_DESKTOP"),
                  ...
                  guint32    timestamp)
{
  ...
  if (gdk_net_wm_supports_for_screen (GDK_WINDOW_SCREEN (window),
	...
	gdk_x11_get_xatom_by_name_for_display (GDK_WINDOW_DISPLAY (window),
      ...
      XSendEvent (GDK_WINDOW_XDISPLAY (window),
		  GDK_WINDOW_XROOTWIN (window), False,
			  ...
			  GdkWindowTypeHint hint)
{
  ...
  XChangeProperty (GDK_WINDOW_XDISPLAY (window),
		   GDK_WINDOW_XID (window),
		   gdk_x11_get_xatom_by_name_for_display 
		   (GDK_WINDOW_DISPLAY (window), "_NET_WM_WINDOW_TYPE"),
		   XA_ATOM, 32, PropModeReplace,
		   (guchar *)&atom, 1);
}
		      ...
		      const gchar *title)
{
  ...
  XChangeProperty (GDK_WINDOW_XDISPLAY (window),
		   GDK_WINDOW_XID (window),
		   gdk_x11_get_xatom_by_name_for_display 
		   (GDK_WINDOW_DISPLAY (window), "_NET_WM_NAME"),
		   gdk_x11_get_xatom_by_name_for_display 
		   (GDK_WINDOW_DISPLAY (window), "UTF8_STRING"),
		   8, PropModeReplace, title, strlen (title));
  
  set_text_property (window,
		     gdk_x11_get_xatom_by_name_for_display 
		     (GDK_WINDOW_DISPLAY (window), "WM_NAME"),
		     title);
  if (!gdk_window_icon_name_set (window))
    {
      XChangeProperty (GDK_WINDOW_XDISPLAY (window),
		       GDK_WINDOW_XID (window),
		       gdk_x11_get_xatom_by_name_for_display 
		       (GDK_WINDOW_DISPLAY (window), "_NET_WM_ICON_NAME"),
		       gdk_x11_get_xatom_by_name_for_display 
		       (GDK_WINDOW_DISPLAY (window), "UTF8_STRING"), 8,
		       PropModeReplace, title, strlen (title));
      set_text_property (window,
			 gdk_x11_get_xatom_by_name_for_display 
			 (GDK_WINDOW_DISPLAY (window), "WM_ICON_NAME"),
			 title);
    }
}
		     ...
		     const gchar *role)
{
  ...
  if (!GDK_WINDOW_DESTROYED (window))
	...
	XChangeProperty (GDK_WINDOW_XDISPLAY (window), GDK_WINDOW_XID (window),
			 gdk_x11_get_xatom_by_name_for_display 
			 (GDK_WINDOW_DISPLAY (window), "WM_WINDOW_ROLE"),
				    ...
				    gint      *y)
{
  ...
  if (!GDK_WINDOW_DESTROYED (window))
			 ...
			 &child, (unsigned int *)&num_children))
	{
	  ...
	  data_return = NULL;
	  XGetWindowProperty (GDK_WINDOW_XDISPLAY (window), win, 
			      atom, 0, 0, False, XA_CARDINAL, 
  ...
  if (xparent == root)
    {
      ...
      if (XGetGeometry (GDK_WINDOW_XDISPLAY (window), xwindow, 
				   ...
				   GdkModifierType *mask)
{
  ...
  if (!GDK_WINDOW_DESTROYED (window) &&
		     ...
		     &root, &child, &rootx, &rooty, &winx, &winy, &xmask))
    {
      if (child)
	return_val = 
	  gdk_window_lookup_for_display (GDK_WINDOW_DISPLAY (window),
					 child);
    }
				  ...
				  gint      *win_y)
{
  ...
  return gdk_screen_get_window_at_pointer (gdk_get_default_screen (),
					   win_x, win_y);
}
...
gdk_window_icon_name_set (GdkWindow *window)
{
  return GPOINTER_TO_UINT 
    (g_object_get_qdata (G_OBJECT (window),
...
gdk_window_stick (GdkWindow *window)
{
  GdkScreenImplX11 *screen_impl;
  g_return_if_fail (GDK_IS_WINDOW (window));
  screen_impl = GDK_SCREEN_IMPL_X11 (GDK_WINDOW_SCREEN (window));
  ...
  if (GDK_WINDOW_IS_MAPPED (window))
    {
      ...
      gdk_wmspec_change_state (FALSE, window,
			       gdk_atom_intern ("_NET_WM_STATE_STICKY", FALSE), 0);
      ...
      XGetWindowProperty (GDK_WINDOW_XDISPLAY (window),
			  GDK_WINDOW_XROOTWIN (window),
			  gdk_x11_get_xatom_by_name_for_display 
			  (GDK_WINDOW_DISPLAY (window), "_NET_CURRENT_DESKTOP"),
                          0, G_MAXLONG,
		      ...
		      gboolean merge)
{
  ...
  if (list)
      ...
      for (i = 0; i < baseh; i++)
	{
      ...
      if (rects)
	{
	  XShapeCombineRectangles (disp, win, ShapeBounding, 0, 0, 
				   rects, num_rects, ShapeSet, YXSorted);
	  g_free (rects);
	}
      XFree (list);
    }
  ...
  if (list)
      ...
      for (i = num - 1; ; i--)
	{
	  if ((!excl_child) || (!g_list_find (excludes, (gpointer *) list[i])))
	    {
	      if ((child = gdk_window_xid_at (xdisplay, list[i], wx, wy, x, y,
...
Window
gdk_window_xid_at_coords_for_screen (GdkScreen *screen,
				     gint       x,
				     gint       y,
				     GList     *excludes,
				     gboolean   excl_child)
{
  ...
  xdisplay = GDK_SCREEN_XDISPLAY (window);
  root = GDK_WINDOW_XID (window);
  display = GDK_SCREEN_DISPLAY (screen);
  ...
  if (list)
      ...
      do
	{
	  ...
	  if ((child = gdk_window_xid_at (xdisplay, list[i], 0, 0, 
					  x, y, excludes, excl_child)) == 0)
	    continue;
...
gdk_window_xid_at_coords (gint      x,
			  ...
			  gboolean  excl_child)
{
  return gdk_window_xid_at_coords_for_screen (gdk_get_default_screen (),
					      x, y, excludes, excl_child);
}
#endif
...
{
  gint dx, dy;
  _MoveResizeData *mv_resize = 
    g_object_get_data (G_OBJECT (display), "gdk-window-moveresize");
...
{
  _MoveResizeData *mv_resize = 
    g_object_steal_data (G_OBJECT (display), "gdk-window-moveresize");

  g_object_set_data (G_OBJECT (display), 
		     "gdk-window-moveresize-window", NULL);
			    ...
			    XPointer arg)
{
  ...
  _MoveResizeData *mv_resize = 
    g_object_get_data (G_OBJECT (gdk_display), "gdk-window-moveresize");
...
moveresize_lookahead (XEvent *event)
{
  ...
  XCheckIfEvent (display->xdisplay, &tmp_event,
...
_gdk_moveresize_handle_event (XEvent *event)
{
  ...
  switch (event->xany.type)
    ...
    case ButtonRelease:
      update_pos_for_display ((GdkDisplay *) display, event->xbutton.x_root,
				...
				GdkWindow  * window)
{
  ...
  if (window != g_object_get_data (G_OBJECT (display), 
				   "gdk-window-moveresize-window"))
    return FALSE;
...
static void
create_moveresize_window_for_screen (GdkScreen *screen,
				     guint32    timestamp)
{
  GdkWindowAttr attributes;
  gint attributes_mask;
  ...
  _MoveResizeData *mv_resize = 
    g_object_get_data (G_OBJECT (GDK_SCREEN_DISPLAY (screen)), 
...
{
  _MoveResizeData *mv_resize = 
    g_object_get_data (G_OBJECT (GDK_WINDOW_DISPLAY (window)),
		       "gdk-window-moveresize");
  g_assert (mv_resize != NULL);
