...
static GdkRegion *gdk_window_impl_x11_get_visible_region (GdkDrawable *
							  drawable);
...
gdk_window_impl_x11_get_colormap (GdkDrawable * drawable)
{
  ...
  if (!((GdkWindowObject *) drawable_impl->wrapper)->input_only &&
      drawable_impl->colormap == NULL)
    {
      ...
      XGetWindowAttributes (GDK_SCREEN_IMPL_X11 (drawable_impl->screen)->
			    xdisplay, drawable_impl->xid, &window_attributes);
...
static void
gdk_window_impl_x11_set_colormap (GdkDrawable * drawable, GdkColormap * cmap)
{
  ...
  if (cmap)
    {
      XSetWindowColormap (GDK_SCREEN_IMPL_X11 (draw_impl->screen)->xdisplay,
			  draw_impl->xid, GDK_COLORMAP_XCOLORMAP (cmap));
...
gdk_window_impl_x11_get_visible_region (GdkDrawable * drawable)
{
  ...
  gdk_rectangle_intersect (&result_rect, &impl->position_info.clip_rect,
			   &result_rect);
...
void
_gdk_windowing_window_init (GdkScreen * scr)
{
			   ...
			   GdkWindowAttr * attributes, gint attributes_mask)
{
  ...
  if (attributes_mask & GDK_WA_VISUAL)
    ...
    visual = gdk_visual_get_system_for_screen (screen);
  ...
  switch (private->window_type)
	...
	{
	  g_warning (G_STRLOC
		     "Toplevel windows must be created as children of\n"
    ...
    case GDK_WINDOW_CHILD:
      break;
    default:
      g_warning (G_STRLOC "cannot make windows of type %d",
		 private->window_type);
      return NULL;
    }
  ...
  if (attributes->wclass == GDK_INPUT_OUTPUT)
	...
	{
	  if ((((GdkVisualPrivate *)
		gdk_visual_get_system_for_screen (screen))->xvisual) ==
	      xvisual)
	    {
	      draw_impl->colormap =
		gdk_colormap_get_system_for_screen (screen);
  ...
  switch (GDK_WINDOW_TYPE (private))
    ...
    case GDK_WINDOW_CHILD:
	  ...
	  && (draw_impl->colormap !=
	      gdk_window_get_colormap (gdk_window_get_toplevel (window))))
	{
  ...
  if (class != InputOnly)
    {
       ...
       */
      impl->focus_window = XCreateSimpleWindow (GDK_WINDOW_XDISPLAY (window),
       ...
       */
      XSelectInput (GDK_WINDOW_XDISPLAY (window), impl->focus_window,
		    KeyPressMask | KeyReleaseMask | FocusChangeMask);

      XMapWindow (GDK_WINDOW_XDISPLAY (window), impl->focus_window);
  ...
  if (!GDK_DISPLAY_IMPL_X11 (scr_impl->display)->wm_client_leader_atom)
    GDK_DISPLAY_IMPL_X11 (scr_impl->display)->wm_client_leader_atom =
      gdk_x11_get_real_atom_by_name (scr_impl->display, "WM_CLIENT_LEADER");


  XChangeProperty (GDK_WINDOW_XDISPLAY (window),
		   GDK_WINDOW_XID (window),
		   GDK_DISPLAY_IMPL_X11 (GDK_SCREEN_IMPL_X11
					 (GDK_WINDOW_SCREEN (window))->
					 display)->wm_client_leader_atom,
		   XA_WINDOW, 32, PropModeReplace,
		   (guchar *) & scr_impl->leader_window, 1);
		...
		GdkWindowAttr * attributes, gint attributes_mask)
{
  ...
  if (parent)
  ...
  else
    {
      GDK_NOTE (MULTIHEAD,
		g_message
		("Use gdk_window_new_for_screen instead instead\n"));
				    ...
				    GdkNativeWindow anid)
{
  ...
  gdk_error_trap_push ();
  result = XGetWindowAttributes (dpy_impl->xdisplay, anid, &attrs);
  ...
  gdk_error_trap_push ();
  result =
    XQueryTree (dpy_impl->xdisplay, anid, &root, &parent, &children,
		&nchildren);
  if (gdk_error_trap_pop () || !result)
    return NULL;
...
{
  return gdk_window_foreign_new_for_display (gdk_get_default_display (),
					     anid);
}

void
_gdk_windowing_window_destroy (GdkWindow * window,
			       gboolean recursing, gboolean foreign_destroy)
{
  ...
  if (private->window_type == GDK_WINDOW_FOREIGN)
      ...
      if (!foreign_destroy && (private->parent != NULL))
	{
	  ...
	  XSendEvent (GDK_WINDOW_XDISPLAY (window),
		      GDK_WINDOW_XID (window), False, 0, (XEvent *) & xevent);
...
gdk_window_destroy_notify (GdkWindow * window)
{
  ...
  if (!GDK_WINDOW_DESTROYED (window))
	...
	g_warning ("GdkWindow %#lx unexpectedly destroyed",
		   GDK_WINDOW_XID (window));
...
set_initial_hints (GdkWindow * window)
{
  ...
  if (private->state & GDK_WINDOW_STATE_MAXIMIZED)
    {
      atoms[i] = gdk_x11_get_real_atom_by_name (GDK_WINDOW_DISPLAY (window),
  ...
  if (i > 0)
    {
      XChangeProperty (GDK_WINDOW_XDISPLAY (window),
		       GDK_WINDOW_XID (window),
		       gdk_x11_get_real_atom_by_name (GDK_WINDOW_DISPLAY
						      (window),
						      "_NET_WM_STATE"),
		       XA_ATOM, 32, PropModeReplace, (guchar *) atoms, i);
    }
  ...
  if (private->state & GDK_WINDOW_STATE_STICKY)
    {
      ...
      XChangeProperty (GDK_WINDOW_XDISPLAY (window),
		       ...
		       gdk_x11_get_real_atom_by_name (GDK_WINDOW_DISPLAY
						      (window),
						      "_NET_WM_DESKTOP"),
		       XA_CARDINAL, 32, PropModeReplace, (guchar *) atoms, 1);
    }
}
  ...
  if (!GDK_WINDOW_DESTROYED (window))
      ...
      else
	{
	  ...
	  XResizeWindow (GDK_WINDOW_XDISPLAY (window),
			 GDK_WINDOW_XID (window), width, height);
	}
    }
}
  ...
  switch (GDK_WINDOW_TYPE (new_parent))
      ...
      if (GDK_WINDOW_TYPE (window) == GDK_WINDOW_CHILD)
	{
	  ...
	  XSetWMProtocols (GDK_WINDOW_XDISPLAY (window),
			   GDK_WINDOW_XID (window),
			   GDK_DISPLAY_IMPL_X11 (GDK_WINDOW_DISPLAY
						 (window))->
			   gdk_wm_window_protocols, 3);
	}
    case GDK_WINDOW_TOPLEVEL:
...
gdk_window_focus (GdkWindow * window, guint32 timestamp)
{
  ...
  if (gdk_net_wm_supports_for_screen (GDK_WINDOW_SCREEN (window),
	...
	gdk_x11_get_real_atom_by_name (GDK_WINDOW_DISPLAY (window),
      ...
      XSendEvent (GDK_WINDOW_XDISPLAY (window),
		  GDK_SCREEN_IMPL_X11 (GDK_WINDOW_SCREEN (window))->
		  xroot_window, False,
  ...
  else
    {
      ...
      XSetInputFocus (GDK_WINDOW_XDISPLAY (window),
		      GDK_WINDOW_XWINDOW (window), RevertToNone, timestamp);
...
gdk_window_set_type_hint (GdkWindow * window, GdkWindowTypeHint hint)
{
  ...
  switch (hint)
    ...
    case GDK_WINDOW_TYPE_HINT_DIALOG:
      atom = gdk_x11_get_real_atom_by_name (GDK_WINDOW_DISPLAY (window),
...
static void
gdk_wmspec_change_state (gboolean add, GdkWindow * window, GdkAtom state1,	/* state1 and state2 are X Atoms */
			 GdkAtom state2)
{
  XEvent xev;
  Atom op;
  ...
  if (add)
    op = gdk_x11_get_real_atom_by_name (GDK_WINDOW_DISPLAY (window),
					"_NET_WM_STATE_ADD");
  else
    op = gdk_x11_get_real_atom_by_name (GDK_WINDOW_DISPLAY (window),
  ...
  XSendEvent (GDK_WINDOW_XDISPLAY (window), GDK_WINDOW_XROOTWIN (window),
  ...
  if (size_hints.flags & PAspect)
    {
      *geom_mask |= GDK_HINT_ASPECT;
      ...
      geometry->min_aspect =
	(gdouble) size_hints.min_aspect.x / (gdouble) size_hints.min_aspect.y;
      geometry->max_aspect =
	(gdouble) size_hints.max_aspect.x / (gdouble) size_hints.max_aspect.y;
    }
 ...
 */
static void
set_text_property (GdkWindow * window, GdkAtom property,	/* property is an X atom */
		   const gchar * utf8_str)
{
  ...
  if (utf8_is_latin1 (utf8_str))
    ...
    {
      gdk_utf8_to_compound_text_for_display (gdk_drawable_get_display
					     (window), utf8_str, &prop_type,
					     &prop_format, &prop_text,
					     &prop_length);
    }
  ...
  if (prop_text)
    {
      XChangeProperty (GDK_WINDOW_XDISPLAY (window),
		       ...
		       PropModeReplace, prop_text, prop_length);
...
gdk_window_set_transient_for (GdkWindow * window, GdkWindow * parent)
{
  ...
  if (!GDK_WINDOW_DESTROYED (window) && !GDK_WINDOW_DESTROYED (parent))
    XSetTransientForHint (GDK_WINDOW_XDISPLAY (window),
			  GDK_WINDOW_XID (window), GDK_WINDOW_XID (parent));
}
...
gdk_window_set_back_pixmap (GdkWindow * window,
			    GdkPixmap * pixmap, gboolean parent_relative)
{
  GdkWindowObject *private = (GdkWindowObject *) window;
  Pixmap xpixmap;
...
gdk_window_set_cursor (GdkWindow * window, GdkCursor * cursor)
{
  ...
  if (!GDK_WINDOW_DESTROYED (window))
    XDefineCursor (GDK_WINDOW_XDISPLAY (window),
		   GDK_WINDOW_XID (window), xcursor);
}

void
gdk_window_get_geometry (GdkWindow * window,
			 gint * x,
			 gint * y, gint * width, gint * height, gint * depth)
{
  Window root;
  gint tx;
  gint ty;
  ...
  if (!GDK_WINDOW_DESTROYED (window))
    {
      XGetGeometry (GDK_WINDOW_XDISPLAY (window),
		    GDK_WINDOW_XID (window),
		    &root, &tx, &ty, &twidth, &theight, &tborder_width,
		    &tdepth);
  ...
  do
    {
      ...
      if (!XQueryTree (GDK_WINDOW_XDISPLAY (window), xwindow,
		       &root, &xparent, &children, &nchildren))
	return;
  ...
  if (xparent == root)
    {
      ...
      if (XGetGeometry
	  (GDK_WINDOW_XDISPLAY (window), xwindow, &root, &wx, &wy, &ww, &wh,
	   &wb, &wd))
	{
	  rect->x = wx;
	  rect->y = wy;
  ...
  if (!window)
    {
      GDK_NOTE (MULTIHEAD,
		g_message
		("window arg is need for multihead safe operation\n"));
      window = GDK_SCREEN_IMPL_X11 (gdk_get_default_screen ())->root_window;
    }
...
GdkWindow *
_gdk_windowing_window_at_pointer (gint * win_x, gint * win_y)
{
  GDK_NOTE (MULTIHEAD,
	    g_message ("Use gdk_screen_get_window_at_pointer instead\n"));
  return gdk_screen_get_window_at_pointer (gdk_get_default_screen (), win_x,
					   win_y);
}
  ...
  if (!GDK_WINDOW_DESTROYED (window))
      ...
      for (i = 0; i < gdk_nevent_masks; i++)
	{
      ...
      XSelectInput (GDK_WINDOW_XDISPLAY (window),
		    GDK_WINDOW_XID (window), xevent_mask);
    }
}
...
gdk_window_add_colormap_windows (GdkWindow * window)
{
  ...
  XSetWMColormapWindows (GDK_WINDOW_XDISPLAY (toplevel),
			 GDK_WINDOW_XID (toplevel), new_windows, count + 1);
  ...
  if (gdk_window_have_shape_ext (GDK_WINDOW_DISPLAY (window)))
      ...
      else
	{
      ...
      XShapeCombineMask (GDK_WINDOW_XDISPLAY (window),
			 GDK_WINDOW_XID (window),
			 ShapeBounding, x, y, pixmap, ShapeSet);
    }
  ...
  if (gdk_window_have_shape_ext (GDK_WINDOW_DISPLAY (window)))
    {
      ...
      _gdk_region_get_xrectangles (shape_region, 0, 0, &xrects, &n_rects);
...
gdk_window_set_icon_list (GdkWindow * window, GList * pixbufs)
{
  ...
  XChangeProperty (GDK_WINDOW_XDISPLAY (window),
		   ...
		   gdk_x11_get_real_atom_by_name (GDK_WINDOW_DISPLAY (window),
						  "_NET_WM_ICON"),
		   XA_CARDINAL, 32, PropModeReplace, (guchar *) data, size);
...
gdk_window_iconify (GdkWindow * window)
{
  ...
  if (GDK_WINDOW_IS_MAPPED (window))
    {
      XIconifyWindow (display, GDK_WINDOW_XWINDOW (window),
  ...
  else
    {
      ...
      gdk_synthesize_window_state (window, 0, GDK_WINDOW_STATE_ICONIFIED);
    }
}
  ...
  if (GDK_WINDOW_IS_MAPPED (window))
    {
      ...
      gdk_wmspec_change_state (TRUE, window,
			       gdk_x11_get_real_atom_by_name (scr_impl->
							      display,
							      "_NET_WM_STATE_STICKY"),
			       0);
      ...
      xev.xclient.display = GDK_WINDOW_XDISPLAY (window);
      xev.xclient.message_type =
	gdk_x11_get_real_atom_by_name (scr_impl->display, "_NET_WM_DESKTOP");
    ...
    {
      /* Flip our client side flag, the real work happens on map. */
      gdk_synthesize_window_state (window, 0, GDK_WINDOW_STATE_STICKY);
    }
}
...
gdk_window_unstick (GdkWindow * window)
{
  ...
  if (GDK_WINDOW_IS_MAPPED (window))
    ...
    {
      /* Flip our client side flag, the real work happens on map. */
      gdk_synthesize_window_state (window, GDK_WINDOW_STATE_STICKY, 0);
...
static void
gdk_window_set_mwm_hints (GdkWindow * window, MotifWmHints * new_hints)
{
  static Atom hints_atom = None;
  MotifWmHints *hints;
  Atom type;
...
static void
gdk_add_to_span (struct _gdk_span **s, gint x, gint xx)
{
  ...
  while (ptr1)
      ...
      else
	{
	  ...
	  /* case: ===##---- */
	  else if ((x >= ptr1->start) && (x <= ptr1->end + 1)
		   && (xx > ptr1->end))
	    {
	      ss = ptr1;
	      spanning = 1;
      ...
      for (i = 0; i < baseh; i++)
	{
	  ...
	  while (ptr1)
	      ...
	      while ((contig) && (j < baseh))
		{
	      ...
	      if ((num_rects % 256) == 0)
		rects =
		  g_realloc (rects, sizeof (XRectangle) * (num_rects + 256));
	      ptr1 = ptr1->next;
	    }
	}
  ...
  if (gravity_works == UNKNOWN)
    {
       ...
       */
      if ((!strcmp
	   (XServerVendor (GDK_DISPLAY_IMPL_X11 (display)->xdisplay),
	    "Sun Microsystems, Inc."))
	  && (VendorRelease (GDK_DISPLAY_IMPL_X11 (display)->xdisplay) ==
	      3400))
	{
	  gravity_works = NO;
	  return FALSE;
...
Window
gdk_window_xid_at (Display * xdisplay,
		   Window base,
		   gint bx,
		   gint by,
		   gint x, gint y, GList * excludes, gboolean excl_child)
{
  Window *list = NULL;
  ...
  if (list)
	...
	{
	  if ((!excl_child)
	      || (!g_list_find (excludes, (gpointer *) list[i])))
	    {
	      if ((child =
		   gdk_window_xid_at (xdisplay, list[i], wx, wy, x, y,
				      excludes, excl_child)) != 0)
		{
		  XFree (list);
		  return child;
		}
...
Window
gdk_window_xid_at_coords_for_screen (gint x,
				     gint y,
				     GList * excludes,
				     gboolean excl_child, GdkScreen * screen)
{
  GdkWindow *window;
  Display *xdisplay;
  Window *list = NULL;
  ...
  if (list)
      ...
      do
	{
	  ...
	  if ((child =
	       gdk_window_xid_at (xdisplay, list[i], 0, 0, x, y, excludes,
				  excl_child)) == 0)
	    continue;
...
gdk_window_xid_at_coords (gint x,
			  gint y, GList * excludes, gboolean excl_child)
{
  return gdk_window_xid_at_coords_for_screen (x, y, excludes, excl_child,
...
static void
update_pos_for_display (GdkDisplay * display,
			gint new_root_x, gint new_root_y)
{
  gint dx, dy;
...
static int
lookahead_motion_predicate (Display * display, XEvent * event, XPointer arg)
{
...
_gdk_moveresize_handle_event (XEvent * event)
{
  ...
  switch (event->xany.type)
	...
	break;
      ...
      update_pos_for_display ((GdkDisplay *) dpy, event->xmotion.x_root,
...
static void
create_moveresize_window_for_screen (GdkScreen * scr, guint32 timestamp)
{
  ...
  status = gdk_pointer_grab (mv_resize->moveresize_emulation_window,
			     ...
			     GDK_POINTER_MOTION_MASK, FALSE, NULL, timestamp);
		     ...
		     gint button, gint root_x, gint root_y, guint32 timestamp)
{
  ...
  mv_resize->_gdk_moveresize_window =
			      ...
			      gint root_x, gint root_y, guint32 timestamp)
{
  ...
  if (gdk_net_wm_supports_for_screen (GDK_WINDOW_SCREEN (window),
				      gdk_atom_intern ("_NET_WM_MOVERESIZE",
						       FALSE)))
...
void
gdk_window_set_screen (GdkWindow * window, GdkScreen * screen)
{
  GdkWindowObject *private;
  GdkDrawableImplX11 *draw_impl;
  private = (GdkWindowObject *) window;
