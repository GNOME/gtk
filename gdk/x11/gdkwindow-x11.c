		...
		gint           attributes_mask)
{
  ...
  if (attributes->wclass == GDK_INPUT_OUTPUT)
	...
	{
	  if ((((GdkVisualPrivate *)gdk_visual_get_system_for_screen (screen))->xvisual) ==  xvisual)
            {
	      draw_impl->colormap =
				    ...
				    GdkNativeWindow anid)
{
   ...
   *        the parent */
  gdk_error_trap_push ();
  result = XQueryTree (display_impl->xdisplay, anid, &root, &parent, &children, &nchildren);
  if (gdk_error_trap_pop () || !result)
    return NULL;
  ...
  if (utf8_is_latin1 (utf8_str))
  ...
  else
    {
      gdk_utf8_to_compound_text_for_display (gdk_drawable_get_display (window),
					     utf8_str, &prop_type, &prop_format,
  ...
  if (prop_text)
    {
      XChangeProperty (GDK_WINDOW_XDISPLAY (window),
		       GDK_WINDOW_XID (window),
		       gdk_x11_get_real_atom (GDK_WINDOW_DISPLAY (window), property),
  ...
  if (GDK_WINDOW_IS_MAPPED (window))
    {
      ...
      xev.xclient.display = GDK_WINDOW_XDISPLAY (window);
      xev.xclient.message_type =
	gdk_x11_get_real_atom_by_name (screen_impl->display, "_NET_WM_DESKTOP");
      xev.xclient.format = 32;

      xev.xclient.data.l[0] = 0xFFFFFFFF;

      XSendEvent (GDK_WINDOW_XDISPLAY (window), screen_impl->xroot_window, False,
                  SubstructureRedirectMask | SubstructureNotifyMask,
                  &xev);
    }
  else
  ...
  if (GDK_WINDOW_IS_MAPPED (window))
    {
      ...
      gdk_wmspec_change_state (FALSE, window,
			       gdk_x11_get_real_atom_by_name (GDK_WINDOW_DISPLAY (window), "_NET_WM_STATE_STICKY"),
			       0);
      ...
      if (type == XA_CARDINAL)
        {
          ...
          xev.xclient.display = GDK_WINDOW_XDISPLAY (window);
	  xev.xclient.message_type =
	    gdk_x11_get_real_atom_by_name (GDK_WINDOW_DISPLAY (window), "_NET_WM_DESKTOP");
          xev.xclient.format = 32;
  ...
  if (gravity_works == UNKNOWN)
    {
       ...
       */
      if ((!strcmp (XServerVendor (GDK_DISPLAY_IMPL_X11 (display)->xdisplay), "Sun Microsystems, Inc.")) &&
