...
static gint	 gdk_event_apply_filters (XEvent   *xevent,
					  GdkEvent *event,
					  GList    *filters);
static gint	 gdk_event_translate	 (GdkDisplay *display,
					  GdkEvent   *event, 
					  XEvent     *xevent,
...
static gboolean
gdk_check_xpending (GdkDisplaySource* source)
{

  if (!source)
    {
      GList *tmp_list = display_sources;
  
      while (tmp_list)
	{
	  GdkDisplaySource *tmp_source = tmp_list->data;
	  if (XPending (GDK_DISPLAY_XDISPLAY (tmp_source->display)))
	    return TRUE;
    ...
    {
      return XPending (GDK_DISPLAY_XDISPLAY (source->display));
    }
}
...
gdk_events_init (GdkDisplay* display)
{
  ...
  gdk_add_client_message_filter (gdk_x11_get_virtual_atom (display, display_impl->gdk_wm_protocols), 
		     ...
		     gboolean    return_exposes)
{
  ...
  GdkScreenImplX11 *screen_impl = NULL;
  GdkDisplayImplX11 *display_impl = GDK_DISPLAY_IMPL_X11 (display);
  ...
  if (screen_impl && screen_impl->wmspec_check_window != None &&
      xevent->xany.window == screen_impl->wmspec_check_window)
    {
      if (xevent->type == DestroyNotify)
...
#define KEYBOARD_GROUP_MASK ((1 << 13) | (1 << 14))
    ...
    case KeyRelease:
      ...
      if (!display_impl->have_xkb_autorepeat && XPending (xevent->xkey.display))
	{
	  XEvent next_event;
      ...
      if (xevent->xdestroywindow.window == xevent->xdestroywindow.event)
	{
	  ...
	  if (window && GDK_WINDOW_XID (window) != screen_impl->xroot_window)
    ...
    case PropertyNotify:
      ...
      event->property.atom = gdk_x11_get_virtual_atom (display, xevent->xproperty.atom);
      event->property.time = xevent->xproperty.time;
      event->property.state = xevent->xproperty.state;

      if (xevent->xproperty.atom == display_impl->wm_state_atom ||
	  xevent->xproperty.atom == display_impl->wm_desktop_atom)
        {
      ...
      if (_gdk_selection_filter_clear_event (&xevent->xselectionclear))
	{
	  ...
	  event->selection.selection = 
	          gdk_x11_get_virtual_atom (display, xevent->xselectionclear.selection);
    ...
    case SelectionRequest:
      ...
      event->selection.window = window;
      event->selection.selection = 
	      gdk_x11_get_virtual_atom (display, xevent->xselectionrequest.selection);
      event->selection.target = 
	      gdk_x11_get_virtual_atom (display, xevent->xselectionrequest.target);
      event->selection.property =
	      gdk_x11_get_virtual_atom (display, xevent->xselectionrequest.property);
    ...
    case SelectionNotify:
      ...
      event->selection.window = window;
      event->selection.selection = 
	      gdk_x11_get_virtual_atom (display, xevent->xselection.selection);
      event->selection.target = 
	      gdk_x11_get_virtual_atom (display, xevent->xselection.target);
      event->selection.property = 
	      gdk_x11_get_virtual_atom (display, xevent->xselection.property);
	...
	while (tmp_list)
	  {
	    ...
	    if (filter->type == gdk_x11_get_virtual_atom (display, xevent->xclient.message_type))
	      {
		result = (*filter->function) (xevent, event, filter->data);
		break;
	      }
	    ...
	    /* Send unknown ClientMessage's on to Gtk for it to use */
            if (window_private == NULL)
            ...
            else
              {
                ...
                event->client.message_type = gdk_x11_get_virtual_atom (display, xevent->xclient.message_type);
    ...
    default:
#ifdef HAVE_XKB
      if (xevent->type == display_impl->xkb_event_type)
	{
			 ...
			 gpointer data)
{
  ...
  GdkDisplayImplX11 *display_impl = GDK_DISPLAY_IMPL_X11 (GDK_WINDOW_DISPLAY (win));

  if ((Atom) xevent->xclient.data.l[0] == display_impl->gdk_wm_delete_window)
    {
  ...
  else if ((Atom) xevent->xclient.data.l[0] == display_impl->gdk_wm_take_focus)
    {
      GdkWindow *win = event->any.window;
  ...
  else if ((Atom) xevent->xclient.data.l[0] == display_impl->gdk_wm_window_protocols[2])
    {
      XEvent xev = *xevent;
					      ...
					      guint    level)
{
  ...
  XGetWindowProperty (GDK_DISPLAY_XDISPLAY (display), xid, 
		      GDK_DISPLAY_IMPL_X11(display)->wm_state_atom, 0, 0, False, AnyPropertyType,
		     ...
		     XPointer arg)
{
  ...
  GdkDisplayImplX11 *display_impl = GDK_DISPLAY_IMPL_X11 (gdk_x11_display_manager_get_display (_gdk_display_manager, display));

  if (xevent->type == PropertyNotify &&
      xevent->xproperty.window == xwindow &&
      xevent->xproperty.atom == display_impl->timestamp_prop_atom)
    return True;
				...
				GdkAtom    property)
{
  ...
  if (screen_impl->wmspec_check_window != None)
    {
      ...
      while (i < n_atoms)
        {
          if (atoms[i] == gdk_x11_get_real_atom (gdk_screen_get_display (screen), property))
            return TRUE;
  ...
  XGetWindowProperty (screen_impl->xdisplay, screen_impl->xroot_window,
		      display_impl->wmspec_check_atom, 0, G_MAXLONG,
  ...
  XSelectInput (screen_impl->xdisplay, *xwindow, StructureNotifyMask);

  gdk_display_sync (screen_impl->display);
  ...
  XGetWindowProperty (screen_impl->xdisplay, screen_impl->xroot_window,
		      display_impl->wmspec_supported_atom, 0, G_MAXLONG,
  ...
  screen_impl->wmspec_check_window = *xwindow;
  XFree (xwindow);
