...
_gdk_quartz_event_should_be_handled_natively (NSEvent* event)
{
   ...
   * we pass through only events related to keyboard that may relate to standard
   * MacOS shortcuts like switch app window (Cmd+` etc.)
   *
   * QUESTION: Should we really do that? Shouldn't we pass all unhandled events back to OS?
   */
