  ...
  if (pixbuf->has_alpha)
    {
      ...
      if (visual)
	{
	  gint bits_per_pixel = _gdk_windowing_get_bits_for_depth 
	    (gdk_drawable_get_display (drawable), visual->depth);
      ...
      if (composite_func && !(dither == GDK_RGB_DITHER_MAX && visual->depth != 24))
	{
	  ...
	  for (y0 = 0; y0 < height; y0 += GDK_SCRATCH_IMAGE_HEIGHT)
	      ...
	      for (x0 = 0; x0 < width; x0 += GDK_SCRATCH_IMAGE_WIDTH)
		{
		  ...
		  GdkImage *image = _gdk_image_get_scratch (gdk_drawable_get_screen (drawable), width1, height1, gdk_drawable_get_depth (drawable), &xs0, &ys0);
