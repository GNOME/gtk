    ...
    case GDK_WAYLAND_SHELL_VARIANT_XDG_SHELL:
      {
        ...
        if (!(anchor_hints & GDK_ANCHOR_STATIC) &&
        ...
        if (ack_parent_configure &&
            xdg_positioner_get_version (positioner) >=
            XDG_POSITIONER_SET_ACK_PARENT_CONFIGURE_SINCE_VERSION)
          {
            ...
            xdg_positioner_set_ack_parent_configure (positioner,
                        ...
                        GdkSurface *parent)
{
  ...
  top_most_popup = g_list_last (display_wayland->current_grabbing_popups)->data;
  ...
  if (grab_input_seat)
    {
      display->current_grabbing_popups =
        g_list_append (display->current_grabbing_popups, surface);
    }
}
                         ...
                         GdkPopupLayout *layout)
{
  ...
  gdk_wayland_surface_hide_surface (surface);
  gdk_surface_show_popup (surface,
                   ...
                   GdkPopupLayout *layout)
{
  ...
  g_clear_pointer (&impl->popup.layout, gdk_popup_layout_unref);
  impl->popup.layout = gdk_popup_layout_ref (layout);
  ...
  xdg_surface_sync_with (parent_impl->display_server.xdg_surface,
                         impl->display_server.xdg_surface);
...
static void
gdk_wayland_surface_show_popup (GdkSurface     *surface,
                                int             width,
                                int             height,
                                GdkPopupLayout *layout)
{
...
static void
gdk_wayland_surface_layout_popup (GdkSurface     *surface,
                                  int             width,
                                  int             height,
                                  GdkPopupLayout *layout)
{
  ...
  if (!impl->mapped)
    {
      gdk_wayland_surface_show_popup (surface, width, height, layout);
      gdk_surface_freeze_updates (surface);
    }
  else
    {
...
gdk_wayland_surface_class_init (GdkWaylandSurfaceClass *klass)
{
  ...
  impl_class->show_popup = gdk_wayland_surface_show_popup;
  impl_class->layout_popup = gdk_wayland_surface_layout_popup;
