...
struct _GdkWaylandSurface
{
  ...
  uint32_t received_popup_move_token;
  ...
  gulong parent_remap_begin_handler_id;
  gulong parent_remap_end_handler_id;
};
...
static void on_parent_remap_begin (GdkSurface *parent,
                                   GdkSurface *surface);
static void on_parent_remap_end (GdkSurface *parent,
                                 GdkSurface *surface);
...
gdk_wayland_surface_dispose (GObject *object)
{
  ...
  g_clear_signal_handler (&impl->parent_remap_begin_handler_id,
                          impl->popup_parent);
  g_clear_signal_handler (&impl->parent_remap_end_handler_id,
                          impl->popup_parent);
...
gdk_wayland_surface_configure_popup (GdkSurface *surface)
{
  ...
  if (!impl->pending_relayout.is_sync)
    {
                        ...
                        uint32_t          token)
{
  ...
  impl->received_popup_move_token = token;
}
                      ...
                      GdkRectangle *out_rect)
{
  ...
  width = (impl->pending_relayout.unconstrained_width -
           (impl->margin_left + impl->margin_right));
  height = (impl->pending_relayout.unconstrained_height -
    ...
    case GDK_WAYLAND_SHELL_VARIANT_XDG_SHELL:
      {
        ...
        if (parent &&
            xdg_positioner_get_version (positioner) >=
            XDG_POSITIONER_SET_PARENT_CONFIGURE_SERIAL_SINCE_VERSION)
          {
                                      ...
                                      GdkWaylandSeat *grab_input_seat)
{
  ...
  impl->parent_remap_begin_handler_id =
    g_signal_connect (parent, "popup-remap-begin",
                      G_CALLBACK (on_parent_remap_begin),
                      surface);
  impl->parent_remap_end_handler_id =
    g_signal_connect (parent, "popup-remap-end",
                      G_CALLBACK (on_parent_remap_end),
                      surface);
}
...
static void
on_parent_remap_begin (GdkSurface *parent,
                       GdkSurface *surface)
{
  g_signal_emit (surface, signals[POPUP_REMAP_BEGIN], 0);

  gdk_wayland_surface_hide (surface);
}

static void
on_parent_remap_end (GdkSurface *parent,
                     GdkSurface *surface)
{
  GdkWaylandSurface *impl = GDK_WAYLAND_SURFACE (surface);
  ...
  g_signal_emit (surface, signals[POPUP_REMAP_END], 0);
}
                                    ...
                                    gint                rect_anchor_dy)
{
  ...
  if (width == 0 || height == 0)
    {
      impl->pending_relayout.unconstrained_width = surface->width;
      impl->pending_relayout.unconstrained_height = surface->height;
    }
  else
    {
      impl->pending_relayout.unconstrained_width = width;
  ...
  impl->pending_relayout.anchor_rect = sanitized_rect;
  impl->pending_relayout.rect_anchor = rect_anchor;
  impl->pending_relayout.surface_anchor = surface_anchor;
  impl->pending_relayout.anchor_hints = anchor_hints;
  impl->pending_relayout.rect_anchor_dx = rect_anchor_dx;
...
is_relayout_finished (GdkSurface *surface)
{
  ...
  if (impl->popup_move_token != impl->received_popup_move_token)
    return FALSE;
                                     ...
                                     gboolean     *flipped_y)
{
  ...
  g_assert (!impl->pending_relayout.is_sync);
  impl->pending_relayout.is_sync = TRUE;
  ...
  if (flipped_rect)
    *flipped_rect = impl->last_layout.flipped_rect;
  if (final_rect)
    *final_rect = impl->last_layout.final_rect;
  if (flipped_x)
    *flipped_x = impl->last_layout.flipped_x;
  if (flipped_y)
    *flipped_y = impl->last_layout.flipped_y;

  impl->pending_relayout.is_sync = FALSE;
...
gdk_wayland_surface_class_init (GdkWaylandSurfaceClass *klass)
{
  ...
  signals[POPUP_REMAP_BEGIN] = g_signal_new (g_intern_static_string ("popup-remap-begin"),
                                             G_TYPE_FROM_CLASS (object_class),
                                             G_SIGNAL_RUN_LAST,
                                             0,
                                             NULL, NULL, NULL,
  ...
  signals[POPUP_REMAP_END] = g_signal_new (g_intern_static_string ("popup-remap-end"),
                                           G_TYPE_FROM_CLASS (object_class),
                                           G_SIGNAL_RUN_LAST,
                                           0,
                                           NULL, NULL, NULL,
