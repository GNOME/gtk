...
#include "gdkpopuplayoutprivate.h"
 ...
 * gdk_surface_show_popup() and gdk_surface_layout_popup() to describe how a
 * popup should be placed and behave on-screen.
 *
 ...
 * on @rect and @surface to pin together.
 *
...
{
  g_return_if_fail (!layout->is_sealed);
...
void
gdk_popup_layout_seal (GdkPopupLayout *layout)
{
  layout->is_sealed = TRUE;
}
