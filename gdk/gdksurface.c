                     ...
                     gpointer    user_data)
{
  ...
  GDK_SURFACE_GET_CLASS (surface)->show_popup (surface,
                                               data->width,
                                               data->height,
                                               data->layout);
}
 ...
 *
 * Show @surface after having processed the #GdkPopupLayout rules. See
 * gdk_surface_layout_popup() for details of the popup layout is processed.
 *
 * After calling this function, the result of the positioning can be queried.
 * See gdk_surface_layout_popup() for details.
 *
 * The showing may have failed to finish, for example if it was immediately
 * automatically hidden if the @surface was set to autohide.
 */
void
gdk_surface_show_popup (GdkSurface     *surface,
                        int             width,
                        int             height,
                        GdkPopupLayout *layout)
{
...
static void
layout_grabbing_popup (GdkSeat    *seat,
                       GdkSurface *surface,
                       gpointer    user_data)
{
  GrabPrepareData *data = user_data;

  GDK_SURFACE_GET_CLASS (surface)->layout_popup (surface,
                                                 data->width,
                                                 data->height,
                                                 data->layout);
}
...
/**
 * gdk_surface_layout_popup:
 * @surface: the popup #GdkSurface to show
 * @width: the unconstrained popup width to layout
 * @height: the unconstrained popup height to layout
 * @layout: the #GdkPopupLayout object used to layout
 *
 * Reposition the popup according to the new size and #GdkPopupLayout.
 *
 * After calling this function, the result of the repositioning can be queried
 ...
 */
void
gdk_surface_layout_popup (GdkSurface     *surface,
                          int             width,
                          int             height,
                          GdkPopupLayout *layout)
{
  ...
  if (surface->autohide && !GDK_SURFACE_IS_MAPPED (surface))
    {
      GrabPrepareData data;
