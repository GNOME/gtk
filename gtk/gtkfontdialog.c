                     ...
                     GtkFontChooserLevel   level)
{
  ...
  window = GTK_FONT_CHOOSER_DIALOG (gtk_font_chooser_dialog_new (title, parent));
                                    ...
                                    GError        **error)
{
  ...
  g_return_val_if_fail (g_task_get_source_tag (task) == gtk_font_dialog_choose_font, NULL);

  font_result = g_task_propagate_pointer (task, error);
  if (font_result)
    {
      g_assert (font_result->font_features == NULL);
      return g_steal_pointer (&font_result->font_desc);
    }
                                                 ...
                                                 GError               **error)
{
  ...
  g_return_val_if_fail (g_task_get_source_tag (task) == gtk_font_dialog_choose_font_and_features, FALSE);
