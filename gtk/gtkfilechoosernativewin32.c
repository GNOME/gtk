              ...
              if (choice->options)
                {
                  ...
                  for (gchar **option = choice->options; *option != NULL; option++, sub_id++) /*TODO const*/
                    {
                      gunichar2 *option_label = g_utf8_to_utf16 (choice->option_labels[sub_id], /*ugly*/
                                                                 -1, NULL, NULL, NULL);
                  ...
                  if (choice->options)
                    {
                      ...
                      hr = IFileDialogCustomize_GetSelectedControlItem (pfdc,
                                                                        ...
                                                                        &dialog_sub_item_id);
                      if (FAILED (hr))
                        g_warning_hr ("Can't get choice selection", hr);
                      else
