...
node_dump (TreeNode    *node,
           ...
           guint        depth)
{
  g_print ("%*s %p %u %u %u\n", 2 * depth, "", node, aug->n_items, aug->n_local, tree_node_get_n_children (node));
  if (node->children)
    { 
      TreeNode *child;
      for (child = gtk_css_rb_tree_get_first (node->children);
           child != NULL;
           child = gtk_css_rb_tree_get_next (node->children, child))
        {
          node_dump (child, gtk_css_rb_tree_get_augment (node->children, child), depth + 1);
        }
    }
}
...
{
  node_dump (&self->root_node, &(TreeAugment){ 0,  }, 0);
}
