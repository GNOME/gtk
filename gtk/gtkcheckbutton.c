				      ...
				      GdkRectangle   *area)
{
  ...
  if (GTK_WIDGET_DRAWABLE (check_button))
      ...
      if (gdk_rectangle_intersect (area, &restrict_area, &new_area))
	{
      ...
      /* FIXME: What is the + 1 doing here?
       */
      x = widget->allocation.x + CHECK_BUTTON_CLASS (widget)->indicator_spacing + GTK_CONTAINER (widget)->border_width + 1;
      y = widget->allocation.y + (widget->allocation.height - CHECK_BUTTON_CLASS (widget)->indicator_size) / 2 + 1;
