 ...
 * the clist's hoffset */
#define COLUMN_LEFT_YPIXEL(clist, column)  ((clist)->column[(column)].area.x + \
					    (clist)->hoffset)
...
static void gtk_clist_size_allocate_columns (GtkCList * clist);
...
/* Scrollbars */
static void gtk_clist_adjust_scrollbars (GtkCList * clist);
static void gtk_clist_vadjustment_changed (GtkAdjustment * adjustment,
					   gpointer data);
static void gtk_clist_vadjustment_value_changed (GtkAdjustment * adjustment,
						 gpointer data);
static void gtk_clist_hadjustment_changed (GtkAdjustment * adjustment,
					   gpointer data);
static void gtk_clist_hadjustment_value_changed (GtkAdjustment * adjustment,
						 gpointer data);


/* Memory Allocation/Distruction Routines */
static GtkCListColumn *gtk_clist_columns_new (GtkCList * clist);

static void gtk_clist_column_title_new (GtkCList * clist,
					gint column,
					gchar * title);
static void gtk_clist_columns_delete (GtkCList * clist);

static GtkCListRow *gtk_clist_row_new (GtkCList * clist);

static void gtk_clist_row_delete (GtkCList * clist,
				  GtkCListRow * clist_row);
static void gtk_clist_cell_empty (GtkCList * clist,
				  GtkCListRow * clist_row,
				  gint column);
static void gtk_clist_cell_set_text (GtkCList * clist,
				     GtkCListRow * clist_row,
				     gint column,
				     gchar * text);
static void gtk_clist_cell_set_pixmap (GtkCList * clist,
				       GtkCListRow * clist_row,
				       gint column,
				       GdkPixmap * pixmap,
				       GdkBitmap * mask);
static void gtk_clist_cell_set_pixtext (GtkCList * clist,
					GtkCListRow * clist_row,
					gint column,
					gchar * text,
					guint8 spacing,
	       ...
	       gchar * titles[])
{
  ...
  clist->column = gtk_clist_columns_new (clist);
  ...
  /* create and connect scrollbars */
  clist->vscrollbar = gtk_vscrollbar_new (NULL);
  adjustment = gtk_range_get_adjustment (GTK_RANGE (clist->vscrollbar));

  gtk_signal_connect (GTK_OBJECT (adjustment), "changed",
		      (GtkSignalFunc) gtk_clist_vadjustment_changed,
		      (gpointer) clist);

  gtk_signal_connect (GTK_OBJECT (adjustment), "value_changed",
		      (GtkSignalFunc) gtk_clist_vadjustment_value_changed,
		      (gpointer) clist);
  ...
  gtk_signal_connect (GTK_OBJECT (adjustment), "changed",
		      (GtkSignalFunc) gtk_clist_hadjustment_changed,
		      (gpointer) clist);

  gtk_signal_connect (GTK_OBJECT (adjustment), "value_changed",
		      (GtkSignalFunc) gtk_clist_hadjustment_value_changed,
		      (gpointer) clist);
...
gtk_clist_thaw (GtkCList * clist)
{
  ...
  gtk_clist_adjust_scrollbars (clist);
		    ...
		    gchar * text)
{
  ...
  if (text)
    gtk_clist_cell_set_text (clist, clist_row, column, text);
  else
    gtk_clist_cell_empty (clist, clist_row, column);
		      ...
		      GdkBitmap * mask)
{
  ...
  gdk_pixmap_ref (mask);
  gtk_clist_cell_set_pixmap (clist, clist_row, column, pixmap, mask);
		       ...
		       GdkBitmap * mask)
{
  ...
  gdk_pixmap_ref (pixmap);
  gdk_pixmap_ref (mask);
  gtk_clist_cell_set_pixtext (clist, clist_row, column, text, spacing, pixmap, mask);
		     ...
		     gint horizontal)
{
  ...
  clist_row->cell[column].verticle = verticle;
...
gtk_clist_destroy (GtkObject * object)
{
  ...
  gtk_widget_destroy (clist->vscrollbar);
  gtk_widget_destroy (clist->hscrollbar);
...
gtk_clist_size_allocate_title_buttons (GtkCList * clist)
{
  ...
  for (i = 0; i < clist->columns; i++)
      ...
      else
	{
      ...
      gtk_widget_size_allocate (clist->column[i].button, &button_allocation);
      button_allocation.x += button_allocation.width;
    }
}

static void
gtk_clist_size_allocate_columns (GtkCList * clist)
{
  gint i, xoffset;
 ...
 *
 * functions:
 *   gtk_clist_adjust_scrollbars
 *   gtk_clist_vadjustment_changed
 *   gtk_clist_hadjustment_changed
 *   gtk_clist_vadjustment_value_changed
 *   gtk_clist_hadjustment_value_changed
 */
static void
gtk_clist_adjust_scrollbars (GtkCList * clist)
{
...
static void
gtk_clist_vadjustment_changed (GtkAdjustment * adjustment,
			       gpointer data)
{
  GtkCList *clist;
...
static void
gtk_clist_hadjustment_changed (GtkAdjustment * adjustment,
			       gpointer data)
{
  GtkCList *clist;
...
static void
gtk_clist_vadjustment_value_changed (GtkAdjustment * adjustment,
				     gpointer data)
{
  GtkCList *clist;
  GdkRectangle area;
...
static void
gtk_clist_hadjustment_value_changed (GtkAdjustment * adjustment,
				     gpointer data)
{
  GtkCList *clist;
  GdkRectangle area;
 ...
 *
 * functions:
 *   gtk_clist_columns_new
 *   gtk_clist_column_title_new
 *   gtk_clist_columns_delete
 *   gtk_clist_row_new
 *   gtk_clist_row_delete
 *   gtk_clist_cell_empty
 *   gtk_clist_cell_set_text
 *   gtk_clist_cell_set_pixmap
 */
static GtkCListColumn *
gtk_clist_columns_new (GtkCList * clist)
{
  gint i;
...
static void
gtk_clist_columns_delete (GtkCList * clist)
{
  gint i;
  ...
  for (i = 0; i < clist->columns; i++)
    {
      ...
      clist_row->cell[i].verticle = 0;
      clist_row->cell[i].horizontal = 0;
    }

  clist_row->foreground = GTK_WIDGET (clist)->style->fg[GTK_STATE_NORMAL];
  clist_row->background = GTK_WIDGET (clist)->style->bg[GTK_STATE_PRELIGHT];
  clist_row->state = GTK_STATE_NORMAL;
  clist_row->data = NULL;
		      ...
		      GtkCListRow * clist_row)
{
  ...
  for (i = 0; i < clist->columns; i++)
    gtk_clist_cell_empty (clist, clist_row, i);
...
static void
gtk_clist_cell_empty (GtkCList * clist,
		      GtkCListRow * clist_row,
		      gint column)
{
...
static void
gtk_clist_cell_set_text (GtkCList * clist,
			 GtkCListRow * clist_row,
			 gint column,
			 gchar * text)
{
...
static void
gtk_clist_cell_set_pixmap (GtkCList * clist,
			   GtkCListRow * clist_row,
			   gint column,
			   GdkPixmap * pixmap,
			   GdkBitmap * mask)
...
static void
gtk_clist_cell_set_pixtext (GtkCList * clist,
			    GtkCListRow * clist_row,
			    gint column,
			    gchar * text,
			    guint8 spacing,
