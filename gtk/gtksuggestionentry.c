...
gtk_suggestion_entry_class_init (GtkSuggestionEntryClass *klass)
{
   ...
   * not used, the entire suggestion is inserted automatically.
   **/
              ...
              if (p > prefix)
                {
                  ...
                  char *s = g_utf8_find_prev_char (prefix, p);
                  switch (g_utf8_get_char_validated (s, p - s))
                    {
                    case (gunichar)-2:
                    case (gunichar)-1:
                      *s = 0;
  ...
  if (self->insert_prefix)
    {
      ...
      text = gtk_editable_get_text (GTK_EDITABLE (self->entry));
      prefix = compute_prefix (self, text);
 ...
 *
 * Sets whether the currently selected item
 ...
 *
 * Sets whether the common prefix of the matching
 * selections should be automatically inserted in the entry.
 */
void
