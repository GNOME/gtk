...
static gboolean
gtk_text_view_accessible_scroll_substring_to_point(AtkText      *text,
                                                   gint          start_offset,
                                                   gint          end_offset,
                                                   AtkCoordType  coords,
                                                   gint          x,
                                                   gint          y)
{
  ...
  /* Assume that the API wants to place the begining of the substring
   * at (x, y). */
  offset = start_offset;
  gtk_text_buffer_get_iter_at_offset (buffer, &iter, offset);
  ...
  xalign = ((gdouble)(buff_x - rect.x)) / (rect.width - 1);
  yalign = ((gdouble)(buff_y - rect.y)) / (rect.height - 1);

  gtk_text_view_scroll_to_iter (view, &iter, 0, TRUE, xalign, yalign);
...
atk_text_interface_init (AtkTextIface *iface)
{
  ...
  iface->scroll_substring_to_point = gtk_text_view_accessible_scroll_substring_to_point;
}
