...
{
  _gtk_tree_rbtree_free (tree_view->priv->tree);
                                          ...
                                          GtkTreeView          *tree_view)
{
  ...
  if (new_y < 0)
    new_y = 0;
  y_offset = -_gtk_tree_rbtree_find_offset (tree_view->priv->tree, new_y, &tree, &node);
                                  ...
                                  GtkTreeView    *tree_view)
{
  ...
  _gtk_tree_rbtree_find_offset (tree_view->priv->tree, bin_y + tree_view->priv->dy,
                           &tree, &node);
                 ...
                 gint         y)
{
  ...
  _gtk_tree_rbtree_find_offset (tree_view->priv->tree,
                           new_y, &tree, &node);
...
gtk_tree_view_update_rubber_band_selection (GtkTreeView *tree_view)
{
  ...
  _gtk_tree_rbtree_find_offset (tree_view->priv->tree, MIN (tree_view->priv->press_start_y, bin_y), &start_tree, &start_node);
  _gtk_tree_rbtree_find_offset (tree_view->priv->tree, MAX (tree_view->priv->press_start_y, bin_y), &end_tree, &end_node);
  ...
  else if (_gtk_tree_rbtree_node_find_offset (start_tree, start_node) <
           _gtk_tree_rbtree_node_find_offset (tree_view->priv->rubber_band_start_tree, tree_view->priv->rubber_band_start_node))
    {
  ...
  if (!tree_view->priv->rubber_band_end_node)
  ...
  else if (!end_node)
    {
      ...
      _gtk_tree_rbtree_find_offset (tree_view->priv->tree, gtk_tree_view_get_height (tree_view) - 1,
			       &end_tree, &end_node);
  ...
  else if (_gtk_tree_rbtree_node_find_offset (end_tree, end_node) >
           _gtk_tree_rbtree_node_find_offset (tree_view->priv->rubber_band_end_tree, tree_view->priv->rubber_band_end_node))
    {
  ...
  if (tree_view->priv->tree)
    {
      ...
      _gtk_tree_rbtree_find_offset (tree_view->priv->tree, new_y, &tree, &node);
			    ...
			    GtkSnapshot *snapshot)
{
  ...
  parity = !(_gtk_tree_rbtree_node_get_index (tree, node) % 2);
	  ...
	  if (gtk_tree_view_is_expander_column (tree_view, column) &&
		...
		x += background_area.width - 1;
		  ...
		  GtkTreeRBTree *tmp_tree;
	          ...
	          if (!_gtk_tree_rbtree_next (tree, node))
                    gtk_tree_view_snapshot_line (tree_view, snapshot,
                                                 GTK_TREE_VIEW_TREE_LINE,
                                                 x + expander_size * (depth - 1.5) * mult,
                                                 y0,
		  ...
		  for (i = depth - 2; i > 0; i--)
		    {
	              if (_gtk_tree_rbtree_next (tmp_tree, tmp_node))
                        gtk_tree_view_snapshot_line (tree_view, snapshot,
                                                     GTK_TREE_VIEW_TREE_LINE,
		 ...
		 GtkTreeRBNode   *node)
{
  ...
  y = _gtk_tree_rbtree_node_find_offset (tree, node);
...
validate_visible_area (GtkTreeView *tree_view)
{
      ...
      if (path && !_gtk_tree_view_find_node (tree_view, path, &tree, &node))
	{
	  ...
	  if (tree_view->priv->scroll_to_use_align)
	  ...
	  else
	    {
	      ...
	      dy = _gtk_tree_rbtree_node_find_offset (tree, node);
  ...
  if (path == NULL)
    {
      ...
      offset = _gtk_tree_rbtree_find_offset (tree_view->priv->tree,
   ...
   * do this, _gtk_tree_rbtree_find_offset will find the row above top_row, because
  ...
  if (area_above == 0)
    {
      ...
      _gtk_tree_rbtree_prev_full (tmptree, tmpnode, &tmptree, &tmpnode);
  ...
  while (area_above > 0)
    {
      _gtk_tree_rbtree_prev_full (tree, node, &tree, &node);
...
initialize_fixed_height_mode (GtkTreeView *tree_view)
{
  ...
  if (tree_view->priv->fixed_height < 0)
   ...
   _gtk_tree_rbtree_set_fixed_height (tree_view->priv->tree,
...
do_validate_rows (GtkTreeView *tree_view, gboolean queue_resize)
{
	  ...
	  do
		  ...
		  GTK_TREE_RBNODE_FLAG_SET (node->left, GTK_TREE_RBNODE_DESCENDANTS_INVALID))
		{
		  node = node->left;
              ...
              else if (!_gtk_tree_rbtree_is_nil (node->right) &&
  ...
  if (!tree_view->priv->fixed_height_check)
   {
     if (fixed_height)
       _gtk_tree_rbtree_set_fixed_height (tree_view->priv->tree, prev_height, FALSE);
...
do_presize_handler (GtkTreeView *tree_view)
{
  if (tree_view->priv->mark_rows_col_dirty)
   {
      if (tree_view->priv->tree)
	_gtk_tree_rbtree_column_invalid (tree_view->priv->tree);
...
gtk_tree_view_top_row_to_dy (GtkTreeView *tree_view)
{
  ...
  new_dy = _gtk_tree_rbtree_node_find_offset (tree, node);
			     ...
			     GtkTreeRBNode   **new_node)
{
  ...
  while (node && row_is_separator (tree_view, NULL, *path))
    {
      if (search_forward)
	_gtk_tree_rbtree_next_full (tree, node, &tree, &node);
      else
	_gtk_tree_rbtree_prev_full (tree, node, &tree, &node);
...
gtk_tree_view_style_updated (GtkWidget *widget)
{
  ...
  if (change == NULL || gtk_css_style_change_affects (change, GTK_CSS_AFFECTS_SIZE))
      ...
      for (list = tree_view->priv->columns; list; list = list->next)
	{
      ...
      tree_view->priv->fixed_height = -1;
      _gtk_tree_rbtree_mark_invalid (tree_view->priv->tree);
    }
}
			   ...
			   gpointer      data)
{
  ...
  if (tree_view->priv->fixed_height_mode
    ...
    {
      _gtk_tree_rbtree_node_set_height (tree, node, tree_view->priv->fixed_height);
      gtk_widget_queue_draw (GTK_WIDGET (tree_view));
    }
  else
    {
			    ...
			    gpointer      data)
{
  ...
  if (tree_view->priv->tree == NULL)
    tree_view->priv->tree = _gtk_tree_rbtree_new ();
  ...
  while (i < depth - 1)
      ...
      if (tree == NULL)
	{
      ...
      tmpnode = _gtk_tree_rbtree_find_count (tree, indices[i] + 1);
      if (tmpnode == NULL)
	{
  ...
  if (indices[depth - 1] == 0)
    {
      tmpnode = _gtk_tree_rbtree_find_count (tree, 1);
      tmpnode = _gtk_tree_rbtree_insert_before (tree, tmpnode, height, FALSE);
    }
  else
    {
      tmpnode = _gtk_tree_rbtree_find_count (tree, indices[depth - 1]);
      tmpnode = _gtk_tree_rbtree_insert_after (tree, tmpnode, height, FALSE);
    }
  ...
  if (height > 0)
    {
      if (tree)
        _gtk_tree_rbtree_node_mark_valid (tree, tmpnode);
                        ...
                        gpointer   data)
{
  ...
  *value |= GTK_TREE_RBNODE_FLAG_SET (node, GTK_TREE_RBNODE_IS_SELECTED);
  ...
  if (node->children && !*value)
    _gtk_tree_rbtree_traverse (node->children, node->children->root, G_POST_ORDER, check_selection_helper, data);
}
			   ...
			   gpointer      data)
{
  ...
  _gtk_tree_rbtree_traverse (tree, node, G_POST_ORDER,
  ...
  if (tree_view->priv->cursor_node &&
      (tree_view->priv->cursor_node == node ||
       (node->children && (tree_view->priv->cursor_tree == node->children ||
                           _gtk_tree_rbtree_contains (node->children, tree_view->priv->cursor_tree)))))
    {
      GtkTreePath *cursor_path;

      cursor_tree = tree;
      cursor_node = _gtk_tree_rbtree_next (tree, node);
      /* find the first node that is not going to be deleted */
      while (cursor_node == NULL && cursor_tree->parent_tree)
        {
          cursor_node = _gtk_tree_rbtree_next (cursor_tree->parent_tree,
      ...
      if (cursor_path == NULL ||
                                         ...
                                         &cursor_tree, &cursor_node))
        {
           ...
           * focusable row.
           */
          _gtk_tree_rbtree_prev_full (tree, node, &cursor_tree, &cursor_node);
          if (cursor_node)
            {
              cursor_path = _gtk_tree_path_new_from_rbtree (cursor_tree, cursor_node);
			      ...
			      gpointer      data)
{
  ...
  _gtk_tree_rbtree_reorder (tree, new_order, len);
                                ...
                                gint        *x2)
{
  ...
  if (rtl)
    x_offset -= expander_size * _gtk_tree_rbtree_get_depth (tree);
  else
    x_offset += expander_size * _gtk_tree_rbtree_get_depth (tree);
			  ...
			  gboolean     recurse)
{
  ...
  do
    {
      gtk_tree_model_ref_node (tree_view->priv->model, iter);
      temp = _gtk_tree_rbtree_insert_after (tree, temp, 0, FALSE);

      if (tree_view->priv->fixed_height > 0)
        {
          if (GTK_TREE_RBNODE_FLAG_SET (temp, GTK_TREE_RBNODE_INVALID))
	    {
              _gtk_tree_rbtree_node_set_height (tree, temp, tree_view->priv->fixed_height);
	      _gtk_tree_rbtree_node_mark_valid (tree, temp);
	    }
        }
	  ...
	  if (gtk_tree_model_iter_children (tree_view->priv->model, &child, iter))
	    {
		  ...
		  && !expand)
	        {
	          temp->children = _gtk_tree_rbtree_new ();
				  ...
				  GtkTreeRBNode   *node)
{
  ...
  node_dy = _gtk_tree_rbtree_node_find_offset (tree, node);
  height = gtk_tree_view_get_row_height (tree_view, node);
  if (! GTK_TREE_RBNODE_FLAG_SET (node, GTK_TREE_RBNODE_INVALID)
			  ...
			  GtkTreeRBNode   **node)
{
  ...
  *tree = NULL;
  ...
  do
    {
      tmpnode = _gtk_tree_rbtree_find_count (tmptree, indices[i] + 1);
      ++i;
      if (tmpnode == NULL)
	{
	  *tree = NULL;
  ...
  do
      ...
      if (node->children)
	{
	  ...
	  new_tree = node->children;
          new_node = _gtk_tree_rbtree_first (new_tree);
				   ...
				   gint         count)
{
  ...
  if (selection_count == 0
    ...
    {
      if (count == -1)
	_gtk_tree_rbtree_prev_full (tree_view->priv->cursor_tree, tree_view->priv->cursor_node,
			       &new_cursor_tree, &new_cursor_node);
      else
	_gtk_tree_rbtree_next_full (tree_view->priv->cursor_tree, tree_view->priv->cursor_node,
			       &new_cursor_tree, &new_cursor_node);
    }
					...
					gint         count)
{
  ...
  y = _gtk_tree_rbtree_node_find_offset (tree_view->priv->cursor_tree, tree_view->priv->cursor_node);
  ...
  tree_view->priv->cursor_offset =
    _gtk_tree_rbtree_find_offset (tree_view->priv->tree, y,
			     &cursor_tree, &cursor_node);
  ...
  if (tree_view->priv->cursor_offset
    ...
    {
      _gtk_tree_rbtree_next_full (cursor_tree, cursor_node,
			     &cursor_tree, &cursor_node);
  ...
  /* update y */
  y = _gtk_tree_rbtree_node_find_offset (cursor_tree, cursor_node);
				     ...
				     gint         count)
{
  ...
  if (count == -1)
    {
      cursor_node = _gtk_tree_rbtree_first (cursor_tree);
  ...
  else
    {
      ...
      do
	{
	  while (cursor_node && !_gtk_tree_rbtree_is_nil (cursor_node->right))
				 ...
				 gboolean     animate)
{
  ...
  _gtk_tree_rbtree_remove (node->children);
                               ...
                               gint               *cell_y)
{
  ...
  y_offset = _gtk_tree_rbtree_find_offset (tree_view->priv->tree,
                                ...
                                GtkTreeRBNode   *node)
{
  ...
  offset = _gtk_tree_rbtree_node_find_offset (tree, node);
  ...
  if (end_path)
    {
      ...
      _gtk_tree_rbtree_find_offset (tree_view->priv->tree, y, &tree, &node);
      if (node)
