...
gtk_widget_class_init (GtkWidgetClass *klass)
{
  ...
  widget_props[PROP_EVENT_TARGET] =
      g_param_spec_boolean ("event-target",
                            P_("Event target"),
                            P_("Whether the widget can receive events"),
                            FALSE,
                            GTK_PARAM_READWRITE|G_PARAM_EXPLICIT_NOTIFY);
			 ...
			 GParamSpec      *pspec)
{
  ...
  switch (prop_id)
	...
	gtk_widget_grab_focus (widget);
      break;
    case PROP_EVENT_TARGET:
      gtk_widget_set_event_target (widget, g_value_get_boolean (value));
      break;
			 ...
			 GParamSpec      *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_IS_FOCUS:
      ...
      break;
    case PROP_EVENT_TARGET:
      g_value_set_boolean (value, gtk_widget_get_event_target (widget));
      break;
    case PROP_FOCUS_ON_CLICK:
                 ...
                 GtkPickFlags  flags)
{
  ...
  if ((flags & GTK_PICK_EVENT_TARGET) &&
      !gtk_widget_get_event_target (widget))
    return NULL;

  if ((flags & GTK_PICK_SENSITIVE) &&
...
/**
 * gtk_widget_set_event_target:
 * @widget: a #GtkWidget
 * @event_target: whether this widget should be able to receive pointer events
 *
 * Sets whether @widget can be the target of pointer events.
 */
void
gtk_widget_set_event_target (GtkWidget *widget,
                             gboolean   event_target)
{
  GtkWidgetPrivate *priv = gtk_widget_get_instance_private (widget);

  event_target = !!event_target;

  if (priv->event_target == event_target)
    return;

  priv->event_target = event_target;

  g_object_notify_by_pspec (G_OBJECT (widget), widget_props[PROP_EVENT_TARGET]);
}

/**
 * gtk_widget_get_event_target:
 * @widget: a #GtkWidget
 * 
 ...
 */
gboolean
gtk_widget_get_event_target (GtkWidget *widget)
{
  GtkWidgetPrivate *priv = gtk_widget_get_instance_private (widget);

  return priv->event_target;
}
