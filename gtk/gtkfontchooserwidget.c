...
get_win32_all_locales_scripts (LPWSTR locale_w, DWORD flags, LPARAM param)
{
  ...
  wchar_t *langname_abbrev_w = NULL;
  gchar *langname, *langname_abbrev, *locale, *p;
  gint i;
  hb_language_t lang;
  GHashTable *ht_scripts_langs = (GHashTable *)param;

  gint langname_size, langname_abbrev_size;
  ...
  langname_abbrev_size = GetLocaleInfoEx (locale_w, LOCALE_SABBREVLANGNAME, langname_abbrev_w, 0);
  if (langname_abbrev_size > 0)
    {
      langname_abbrev_w = g_new0 (wchar_t, langname_abbrev_size);
      GetLocaleInfoEx (locale_w, LOCALE_SABBREVLANGNAME, langname_abbrev_w, langname_abbrev_size);
      langname_abbrev = g_utf16_to_utf8 (langname_abbrev_w, -1, NULL, NULL, NULL);
      lang = hb_language_from_string (langname_abbrev, -1);
      if (g_hash_table_lookup (ht_scripts_langs, lang) == NULL)
        g_hash_table_insert (ht_scripts_langs, lang, langname);
      g_free (langname_abbrev);
      g_free (langname_abbrev_w);
    }
...
_GDK_EXTERN FT_Face
get_ft_face_from_pango_font (PangoFont *pango_font)
{
  FT_Face ft_face;
...
gtk_font_chooser_widget_update_font_variations (GtkFontChooserWidget *fontchooser)
{
  ...
  ft_face = get_ft_face_from_pango_font (pango_font);
  ...
  release_pango_font_ft_face (pango_font);
