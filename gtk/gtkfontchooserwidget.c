...
gtk_font_chooser_widget_update_marks (GtkFontChooserWidget *fontchooser)
{
  ...
  face = gtk_single_selection_get_selected_item (priv->selection);
  if (face)
    {
                      ...
                      GtkFontChooserWidget *fontchooser)
{
  ...
  face = gtk_single_selection_get_selected_item (selection);
  if (face)
    {
...
static char *
gtk_font_chooser_widget_get_font_name (GtkFontChooserWidget *self,
                                       PangoFontFace        *face)
{
  ...
  if ((priv->level & GTK_FONT_CHOOSER_LEVEL_STYLE) != 0)
    {
      return g_strconcat (pango_font_face_get_face_name (face),
                          " ",
    ...
    {
      return g_strdup (pango_font_family_get_name (pango_font_face_get_family (face)));
    }
}

static PangoAttrList *
get_face_attributes (PangoFontFace *face)
{
...
static PangoAttrList *
gtk_font_chooser_widget_get_face_attributes (GtkFontChooserWidget *fontchooser,
                                             PangoFontFace        *face)
{
  return get_face_attributes (face);
}
...
gtk_font_chooser_widget_class_init (GtkFontChooserWidgetClass *klass)
{
  ...
  gtk_widget_class_bind_template_callback (widget_class, gtk_font_chooser_widget_get_font_name);
  gtk_widget_class_bind_template_callback (widget_class, gtk_font_chooser_widget_get_face_attributes);
  gtk_widget_class_bind_template_callback (widget_class, get_face_attributes);
...
gtk_font_chooser_widget_ensure_matching_selection (GtkFontChooserWidget *fontchooser)
{
  ...
  for (i = 0; i < n; i++)
    {
      PangoFontFace *face = g_list_model_get_item (G_LIST_MODEL (priv->selection), i);
...
gtk_font_chooser_widget_get_face (GtkFontChooser *chooser)
{
  ...
  return gtk_single_selection_get_selected_item (priv->selection);
}
...
gtk_font_chooser_widget_get_family (GtkFontChooser *chooser)
{
  ...
  face = gtk_font_chooser_widget_get_face (chooser);
  if (face == NULL)
    return NULL;

  return pango_font_face_get_family (face);
}
