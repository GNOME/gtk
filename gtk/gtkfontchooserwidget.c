...
get_dwrite_factory (void)
{
  ...
  static volatile gsize dwrite_inited = 0;
  ...
  if (g_once_init_enter (&dwrite_inited))
    {
      ...
      g_once_init_leave (&dwrite_inited, 1);
    }
...
get_dwrite_gdi_interop (IDWriteFactory *dwrite_factory)
{
  ...
  static volatile gsize dwrite_gdi_inited = 0;
  ...
  if (g_once_init_enter (&dwrite_gdi_inited))
    {
      ...
      g_once_init_leave (&dwrite_gdi_inited, 1);
    }

  return gdi;
}

_GDK_EXTERN FT_Library
...
get_win32_langtag_locales (void)
{
...
_GDK_EXTERN FT_Face
get_ft_face_from_pango_font (PangoFont *pango_font, FT_Byte *font_file_data)
{
  FT_Face ft_face;
  ...
  if (PANGO_WIN32_IS_FONT (pango_font))
    {
      ...
      hr = IDWriteFontFile_GetReferenceKey (dwrite_font_file,
                                            &dwrite_ref_key,
                                            &dwrite_ref_key_size);
      ...
      hr = IDWriteFontFile_GetLoader (dwrite_font_file, &dwrite_font_file_loader);
      ...
      hr = IDWriteFontFileLoader_CreateStreamFromKey (dwrite_font_file_loader,
                                                      dwrite_ref_key,
                                                      dwrite_ref_key_size,
      ...
      if (FAILED (hr))
        {
          g_warning ("Font Face File loader stream creation failed");
          return NULL;
        }

      hr = IDWriteFontFileStream_GetFileSize (dwrite_font_file_stream, &font_file_size);

      if (FAILED (hr))
        {
          g_warning ("Could not acquire font file size");
          return NULL;
        }

      hr = IDWriteFontFileStream_ReadFileFragment (dwrite_font_file_stream,
                                                   &font_fragment_start,
                                                   0,
                                                   font_file_size,
                                                   &font_fragment_ctx);
      ...
      if (FAILED (hr))
        {
          g_warning ("Could not acquire raw font data for font");
          return NULL;
        }
...
/*
      ...
      if (ft_err != FT_Err_Ok)
        {
          g_warning ("Unable to obtain FreeType Face from font");
          return NULL;
        }
#if 0
...
_GDK_EXTERN void
release_pango_font_ft_face (PangoFont *pango_font,
                            FT_Face    ft_face,
...
#if defined (HAVE_HARFBUZZ) && defined (GDK_WINDOWING_WIN32)
  IDWriteFactory *dwrite_factory = NULL;
#endif
...
gtk_font_chooser_widget_update_font_variations (GtkFontChooserWidget *fontchooser)
{
  ...
  ft_face = get_ft_face_from_pango_font (pango_font, font_file_data);
  ...
  release_pango_font_ft_face (pango_font, ft_face, font_file_data);
