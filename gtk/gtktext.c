  ...
  if (G_VALUE_HOLDS (value, G_TYPE_STRING))
    {
      ...
      if (gtk_text_get_selection_bounds (GTK_EDITABLE (content->self), &start, &end))
        {
                          ...
                          GdkClipboard       *clipboard)
{
  ...
  gtk_text_get_selection_bounds (GTK_EDITABLE (content->self), &current_pos, &selection_bound);
  gtk_text_set_selection_bounds (GTK_EDITABLE (content->self), current_pos, current_pos);
}
...
gtk_text_class_init (GtkTextClass *class)
{
  ...
  quark_password_hint = g_quark_from_static_string ("gtk-self-password-hint");
                       ...
                       GParamSpec *pspec)
{
  ...
  switch (prop_id)
    ...
    /* GtkText properties */
    case PROP_BUFFER:
      g_value_set_object (value, gtk_text_get_buffer (self));
      break;
                           ...
                           int      *n_ranges)
{
  ...
  if (gtk_editable_get_selection_bounds (GTK_EDITABLE (self), &start_char, &end_char))
    {
  ...
  if (in_drag)
    {
      ...
      gtk_editable_set_position (GTK_EDITABLE (self), tmp_pos);
    }
                              ...
                              GtkStateFlags  previous_state)
{
  ...
  if (!gtk_widget_is_sensitive (widget))
    {
      /* Clear any selection */
      gtk_editable_select_region (GTK_EDITABLE (self), priv->current_pos, priv->current_pos);
    }
                            ...
                            int          position)
{
  ...
  if (position != priv->current_pos ||
    ...
    {
      gtk_text_reset_im_context (self);
      gtk_text_set_positions (self, position, position);
    }
}
  ...
  if (n_chars == 1 && !priv->visible)
    {
      g_object_get (gtk_widget_get_settings (GTK_WIDGET (self)),
                    "gtk-self-password-hint-timeout", &password_hint_timeout,
                    NULL);
                      ...
                      gboolean         extend_selection)
{
  ...
  if (extend_selection)
    gtk_editable_select_region (GTK_EDITABLE (self), priv->selection_bound, new_pos);
  else
    gtk_editable_set_position (GTK_EDITABLE (self), new_pos);
  ...
  if (gtk_editable_get_selection_bounds (editable, &start, &end))
    {
      ...
      str = gtk_text_get_display_text (self, start, end);
                    ...
                    GtkSnapshot *snapshot)
{
  ...
  if (gtk_editable_get_selection_bounds (GTK_EDITABLE (self), &start_pos, &end_pos))
    {
      const char *text = pango_layout_get_text (layout);
...
gtk_text_select_word (GtkText *self)
{
  ...
  gtk_editable_select_region (GTK_EDITABLE (self), start_pos, end_pos);
}
...
gtk_text_selection_bubble_popup_show (gpointer user_data)
{
  ...
  has_selection = gtk_editable_get_selection_bounds (GTK_EDITABLE (self), &start, &end);
  length = gtk_entry_buffer_get_length (get_buffer (self));
                      ...
                      int        y)
{
  ...
  if (priv->editable &&
      gtk_drag_dest_find_target (widget, drop, NULL) != NULL)
    {
      ...
      if (!gtk_editable_get_selection_bounds (GTK_EDITABLE (self), &sel1, &sel2) ||
