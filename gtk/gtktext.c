 ...
 * @Title: GtkText
 * @See_also: #GtkTextView
 *
 * The #GtkText widget is a single line text entry
 ...
 * gtk_text_set_invisible_char(). GTK+ displays a warning
 * when Caps Lock or input methods might interfere with entering text in
 * a password entry. The warning can be turned off with the
 * #GtkText:caps-lock-warning property.
 *
 * GtkText has the ability to display progress or activity
 * information behind the text. To make an entry display such information,
 * use gtk_text_set_progress_fraction() or gtk_text_set_progress_pulse_step().
 *
 * Additionally, GtkText can show icons at either side of the entry. These
 * icons can be activatable by clicking, can be set up as drag source and
 * can have tooltips. To add an icon, use gtk_text_set_icon_from_gicon() or
 ...
 * connect to the #GtkText::icon-press signal. To allow DND operations
 * from an icon, use gtk_text_set_icon_drag_source(). To set a tooltip on
 * an icon, use gtk_text_set_icon_tooltip_text() or the corresponding function
 * for markup.
 *
...
static void     gtk_text_delete_text          (GtkEditable *editable,
						gint         start_pos,
						gint         end_pos);
static const char *gtk_text_real_get_text     (GtkEditable *editable);
static int      gtk_text_get_length           (GtkEditable *editable);
static void     gtk_text_real_set_position    (GtkEditable *editable,
						gint         position);
static gint     gtk_text_get_position         (GtkEditable *editable);
...
static gboolean gtk_text_get_selection_bounds (GtkEditable *editable,
						gint        *start,
						gint        *end);
static void     gtk_text_set_editable         (GtkEditable *editable,
...
static void keymap_state_changed         (GdkKeymap       *keymap,
					  GtkText        *entry);
static void remove_capslock_feedback     (GtkText        *entry);
...
static gint         gtk_text_find_position            (GtkText       *entry,
							gint            x);
static void         gtk_text_get_cursor_locations     (GtkText       *entry,
							gint           *strong_x,
							gint           *weak_x);
static void         gtk_text_adjust_scroll            (GtkText       *entry);
static gint         gtk_text_move_visually            (GtkText       *editable,
							gint            start,
							gint            count);
static gint         gtk_text_move_logically           (GtkText       *entry,
							gint            start,
							gint            count);
static gint         gtk_text_move_forward_word        (GtkText       *entry,
							gint            start,
                                                        gboolean        allow_whitespace);
static gint         gtk_text_move_backward_word       (GtkText       *entry,
							gint            start,
                                                        gboolean        allow_whitespace);
  ...
  if (G_VALUE_HOLDS (value, G_TYPE_STRING))
    {
      ...
      if (gtk_editable_get_selection_bounds (GTK_EDITABLE (content->entry), &start, &end))
        {
          gchar *str = _gtk_text_get_display_text (content->entry, start, end);
...
static void
gtk_text_set_editable (GtkEditable *editable,
                        gboolean     is_editable)
{
...
gtk_text_editable_init (GtkEditableInterface *iface)
{
  ...
  iface->get_text = gtk_text_real_get_text;
  iface->get_length = gtk_text_get_length;
                        ...
                        GParamSpec      *pspec)
{
  ...
  switch (prop_id)
    ...
    case GTK_EDITABLE_PROP_EDITABLE:
      gtk_text_set_editable (GTK_EDITABLE (entry), g_value_get_boolean (value));
      break;
                        ...
                        GParamSpec      *pspec)
{
  ...
  switch (prop_id)
    ...
    case GTK_EDITABLE_PROP_TEXT:
      g_value_set_string (value, gtk_text_get_text (entry));
      break;

    case GTK_EDITABLE_PROP_XALIGN:
      g_value_set_float (value, gtk_text_get_alignment (entry));
      break;
...
gchar*
_gtk_text_get_display_text (GtkText *entry,
                             gint      start_pos,
                             gint      end_pos)
{
                            ...
                            GtkText       *entry)
{
  ...
  if (in_drag)
    {
      gint tmp_pos = gtk_text_find_position (entry, priv->drag_start_x);
...
static const char *
gtk_text_real_get_text (GtkEditable *editable)
{
  return gtk_entry_buffer_get_text (gtk_text_get_buffer (GTK_TEXT (editable)));
}

static int
gtk_text_get_length (GtkEditable *editable)
{
  return gtk_entry_buffer_get_length (gtk_text_get_buffer (GTK_TEXT (editable)));
}
			    ...
			    gint        *position)
{
  ...
  n_inserted = gtk_entry_buffer_insert_text (get_buffer (GTK_TEXT (editable)), *position, new_text, n_chars);
...
gtk_text_backspace (GtkText *entry)
{
  ...
  if (prev_pos < priv->current_pos)
      ...
      if (log_attrs[priv->current_pos].backspace_deletes_character)
	{
	  ...
	  cluster_text = _gtk_text_get_display_text (entry, prev_pos,
                                                      priv->current_pos);
  ...
  if (gtk_editable_get_selection_bounds (editable, &start, &end))
    {
      ...
      str = _gtk_text_get_display_text (entry, start, end);
			 ...
			 gboolean  include_preedit)
{
  ...
  display_text = _gtk_text_get_display_text (entry, 0, -1);
...
/**
 * gtk_text_im_context_filter_keypress:
 * @entry: a #GtkText
 * @event: (type Gdk.EventKey): the key event
 *
 * Allow the #GtkText input method to internally handle key press
 ...
 * and the default key event handling of the #GtkText.
 * See gtk_text_view_reset_im_context() for an example of use.
 *
 * Returns: %TRUE if the input method handled the key event.
 */
gboolean
gtk_text_im_context_filter_keypress (GtkText    *entry,
                                      GdkEventKey *event)
{
  GtkTextPrivate *priv = gtk_text_get_instance_private (entry);
...
GtkIMContext*
_gtk_text_get_im_context (GtkText *entry)
{
  GtkTextPrivate *priv = gtk_text_get_instance_private (entry);
...
{
  GtkTextPrivate *priv = gtk_text_get_instance_private (entry);
  gint start_pos = gtk_text_move_backward_word (entry, priv->current_pos, TRUE);
  gint end_pos = gtk_text_move_forward_word (entry, priv->current_pos, TRUE);
 ...
 * for entries with visibility set to false. See gtk_text_set_invisible_char().
 *
 ...
 **/
const gchar*
gtk_text_get_text (GtkText *entry)
{
  g_return_val_if_fail (GTK_IS_TEXT (entry), NULL);
 ...
 * the default handler for the #GtkText::activate signal.)
 **/
void
...
/**
 * gtk_text_get_width_chars:
 * @entry: a #GtkText
 * 
 * Gets the value set by gtk_text_set_width_chars().
 * 
 * Returns: number of chars to request space for, or negative if unset
 **/
gint
gtk_text_get_width_chars (GtkText *entry)
{
  GtkTextPrivate *priv = gtk_text_get_instance_private (entry);
...
/**
 * gtk_text_get_max_width_chars:
 * @entry: a #GtkText
 *
 * Retrieves the desired maximum width of @entry, in characters.
 * See gtk_text_set_max_width_chars().
 *
 * Returns: the maximum width of the entry, in characters
 */
gint
gtk_text_get_max_width_chars (GtkText *entry)
{
  GtkTextPrivate *priv = gtk_text_get_instance_private (entry);
                         ...
                         gboolean  setting)
{
  ...
  if (setting == gtk_text_get_has_frame (entry))
    return;
 ...
 * pixel positions, in combination with gtk_text_get_layout_offsets().
 * The returned layout is owned by the entry and must not be 
 * modified or freed by the caller.
 *
 * Keep in mind that the layout text may contain a preedit string, so
 * gtk_text_layout_index_to_text_index() and
 * gtk_text_text_index_to_layout_index() are needed to convert byte
...
/**
 * gtk_text_layout_index_to_text_index:
 * @entry: a #GtkText
 * @layout_index: byte index into the entry layout text
 *
 * Converts from a position in the entry’s #PangoLayout (returned by
 * gtk_text_get_layout()) to a position in the entry contents
 * (returned by gtk_text_get_text()).
 *
 * Returns: byte index into the entry contents
 **/
gint
gtk_text_layout_index_to_text_index (GtkText *entry,
                                      gint      layout_index)
{
  GtkTextPrivate *priv = gtk_text_get_instance_private (entry);
  PangoLayout *layout;
...
/**
 * gtk_text_text_index_to_layout_index:
 * @entry: a #GtkText
 * @text_index: byte index into the entry contents
 *
 * Converts from a position in the entry contents (returned
 * by gtk_text_get_text()) to a position in the
 * entry’s #PangoLayout (returned by gtk_text_get_layout(),
 * with text retrieved via pango_layout_get_text()).
 *
 * Returns: byte index into the entry layout text
 **/
gint
gtk_text_text_index_to_layout_index (GtkText *entry,
                                      gint      text_index)
{
  GtkTextPrivate *priv = gtk_text_get_instance_private (entry);
  PangoLayout *layout;
 ...
 **/
void
gtk_text_set_alignment (GtkText *entry, gfloat xalign)
{
  GtkTextPrivate *priv = gtk_text_get_instance_private (entry);
...
/**
 * gtk_text_get_alignment:
 * @entry: a #GtkText
 *
 * Gets the value set by gtk_text_set_alignment().
 *
 * Returns: the alignment
 **/
gfloat
gtk_text_get_alignment (GtkText *entry)
{
  GtkTextPrivate *priv = gtk_text_get_instance_private (entry);

  g_return_val_if_fail (GTK_IS_TEXT (entry), 0.0);
			 ...
			 GtkSelectionData *selection_data)
{
  ...
  if (gtk_editable_get_selection_bounds (editable, &sel_start, &sel_end))
    {
      gchar *str = _gtk_text_get_display_text (GTK_TEXT (widget), sel_start, sel_end);
...
static void
show_capslock_feedback (GtkText    *entry,
                        const gchar *text)
{
}

static void
remove_capslock_feedback (GtkText *entry)
{
}
                      ...
                      GtkText  *entry)
{
  ...
  if (priv->editable &&
      gtk_text_get_display_mode (entry) != DISPLAY_NORMAL &&
