		       ...
		       gboolean          need_reparent)
{
  ...
  if (user_data)		/* A widget's window in this process */
      ...
      else
	{
      ...
      if (gdk_display_drag_get_protocol (gtk_widget_get_display (widget), 
					 xid, &protocol))
		     ...
		     guint32    time)
{
  ...
  if (socket->plug_window)
	...
	(gtk_widget_get_display (GTK_WIDGET (socket)), "_XEMBED");
  ...
  switch (xevent->type)
	  ...
	  xevent->xproperty.window == GDK_WINDOW_XWINDOW (socket->plug_window))
	{
	  ...
	  if ((xevent->xproperty.atom == 
	    ...
	    {
	      gdk_error_trap_push ();
	      if (gdk_display_drag_get_protocol (display, 
