...
static PangoAttrList *gtk_entry_get_attr_list        (GtkEntry       *entry);
static void           gtk_entry_reset_attrs          (GtkEntry       *entry);
  ...
  if (!entry->layout)
    {
      ...
      attrs = gtk_entry_get_attr_list (entry);
      pango_layout_set_text (entry->layout, entry->text, entry->n_bytes);
      pango_layout_set_attributes (entry->layout, attrs);
...
static void
gtk_entry_reset_attrs (GtkEntry *entry)
{
  if (entry->layout)
    {
      PangoAttrList *attrs = gtk_entry_get_attr_list (entry);
      pango_layout_set_text (entry->layout, entry->text, entry->n_bytes);
      pango_layout_set_attributes (entry->layout, attrs);
      pango_attr_list_unref (attrs);
    }
