...
gtk_drop_down_dispose (GObject *object)
{
  ...
  g_clear_object (&self->selection_monitor);
...
gtk_drop_down_class_init (GtkDropDownClass *klass)
{
  ...
  gtk_widget_class_bind_template_child (widget_class, GtkDropDown, button_list);
  ...
  if (model == NULL)
    {
      ...
      gtk_list_view_set_model (GTK_LIST_VIEW (self->button_list), NULL);
  ...
  else
    {
      ...
      g_object_bind_property (self->selection, "selected",
                              self->popup_selection, "selected",
                              G_BINDING_SYNC_CREATE);

      selection = G_LIST_MODEL (gtk_selection_monitor_new (GTK_SELECTION_MODEL (self->selection)));
      g_set_object (&self->selection_monitor, selection);
      gtk_list_view_set_model (GTK_LIST_VIEW (self->button_list), selection);
      g_object_unref (selection);
    }
                           ...
                           GtkListItemFactory *factory)
{
  ...
  gtk_list_view_set_factory (GTK_LIST_VIEW (self->button_list), factory);
