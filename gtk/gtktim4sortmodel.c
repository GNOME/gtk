...
sort_func (gconstpointer a,
           ...
           gpointer      data)
{
  GtkSorterKeys *sorter_keys = data;
  gpointer *sa = (gpointer *) a;
  gpointer *sb = (gpointer *) b;

  return sorter_keys->key_compare (*sa, *sb, sorter_keys);
}
                                 ...
                                 guint            *n_items)
{
  ...
  for (i = 0; i < self->n_items; i++)
    {
      gtk_sorter_keys_clear_key (self->sorter_keys, key_from_pos (self, i));
    }
  g_free (self->keys);
  self->sorter_keys->free (self->sorter_keys);
  self->sorter_keys = NULL;
  self->key_size = 0;
} 
...
gtk_tim4_sort_model_create_items (GtkTim4SortModel *self)
{
  ...
  self->sorter_keys = gtk_sorter_get_keys (self->sorter);
  self->key_size = self->sorter_keys->key_size;
  self->keys = g_malloc_n (self->n_items, self->key_size);
  for (i = 0; i < self->n_items; i++)
    {
      gpointer item = g_list_model_get_item (self->model, i);
      gtk_sorter_keys_init_key (self->sorter_keys, item, key_from_pos (self, i));
      g_object_unref (item);
  ...
  if (removed)
    {
      ...
      if (removed > added)
        {
          memmove (key_from_pos (self->keys, position + removed),
                   key_from_pos (self->keys, position + added),
  ...
  for (i = 0; i < added; i++)
    {
      ...
      gtk_sorter_keys_init_key (self->sorter_keys, item, key_from_pos (self, position + i));
      g_object_unref (item);
