 ...
 * @string: the string to insert
 *
 * Inserts @string into @self at @position, and takes
 * ownership of it.
 *
 * This variant of gtk_string_list_insert() is
 * convenient for formatting strings:
 *
 * |[
 * gtk_string_list_take (self, 0, g_strdup_print ("%d dollars", lots));
 * ]|
 */
void
                      ...
                      char          *string)
{
  ...
  iter = g_sequence_get_iter_at_pos (self->items, position);
  g_sequence_insert_before (iter, gtk_string_object_new_take (string));
