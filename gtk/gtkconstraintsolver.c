  ...
  while (gtk_constraint_expression_iter_next (&iter, &t_v, &t_c))
    {
      ...
      if (gtk_constraint_variable_is_external (t_v))
        g_hash_table_add (self->external_parametric_vars,
                          gtk_constraint_variable_ref (t_v));
    }

  if (gtk_constraint_variable_is_external (variable))
    g_hash_table_add (self->external_rows, gtk_constraint_variable_ref (variable));
}
                                      ...
                                      GtkConstraintExpression *expression)
{
  ...
  if (gtk_constraint_variable_is_external (old_variable))
    {
      g_hash_table_add (self->external_rows, gtk_constraint_variable_ref (old_variable));
                             ...
                             GtkConstraintVariable *exit_var)
{
  ...
  g_return_if_fail (entry_var != NULL);
  g_return_if_fail (exit_var != NULL);
  ...
  while (TRUE)
    {
      ...
      if (fabs (objective_coefficient) >= -DBL_EPSILON)
        break;
                                               ...
                                               GtkConstraintRef *constraint)
{
...
#ifdef G_ENABLE_DEBUG
  {
    ...
    GTK_NOTE (LAYOUT, g_print ("Adding constraint '%s' (normalized expression: '%s')",
                               ref_s,
                               expr_s));
                                      ...
                                      double strength)
{
  ...
  if (expression == NULL)
    ...
    {
      res->expression = gtk_constraint_expression_ref (expression);
