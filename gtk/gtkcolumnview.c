                          ...
                          int        baseline)
{
  ...
  full_width = gtk_column_view_allocate_columns (self, x, width);
  ...
  for (i = 0; !self->in_column_resize && i < g_list_model_get_n_items (G_LIST_MODEL (self->columns)); i++)
    {
      ...
      if (gtk_column_view_column_in_header (column, start_x, start_y))
        {
          int pos;
...
static void
drag_update_drag_column (GtkColumnView *self,
                         double         x,
                         double         y)
{
  GtkColumnViewColumn *column;
                    ...
                    GtkColumnView  *self)
{
  ...
  if (self->in_column_resize)
    ...
    drag_update_drag_column (self, x, y);
}
