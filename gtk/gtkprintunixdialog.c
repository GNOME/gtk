...
gtk_print_unix_dialog_init (GtkPrintUnixDialog *dialog)
{
  ...
  gtk_column_view_set_model (GTK_COLUMN_VIEW (priv->printer_list), selection);
  ...
  if (priv->request_details_tag)
    {
      ...
      if (details_failed)
        gtk_printer_set_state_message (priv->request_details_printer, _("Getting printer information failed"));
      g_clear_object (&priv->request_details_printer);
    }
}
                   ...
                   GtkPrintUnixDialog *dialog)
{
  ...
  model = gtk_column_view_get_model (GTK_COLUMN_VIEW (priv->printer_list));
  ...
  if (printer)
    {
      ...
      g_signal_handler_block (priv->options, priv->options_changed_handler);

      gtk_printer_option_set_clear_conflicts (priv->options);
      have_conflict = _gtk_printer_mark_conflicts (printer, priv->options);

      g_signal_handler_unblock (priv->options, priv->options_changed_handler);
    }
...
{
  GtkPrintUnixDialogPrivate *priv = gtk_print_unix_dialog_get_instance_private (dialog);
  GListModel *model = gtk_column_view_get_model (GTK_COLUMN_VIEW (priv->printer_list));
  GtkPrinter *printer;
  ...
  if (printer != NULL && !gtk_printer_has_details (printer))
    {
      gtk_dialog_set_response_sensitive (GTK_DIALOG (dialog), GTK_RESPONSE_OK, FALSE);
      priv->request_details_tag = g_signal_connect (printer, "details-acquired",
  ...
  if (printer != NULL && gtk_printer_is_accepting_jobs (printer))
    gtk_dialog_set_response_sensitive (GTK_DIALOG (dialog), GTK_RESPONSE_OK, TRUE);
  priv->current_printer = g_object_ref (printer);
