...
struct _GtkFileChooserWidgetPrivate {
  ...
  GtkWidget *browse_files_context_menu;
 ...
 */
static void
check_file_list_context_menu_sensitivity (GtkFileChooserWidget *impl)
{
  GtkFileChooserWidgetPrivate *priv = impl->priv;
  gint num_selected;
  gboolean all_files;
  ...
  GAction *action, *action_delete, *action_trash;
  ...
  action_delete = g_action_map_lookup_action (G_ACTION_MAP (actions), "delete");
  action_trash = g_action_map_lookup_action (G_ACTION_MAP (actions), "trash");
  ...
  if (num_selected == 1)
    {
      ...
      if (g_file_info_get_attribute_boolean (info, G_FILE_ATTRIBUTE_ACCESS_CAN_TRASH))
        {
          g_simple_action_set_enabled (G_SIMPLE_ACTION (action_delete), FALSE);
          g_simple_action_set_enabled (G_SIMPLE_ACTION (action_trash), TRUE);
        }
      else if (g_file_info_get_attribute_boolean (info, G_FILE_ATTRIBUTE_ACCESS_CAN_DELETE))
        {
          g_simple_action_set_enabled (G_SIMPLE_ACTION (action_delete), TRUE);
          g_simple_action_set_enabled (G_SIMPLE_ACTION (action_trash), FALSE);
        }
      else
        {
          g_simple_action_set_enabled (G_SIMPLE_ACTION (action_delete), FALSE);
...
/* Updates context menu items for the file list, creating it if necessary */
static void
file_list_update_context_menu (GtkFileChooserWidget *impl)
{
  GtkFileChooserWidgetPrivate *priv = impl->priv;
  GActionGroup *actions;
  GAction *action;

  file_list_build_context_menu (impl);
  check_file_list_context_menu_sensitivity (impl);
  ...
  if (priv->action == GTK_FILE_CHOOSER_ACTION_OPEN ||
      ...
      priv->operation_mode != OPERATION_MODE_BROWSE)
    {
      ...
      action = g_action_map_lookup_action (G_ACTION_MAP (actions), "trash");
      g_simple_action_set_enabled (G_SIMPLE_ACTION (action), FALSE);
    }
...
static void
file_list_show_context_menu (GtkFileChooserWidget *impl,
                             gdouble               x,
                             gdouble               y)
{
  GtkFileChooserWidgetPrivate *priv = impl->priv;
  ...
  file_list_update_context_menu (impl);

  if (GTK_IS_MENU (priv->browse_files_context_menu))
    {
      gtk_menu_popup_at_pointer (GTK_MENU (priv->browse_files_context_menu), NULL);
      return;
    }
  ...
  gtk_popover_set_pointing_to (GTK_POPOVER (priv->browse_files_context_menu), &rect);
  gtk_popover_popup (GTK_POPOVER (priv->browse_files_context_menu));
}
                    ...
                    GtkFileChooserWidget *impl)
{
  ...
  file_list_show_context_menu (impl,
                            ...
                            GtkFileChooserWidget *impl)
{
  ...
  file_list_show_context_menu (impl, event->x, event->y);
...
long_press_cb (GtkGesture           *gesture,
               ...
               GtkFileChooserWidget *impl)
{
  file_list_show_context_menu (impl, x, y);
}
...
gtk_file_chooser_widget_dispose (GObject *object)
{
  ...
  if (priv->browse_files_context_menu)
    {
      gtk_widget_destroy (priv->browse_files_context_menu);
      priv->browse_files_context_menu = NULL;
    }
