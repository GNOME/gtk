                               ...
                               int              *natural_baseline)
{
       ...
       child = _gtk_widget_get_next_sibling (child))
    {
      ...
      if (child_info->width_constraint[0] != NULL)
        {
          ...
          child_info->width_constraint[0] = NULL;
          child_info->width_constraint[1] = NULL;
        }
      ...
      if (child_info->height_constraint[0] != NULL)
        {
          ...
          child_info->height_constraint[0] = NULL;
          child_info->height_constraint[1] = NULL;
        }
      ...
      switch (orientation)
        ...
        case GTK_ORIENTATION_HORIZONTAL:
          attr = get_child_attribute (child_info, solver, child, GTK_CONSTRAINT_ATTRIBUTE_WIDTH);

          /* width ≥ child_min (required) */
          e = gtk_constraint_expression_new (child_min);
          child_info->width_constraint[0] =
            gtk_constraint_solver_add_constraint (solver,
                                                  attr, GTK_CONSTRAINT_RELATION_GE, e,
                                                  GTK_CONSTRAINT_WEIGHT_REQUIRED);
          gtk_constraint_expression_unref (e);

          /* width = child_nat (strong) */
          e = gtk_constraint_expression_new (child_nat);
          child_info->width_constraint[1] =
            gtk_constraint_solver_add_constraint (solver,
                                                  attr, GTK_CONSTRAINT_RELATION_EQ, e,
                                                  GTK_CONSTRAINT_WEIGHT_MEDIUM);
          gtk_constraint_expression_unref (e);
          break;

        case GTK_ORIENTATION_VERTICAL:
          attr = get_child_attribute (child_info, solver, child, GTK_CONSTRAINT_ATTRIBUTE_HEIGHT);

          /* height ≥ child_min (required) */
          e = gtk_constraint_expression_new (child_min);
  ...
  gtk_constraint_solver_remove_constraint (solver, edit_s);
  gtk_constraint_solver_remove_constraint (solver, edit_o);
                                ...
                                int               baseline)
{
  ...
  {
    char *s = gtk_constraint_solver_to_string (solver);

    g_debug ("*** Solver state ***\n%s\n", s);
  ...
  for (child = _gtk_widget_get_first_child (widget);
       ...
       child = _gtk_widget_get_next_sibling (child))
    {
      ...
      _gtk_widget_get_preferred_size_and_baseline (child, &child_req, NULL, &child_baseline, NULL);
      ...
      child_alloc.width = ceil (gtk_constraint_variable_get_value (var_width)) > child_req.width
                        ? ceil (gtk_constraint_variable_get_value (var_width))
                        : child_req.width;
      child_alloc.height = ceil (gtk_constraint_variable_get_value (var_height)) > child_req.height
                         ? ceil (gtk_constraint_variable_get_value (var_height))
                         : child_req.height;
