                              ...
                              gpointer             unused)
{
  ...
  gtk_css_gadget_get_preferred_size (priv->on_gadget,
                                     ...
                                     &height, &dummy,
                                     NULL, NULL);
  child_alloc.x = allocation->x + ((allocation->width/2 - width) / 2);
  child_alloc.y = allocation->y + ((allocation->height - height) / 2);
  ...
  gtk_css_gadget_get_preferred_size (priv->off_gadget,
                                     ...
                                     &height, &dummy,
                                     NULL, NULL);
  child_alloc.x = allocation->x + allocation->width/2 + ((allocation->width/2 - width) / 2);
  child_alloc.y = allocation->y + ((allocation->height - height) / 2);
  child_alloc.width = width;
  child_alloc.height = height;
...
gtk_switch_init (GtkSwitch *self)
{
  ...
  priv->on_gadget = gtk_builtin_icon_new ("on-symbolic",
                                          GTK_WIDGET (self),
                                          priv->gadget,
                                          NULL);
  gtk_builtin_icon_set_default_size (GTK_BUILTIN_ICON (priv->on_gadget), 16);

  priv->off_gadget = gtk_builtin_icon_new ("off-symbolic",
                                           GTK_WIDGET (self),
                                           priv->gadget,
                                           NULL);
  gtk_builtin_icon_set_default_size (GTK_BUILTIN_ICON (priv->off_gadget), 16);
