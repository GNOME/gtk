 ...
 *
 * The message is shown with the [method@Gtk.AlertDialog.show]
 * function. This API follows the GIO async pattern, and the
 * result can be obtained by calling
 * [method@Gtk.AlertDialog.show_finish].
 *
...
gtk_alert_dialog_class_init (GtkAlertDialogClass *class)
{
   ...
   *
   * If this property is the index of a button in [property@Gtk.AlertDialog:buttons],
   * then pressing Escape is treated as if that button was pressed. If this property
   * is -1 or not a valid index for the `buttons` array, then an error is returned.
   *
   * If `buttons` is `NULL`, then the automatically created 'Cancel' button
   ...
   *
   * If this property is the index of a button in [property@Gtk.AlertDialog:buttons],
   * then pressing Return is treated as if that button was pressed. If this property
   * is -1 or not a valid index for the `buttons` array, then nothing happens.
   *
   * If `buttons` is `NULL`, then the automatically created 'Cancel' button
   * is treated as both cancel and default button, so 0 is returned.
   *
 ...
 * It should call [method@Gtk.AlertDialog.show_finish]
 * to obtain the result.
 *
 * It is ok to pass `NULL` for the callback if the alert
 * does not have any buttons to check on.
 *
 * Since: 4.10
 */
void
                       ...
                       gpointer             user_data)
{
  ...
  g_task_set_source_tag (task, gtk_alert_dialog_show);
...
/**
 * gtk_alert_dialog_show_finish:
 * @self: a `GtkAlertDialog`
 * @result: a `GAsyncResult`
 * @error: return location for a [enum@Gtk.DialogError] error
 *
 * Finishes the [method@Gtk.AlertDialog.show] call
 ...
 */
int
gtk_alert_dialog_show_finish (GtkAlertDialog  *self,
                              GAsyncResult   *result,
                              GError        **error)
{
  GTask *task = G_TASK (result);

  g_return_val_if_fail (g_task_get_source_tag (task) == gtk_alert_dialog_show, -1);

  return (int) g_task_propagate_int (task, error);
}
