...
gtk_grid_view_update_adjustment (GtkGridView    *self,
                                 GtkOrientation  orientation)
{
  int offset, page_size, cell_size, total_size;
  guint anchor_pos;
  ...
  if (self->orientation == orientation)
    {
      ...
      gtk_grid_view_update_adjustment_with_values (self,
                                                   self->orientation,
                                                   offset - self->anchor_yalign * page_size,
                                                   aug->size,
                                                   page_size);
    }
  else
    {
      ...
      if (self->anchor_xstart)
        offset = ceil (self->column_width * i);
      else
        offset = ceil (self->column_width * (i + 1));
      total_size = round (self->n_columns * self->column_width);

      gtk_grid_view_update_adjustment_with_values (self,
                                                   OPPOSITE_ORIENTATION (self->orientation),
                                                   offset - self->anchor_xalign * page_size,
                                                   total_size,
                                                   page_size);
    }
}
                             ...
                             int        baseline)
{
  ...
  gtk_grid_view_update_adjustment (self, GTK_ORIENTATION_HORIZONTAL);
  gtk_grid_view_update_adjustment (self, GTK_ORIENTATION_VERTICAL);
                              ...
                              GtkAdjustment  *adjustment)
{
  ...
  gtk_grid_view_update_adjustment (self, orientation);
...
static void
gtk_grid_view_compute_scroll_align (GtkAdjustment *visible_area,
                                    int            cell_start,
                                    int            cell_end,
                                    ...
                                    gboolean      *new_start)
{
  ...
  visible_start = gtk_adjustment_get_value (visible_area);
  visible_size = gtk_adjustment_get_page_size (visible_area);
                              ...
                              GVariant   *parameter)
{
  ...
  gtk_grid_view_compute_scroll_align (self->adjustment[self->orientation],
                                      start, end,
  ...
  gtk_grid_view_compute_scroll_align (self->adjustment[OPPOSITE_ORIENTATION (self->orientation)],
                                      start, end,
