...
gtk_link_button_init (GtkLinkButton *link_button)
{
  ...
  priv->source = gtk_drag_source_new ();
  gtk_drag_source_set_actions (priv->source, GDK_ACTION_COPY);
  content = g_object_new (GTK_TYPE_LINK_CONTENT, NULL);
  GTK_LINK_CONTENT (content)->link = link_button;
  gtk_drag_source_set_content (priv->source, content);
  g_object_unref (content);
  gtk_drag_source_attach (priv->source, GTK_WIDGET (link_button), GDK_BUTTON1_MASK);
