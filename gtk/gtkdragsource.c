...
gtk_drag_source_class_init (GtkDragSourceClass *class)
{
  ...
  properties[PROP_CONTENT] =
       g_param_spec_object ("content", P_("Content"), P_("Content"),
                            ...
                            int            y)
{
  ...
  if (!source->paintable)
    {
      ...
      source->paintable = gtk_icon_theme_load_icon (theme, "text-x-generic", 24, 0, NULL);
      source->hot_x = 0;
      source->hot_y = 0;
    }
  ...
  gtk_widget_set_size_request (source->icon_window, 24, 24);
