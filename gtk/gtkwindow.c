...
static void     gtk_window_update_fixed_size         (GtkWindow    *window,
                                                      ...
                                                      int           new_width,
                                                      int           new_height);
static void     gtk_window_compute_size              (GtkWindow    *window,
                                                      int          *width,
                                                      int          *height,
                                                      int          *min_width,
                                                      int          *min_height);
                   ...
                   int        height)
{
  ...
  priv->resize_width = width;
  priv->resize_height = height;
                     ...
                     int       *height)
{
  ...
  if (_gtk_widget_get_mapped (GTK_WIDGET (window)))
    ...
    {
      gtk_window_compute_size (window, &w, &h, NULL, NULL);
    }
                       ...
                       GtkWidget       *widget)
{
  ...
  if (priv->maximized || priv->fullscreen)
    ...
    {
      int resize_width_csd = priv->resize_width;
      int resize_height_csd = priv->resize_height;
      gtk_window_update_csd_size (window,
                                  &resize_width_csd, &resize_height_csd,
                                  INCLUDE_CSD_SIZE);

      if (priv->resize_width > 0)
        width = resize_width_csd;
      if (priv->resize_height > 0)
  ...
  if (priv->use_client_shadow)
    gdk_toplevel_size_set_shadow_size (size,
...
static void
gtk_window_compute_size_request (GtkWindow *window,
                                 int       *width,
                                 int       *height)
{
  GtkWindowPrivate *priv = gtk_window_get_instance_private (window);
...
static void
gtk_window_compute_size (GtkWindow *window,
                         int       *width,
                         int       *height,
                         int       *min_width,
                         int       *min_height)
{
  ...
  gtk_window_compute_size_request (window, &w, &h);
...
gtk_window_do_resize (GtkWindow *window)
{
  ...
  gtk_window_compute_size (window,
  ...
  else if ((size_changed || min_size_changed) &&
