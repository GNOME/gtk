...
update_realized_window_properties (GtkWindow *window)
{
  ...
  rect.x = shadow.left;
  rect.y = shadow.top;
  rect.width = gdk_surface_get_width (priv->surface) - shadow.left - shadow.right;
  rect.height = gdk_surface_get_height (priv->surface) - shadow.top - shadow.bottom;
...
gtk_window_move_resize (GtkWindow *window)
{
  ...
  g_message ("Last size: %d×%d. New size: %d×%d",
  ...
  if (priv->configure_notify_received)
    {
      ...
      g_message ("size changed: %d. pos changed: %d", configure_request_size_changed, configure_request_pos_changed
                );
