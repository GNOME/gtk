...
gtk_gc_get_drawable_ht(GdkDisplay *display)
{
  ...
  g_slist_append(gc_drawable_ht_list, ht);
  ...
  if (!drawable)
    {
      ...
      drawable->drawable = gdk_pixmap_new (gdk_screen_get_parent_root(keyval->colormap->screen), 1, 1, drawable->depth);
