                                  ...
                                  int        baseline)
{
  ...
  int full_size_along, full_size_across;
  ...
  for (i = 0, child = gtk_widget_get_first_child (widget);
       ...
       child = gtk_widget_get_next_sibling (child))
    {
      ...
      if (i > 0)
        full_size_along += spacing;
      full_size_along += sizes[i].minimum_size;

      i++;
    }

  /* Distribute available space */
  extra_space = size_along - full_size_along;
  if (extra_space > 0)
    {
...
G_GNUC_END_IGNORE_DEPRECATIONS
              ...
              /* There is a soft limit on what the child's adjustment value can reach,
               * because when we're scrolled, we'll allocate a larger size to the child
               * instead of increasing its adjustment value. The child is not aware of
               * this when it sets the target value, hence we compensate on our side.
               */
              adjustment_value = MIN (adjustment_value, full_size_along - size_along);
  ...
  g_assert (full_size_along == MAX (size_along, adjustment_value + position));
