...
gtk_label_init (GtkLabel *label)
{
  ...
  priv->mnemonic_shortcut = gtk_shortcut_new (NULL,
  ...
  gtk_shortcut_controller_add_shortcut (GTK_SHORTCUT_CONTROLLER (controller), priv->mnemonic_shortcut);
...
gtk_label_setup_mnemonic (GtkLabel *label)
{
  ...
  if (priv->mnemonic_keyval == GDK_KEY_VoidSymbol)
    {
      gtk_shortcut_set_trigger (priv->mnemonic_shortcut, NULL);
      return;
    }

  connect_mnemonics_visible_notify (GTK_LABEL (widget));

  gtk_shortcut_set_trigger (priv->mnemonic_shortcut, gtk_mnemonic_trigger_new (priv->mnemonic_keyval));
}
...
gtk_label_finalize (GObject *object)
{
  ...
  g_clear_object (&priv->mnemonic_shortcut);
