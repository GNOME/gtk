...
update_highlight_position (GtkRange *range)
{
  ...
  if (should_invert (range))
    {
      gtk_widget_add_style_class (GTK_WIDGET (range), GTK_STYLE_CLASS_BOTTOM);
      gtk_widget_remove_style_class (GTK_WIDGET (range), GTK_STYLE_CLASS_TOP);
    }
  else
    {
      gtk_widget_add_style_class (GTK_WIDGET (range), GTK_STYLE_CLASS_TOP);
      gtk_widget_remove_style_class (GTK_WIDGET (range), GTK_STYLE_CLASS_BOTTOM);
    }
}
