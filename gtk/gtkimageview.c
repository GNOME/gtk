...
gtk_image_view_ensure_gestures (GtkImageView *self)
{
  ...
  if (priv->zoomable && priv->zoom_gesture == NULL)
    {
      priv->zoom_gesture = gtk_gesture_zoom_new (GTK_WIDGET (self));
      ...
      priv->zoom_controller = gtk_event_controller_scroll_new (GTK_WIDGET (self),
       ...
       * a ::scroll emission */
      priv->motion_controller = gtk_event_controller_motion_new (GTK_WIDGET (self));
  ...
  if (priv->rotatable && priv->rotate_gesture == NULL)
    {
      priv->rotate_gesture = gtk_gesture_rotate_new (GTK_WIDGET (self));
