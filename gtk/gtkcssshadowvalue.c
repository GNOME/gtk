                                 ...
                                 double       progress)
{
  ...
  for (i = 0; i < MIN (start->n_shadows, end->n_shadows); i++)
    {
      shadow_value_transition (&start->shadows[i], &end->shadows[i],
      ...
      for (; i < len; i++)
        {
          ...
          shadow_value_transition (&start->shadows[i], &fill, property_id, property_id, &shadows[i]);
      ...
      for (; i < len; i++)
        {
          ...
          shadow_value_transition (&fill, &end->shadows[i], property_id, property_id, &shadows[i]);
          shadow_value_unref (&fill);
        }
    }
