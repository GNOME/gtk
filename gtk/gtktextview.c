...
static gboolean gtk_text_view_query_action (GtkWidget           *widget,
                                            const char          *action_name,
                                            gboolean            *enabled,
...
gtk_text_view_init (GtkTextView *text_view)
{
  ...
  gtk_widget_add_class_actions (GTK_WIDGET (text_view));
}
...
static gboolean
gtk_text_view_query_action (GtkWidget           *widget,
                            const char          *action_name,
                            gboolean            *enabled,
                            const GVariantType **parameter_type,
                            const GVariantType **state_type,
                            ...
                            GVariant           **state)
{
  ...
  if (strcmp (action_name, "cut") == 0)
    ...
    enable = can_insert & can_paste;
  else if (strcmp (action_name, "delete") == 0)
    enable = have_selection &&
             range_contains_editable_text (&sel_start, &sel_end, priv->editable);
  else if (strcmp (action_name, "select-all") == 0)
    enable = gtk_text_buffer_get_char_count (priv->buffer) > 0;
  else if (strcmp (action_name, "insert-emoji") == 0)
    enable = (gtk_text_view_get_input_hints (text_view) & GTK_INPUT_HINT_NO_EMOJI) == 0;
  else
    return FALSE;
...
gtk_text_view_update_clipboard_actions (GtkTextView *text_view)
{
  ...
  group = gtk_widget_get_action_group (GTK_WIDGET (text_view), "clipboard");
  g_action_group_action_enabled_changed (group, "cut",
			                 have_selection &&
                                         range_contains_editable_text (&sel_start, &sel_end, priv->editable));

  g_action_group_action_enabled_changed (group, "copy", have_selection);

  g_action_group_action_enabled_changed (group, "paste", can_insert && can_paste);

  group = gtk_widget_get_action_group (GTK_WIDGET (text_view), "selection");
  g_action_group_action_enabled_changed (group, "delete",
			                 have_selection &&
                                         range_contains_editable_text (&sel_start, &sel_end, priv->editable));

  g_action_group_action_enabled_changed (group, "select-all",
...
gtk_text_view_update_emoji_action (GtkTextView *text_view)
{
  ...
  group = gtk_widget_get_action_group (GTK_WIDGET (text_view), "misc");
  g_action_group_action_enabled_changed (group, "insert-emoji",
