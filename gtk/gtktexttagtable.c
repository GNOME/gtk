...
static void gtk_text_tag_table_finalize (GtkObject *object);
...
static void
gtk_text_tag_table_finalize (GtkObject *object)
{
  GtkTextTagTable *table;
...
gtk_text_tag_table_add(GtkTextTagTable *table, GtkTextTag *tag)
{
  ...
  g_return_if_fail(GTK_IS_TEXT_VIEW_TAG_TABLE(table));
...
{
  g_return_val_if_fail(GTK_IS_TEXT_VIEW_TAG_TABLE(table), 0);
