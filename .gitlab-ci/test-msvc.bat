...
pip3 install --upgrade --user meson==1.8.1 || goto :error
meson setup _build -Dbackend_max_links=1 -Ddebug=false -Dwin32-backend=true -Dmedia-gstreamer=disabled -Dvulkan=disabled -Dsysprof=disabled -Daccesskit=enabled -Daccesskit-c:triplet=%RUST_HOST% %~1 || goto :error
