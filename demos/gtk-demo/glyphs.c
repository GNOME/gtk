...
update_short_path (GlyphDemo *self)
{
  ...
  gsk_path_foreach (self->path, GSK_PATH_FOREACH_ALLOW_CUBIC, short_cb, builder);
  ...
  if (self->show_controls)
    {
      ...
      gsk_path_foreach (self->path, GSK_PATH_FOREACH_ALLOW_CUBIC, curve_cb, snapshot);
    }
  else if (self->show_points)
    {
      gsk_path_foreach (self->path, GSK_PATH_FOREACH_ALLOW_CUBIC, point_cb, snapshot);
    }
                      ...
                      GskPath    *path2)
{
  ...
  builder = gsk_path_builder_new ();
  gsk_path_foreach (self->path, GSK_PATH_FOREACH_ALLOW_CUBIC, control_cb, builder);
                          ...
                          const char *path)
{
  ...
  gtk_button_set_label (GTK_BUTTON (demo->font_button), basename);
  g_free (basename);

  g_free (demo->font_file);
  demo->font_file = g_strdup (path);
             ...
             gpointer      data)
{
  ...
  GlyphDemo *demo = GLYPH_DEMO (data);
  GFile *file;
  GError *error = NULL;
                ...
                GlyphDemo *demo)
{
  ...
  root = gtk_widget_get_root (GTK_WIDGET (demo));
  gtk_file_dialog_open (dialog, GTK_WINDOW (root), NULL, response_cb, demo);
}
  ...
  if (!window)
    {
      ...
      gtk_css_provider_load_from_string (style, css_data);
      ...
      GLYPH_DEMO (demo)->font_button = button;
