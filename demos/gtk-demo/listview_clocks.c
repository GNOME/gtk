...
static GdkPaintable *
cast_to_paintable (GObject  *image,
                   GObject  *item,
                   gpointer  unused)
{
  return GDK_PAINTABLE (item);
}
                   ...
                   GtkListItem        *list_item)
{
   ...
   * This is somewhat complicated because the item property returns
   * a "GObject" type but the property needs a "GdkPaintable" type. So
   * we need to do a casting operation.
   * Just like above, we can do that with a simple function.
   */
  expression = gtk_cclosure_expression_new (GTK_TYPE_CLOCK,
                                            NULL,
                                            1, (GtkExpression *[1]) { gtk_expression_ref (clock_expression) },
                                            G_CALLBACK (cast_to_paintable),
                                            NULL, NULL);
