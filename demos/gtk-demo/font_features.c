...
extern FT_Library get_win32_ft_library (void);
extern FT_Face get_ft_face_from_pango_font (PangoFont *pango_font, FT_Byte *font_file_data);
extern void release_pango_font_ft_face (PangoFont *pango_font, FT_Face ft_face, FT_Byte *font_file_data);
extern GHashTable *get_win32_langtag_locales (void);
  ...
  while (g_hash_table_iter_next (&iter, (gpointer *)&pair, NULL))
    {
      ...
      if (pair->lang_tag == HB_OT_TAG_DEFAULT_LANGUAGE)
      ...
      else
        {
          ...
          ht_win32_languages = get_win32_langtag_locales ();
