...
static void
gtk_path_transform_point (GskPath                *path,
                          ...
                          graphene_point_t       *res)
{
  ...
  if (gsk_path_get_point (path, (pt->x + offset->x) * scale, &point))
    {
                       ...
                       gpointer                data)
{
  ...
  switch (op)
  {
    case GSK_PATH_MOVE:
      {
        graphene_point_t res;
        gtk_path_transform_point (transform->path, &pts[0], &transform->offset, transform->scale, &res);
    ...
    case GSK_PATH_LINE:
      {
        graphene_point_t res;
        gtk_path_transform_point (transform->path, &pts[1], &transform->offset, transform->scale, &res);
    ...
    case GSK_PATH_QUAD:
      {
        ...
        gtk_path_transform_point (transform->path, &pts[1], &transform->offset, transform->scale, &res[0]);
        gtk_path_transform_point (transform->path, &pts[2], &transform->offset, transform->scale, &res[1]);
    ...
    case GSK_PATH_CUBIC:
      {
        ...
        gtk_path_transform_point (transform->path, &pts[1], &transform->offset, transform->scale, &res[0]);
        gtk_path_transform_point (transform->path, &pts[2], &transform->offset, transform->scale, &res[1]);
        gtk_path_transform_point (transform->path, &pts[3], &transform->offset, transform->scale, &res[2]);
...
gtk_path_transform (GskPath                *line_path,
                    ...
                    const graphene_point_t *offset)
{
  GtkPathTransform transform = { line_path, gsk_path_builder_new (), *offset };
  graphene_rect_t bounds;

  gsk_path_get_bounds (path, &bounds);
  if (bounds.origin.x + bounds.size.width > 0)
    transform.scale = gsk_path_get_length (line_path) / (bounds.origin.x + bounds.size.width);
  else
