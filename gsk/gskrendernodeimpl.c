                     ...
                     GdkColorState *ccs)
{
  ...
  gdk_cairo_set_source_color_ccs (cr, ccs, self->color_state, (const float *) &self->color);
...
gsk_color_node_get_color2 (const GskRenderNode *node)
{
  ...
  return (const float *) &self->color;
}

GdkColorState *
gsk_color_node_get_color_state (const GskRenderNode *node)
{
  GskColorNode *self = (GskColorNode *) node;
...
{
  return gsk_color_node_new2 (GDK_COLOR_STATE_SRGB, (const float *) rgba, bounds);
}

GskRenderNode *
gsk_color_node_new2 (GdkColorState         *color_state,
                     const float            color[4],
                     const graphene_rect_t *bounds)
{
  GskColorNode *self;
  ...
  node->fully_opaque = color[3] > (float) 0xff00 / (float) 0xffff;
  node->preferred_depth = gdk_color_state_get_depth (color_state);
                               ...
                               GdkColorState *ccs)
{
  ...
  if (self->stops[0].offset > 0.0)
    gdk_cairo_pattern_add_color_stop_color_ccs (pattern,
                                                ccs,
                                                0.0,
                                                self->color_states[0],
                                                &self->stops[0].color);
  ...
  if (self->stops[self->n_stops-1].offset < 1.0)
    gdk_cairo_pattern_add_color_stop_color_ccs (pattern,
                                                ccs,
                                                1.0,
                                                self->color_states[self->n_stops - 1],
                              ...
                              gsize                   n_color_stops)
{
  ...
  color_states = g_newa (GdkColorState *, n_color_stops);
                               ...
                               GdkColorState          **color_states)
{
  ...
  self->color_states = g_malloc_n (n_color_stops, sizeof (GdkColorState *));
...
GdkColorState **
gsk_linear_gradient_node_get_color_states (const GskRenderNode *node)
{
  const GskLinearGradientNode *self = (const GskLinearGradientNode *) node;
...
GdkColorState **
gsk_radial_gradient_node_get_color_states (const GskRenderNode *node)
{
  const GskRadialGradientNode *self = (const GskRadialGradientNode *) node;
  ...
  for (i = 0; i <= self->n_stops; i++)
    {
      ...
      GdkColorState *color_state1 = self->color_states[MAX (i, 1) - 1];
      GdkColorState *color_state2 = self->color_states[MIN (i, self->n_stops - 1)];
      ...
      gdk_color_state_convert_color (color_state1, ccs, (const float *)&stop1->color, color1);
      gdk_color_state_convert_color (color_state2, ccs, (const float *)&stop2->color, color2);
                             ...
                             gsize                   n_color_stops)
{
  ...
  return gsk_conic_gradient_node_new2 (bounds, center,
                                       rotation,
                                       color_stops,
                                       n_color_stops,
                                       color_states);
...
GdkColorState **
gsk_conic_gradient_node_get_color_states (const GskRenderNode *node)
{
  const GskConicGradientNode *self = (const GskConicGradientNode *) node;
             ...
             GskBlurFlags          blur_flags)
{
  ...
  gdk_cairo_set_source_color_ccs (cr, ccs, color_state, (const float *) color);
  ...
  gsk_cairo_blur_finish_drawing (shadow_cr, ccs, radius, color_state, color, blur_flags);
}
  ...
  if (inset || overlapped)
    {
      ...
      draw_shadow (cr, ccs, inset, box, clip_box, radius, color_state, color, GSK_BLUR_X | GSK_BLUR_Y);
      return;
    }
                  ...
                  cairo_rectangle_int_t *drawn_rect)
{
  ...
  cairo_rectangle (cr, x1, y1, x2 - x1, y2 - y1);
  cairo_clip (cr);
  draw_shadow (cr, ccs, inset, box, clip_box, radius, color_state, color, blur_flags);
}
                            ...
                            GdkColorState *ccs)
{
  ...
  if (!needs_blur (blur_radius))
    draw_shadow (cr, ccs, TRUE, &box, &clip_box, blur_radius, self->color_state, &self->color, GSK_BLUR_NONE);
  else
    {
      ...
      for (i = 0; i < 4; i++)
        {
          ...
          cairo_clip (cr);
          draw_shadow_corner (cr, ccs, TRUE, &box, &clip_box, blur_radius, self->color_state, &self->color, i, &r);
          cairo_restore (cr);
      ...
      for (i = 0; i < 4; i++)
        {
          ...
          gdk_cairo_region (cr, remaining);
          cairo_clip (cr);
          draw_shadow_side (cr, ccs, TRUE, &box, &clip_box, blur_radius, self->color_state, &self->color, i, &r);
          cairo_restore (cr);
...
gsk_inset_shadow_node_new (const GskRoundedRect *outline,
                           ...
                           float                 blur_radius)
{
  return gsk_inset_shadow_node_new2 (outline,
                                     GDK_COLOR_STATE_SRGB,
                                     (const float *) color,
...
GdkColorState *
gsk_inset_shadow_node_get_color_state (const GskRenderNode *node)
{
  const GskInsetShadowNode *self = (const GskInsetShadowNode *) node;
                             ...
                             GdkColorState *ccs)
{
  ...
  if (!needs_blur (blur_radius))
    draw_shadow (cr, ccs, FALSE, &box, &clip_box, blur_radius, self->color_state, &self->color, GSK_BLUR_NONE);
  else
    {
      int i;
      cairo_region_t *remaining;
      ...
      for (i = 0; i < 4; i++)
        {
          ...
          gdk_cairo_region (cr, remaining);
          cairo_clip (cr);
          draw_shadow_corner (cr, ccs, FALSE, &box, &clip_box, blur_radius, self->color_state, &self->color, i, &r);
          cairo_restore (cr);
      ...
      for (i = 0; i < 4; i++)
        {
          ...
          gdk_cairo_region (cr, remaining);
          cairo_clip (cr);
          draw_shadow_side (cr, ccs, FALSE, &box, &clip_box, blur_radius, self->color_state, &self->color, i, &r);
          cairo_restore (cr);
...
gsk_outset_shadow_node_new (const GskRoundedRect *outline,
                            ...
                            float                 blur_radius)
{
  return gsk_outset_shadow_node_new2 (outline,
                                      GDK_COLOR_STATE_SRGB,
                                      (const float *)color,
                                      dx, dy, spread, blur_radius);
}
...
GdkColorState *
gsk_outset_shadow_node_get_color_state (const GskRenderNode *node)
{
  const GskOutsetShadowNode *self = (const GskOutsetShadowNode *) node;
...
gsk_text_node_new (PangoFont              *font,
                   ...
                   const graphene_point_t *offset)
{
  return gsk_text_node_new2 (font, glyphs, GDK_COLOR_STATE_SRGB, (const float *) color, offset);
}
...
GdkColorState *
gsk_text_node_get_color_state (const GskRenderNode *node)
{
  GskTextNode *self = (GskTextNode *) node;
