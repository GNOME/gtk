...
gsk_path_remove_unclosed (GskPath *path)
{
  ...
  GSK_DEBUG (PATHS, "%ld contours, %d closed", gsk_path_get_n_contours (path), g_slist_length (contours));
               ...
               PathOpData *opdata)
{
  ...
  gsk_path_foreach (p, GSK_PATH_FOREACH_ALLOW_CUBIC, collect_cb, opdata);
  gsk_path_unref (p);
      ...
      while (ll)
        {
          ...
          if (n > 0)
            {
              GSK_DEBUG (PATHS, "%d intersections between %s and %s", n, cd1->name, cd2->name);
            }
                      ...
                      if (i == 0)
                        {
                          ...
                          gdk_debug_message ("split %s from %s at %g: %s", cd1->name, name, sp[i].t1, s);
                          g_free (s);
                        }
                      char * s = gsk_curve_to_string (&cd->curve);
                      gdk_debug_message ("split %s from %s at %g: %s", cd->name, name, sp[i].t1, s);
                      g_free (s);
                    }
#endif
                      ...
                      if (i == 0)
                        {
                          ...
                          gdk_debug_message ("split %s from %s at %g: %s", cd2->name, name, sp[i].t2, s);
                          g_free (s);
                        }
                      char *s = gsk_curve_to_string (&cd->curve);
                      gdk_debug_message ("split %s from %s at %g: %s", cd->name, name, sp[i].t2, s);
                      g_free (s);
                    }
#endif
...
apply_fixups (PathOpData *opdata)
{
  ...
  GSK_DEBUG (PATHS, "found %d bad nodes", g_list_length (bad));
  ...
  while (bad != NULL)
    {
      ...
      if (!n1->inconsistent)
        {
          GSK_DEBUG (PATHS, "inconsistency disappeared");
          continue;
        }
          ...
          if (n2 != NULL)
            {
              ...
              if (n1->inconsistent == 1)
                {
                  GSK_DEBUG (PATHS, "found 2 odd boundary nodes");
                  GSK_DEBUG (PATHS, "toggling %s from %s to %s",
                                    edge->name,
...
find_next (Edge *edge)
{
  ...
  GSK_DEBUG (PATHS, "picking %s", edge->area_left == AREA_IN ? "cw" : "ccw");
  ...
  for (l = opdata->edges; l; l = l->next)
    {
      ...
      GSK_DEBUG (PATHS, "start new contour %s", edge->name);
      ...
      for (edge = find_next (edge); edge; edge = find_next (edge))
        {
          ...
          GSK_DEBUG (PATHS, "append %s", edge->name);
          gsk_curve_builder_to (&edge->curve, builder);
          edge->collected = TRUE;

          if (edge->curve.op == GSK_PATH_CLOSE)
            {
              GSK_DEBUG (PATHS, "explicitly closed");
              break;
            }

          if (edge->end == start)
            {
              GSK_DEBUG (PATHS, "implicitly closed");
