...
color_is_opaque (const float color[4])
{
  return color[3] > (float) 0xff00 / (float) 0xffff;
}
                                              ...
                                              GskRenderNode       *node)
{
  ...
  if (gsk_render_node_get_node_type (child) == GSK_COLOR_NODE &&
    ...
    {
      float color[4];
      const float *source_color = gsk_color_node_get_color2 (child);
      GdkColorState *color_state = gsk_color_node_get_color_state (child);
      GskGpuColorStates color_states;

      if (GDK_IS_DEFAULT_COLOR_STATE (color_state))
        {
          memcpy (color, source_color, sizeof (float) * 4);
          color_states = gsk_gpu_node_processor_color_states_explicit (self, color_state, FALSE);
        }
      else
        {
          gdk_color_state_convert_color (color_state, self->ccs, source_color, color);
                                       ...
                                       GskRenderNode       *node)
{
  ...
  node_color = gsk_color_node_get_color2 (node);
  node_color_state = gsk_color_node_get_color_state (node);
...
gsk_gpu_node_processor_add_border_node (GskGpuNodeProcessor *self,
                                        GskRenderNode       *node)
{
  const float *node_colors;
  GdkColorState **node_color_states;
  float colors[4][4];
  gsize i;

  node_colors = gsk_border_node_get_colors2 (node);
  node_color_states = gsk_border_node_get_color_states (node);
  for (i = 0; i < G_N_ELEMENTS (colors); i++)
    {
      gdk_color_state_convert_color (node_color_states[i], self->ccs, &node_colors[4 * i], colors[i]);
                                              ...
                                              GskRenderNode       *node)
{
  ...
  node_color = gsk_inset_shadow_node_get_color2 (node);
  node_color_state = gsk_inset_shadow_node_get_color_state (node);
  if (GDK_IS_DEFAULT_COLOR_STATE (node_color_state))
    {
      memcpy (color, node_color, sizeof (float) * 4);
      color_states = gsk_gpu_node_processor_color_states_explicit (self, node_color_state, FALSE);
                                               ...
                                               GskRenderNode       *node)
{
  ...
  node_color = gsk_outset_shadow_node_get_color2 (node);
  node_color_state = gsk_outset_shadow_node_get_color_state (node);
  if (GDK_IS_DEFAULT_COLOR_STATE (node_color_state))
    {
      memcpy (color, node_color, sizeof (float) * 4);
      color_states = gsk_gpu_node_processor_color_states_explicit (self, node_color_state, FALSE);
          ...
          for (i = 0; i < n_stops; i++)
            {
              ...
              gdk_color_state_convert_color (color_states[i], ics, (const float *) &stops[i].color, (float *) &real_stops[i].color);
              real_stops[i].color.alpha *= self->opacity;
            }
          stops = real_stops;
        }

      func (self, node, ics, stops, n_stops);
  ...
  for (i = 0; i < n_stops; /* happens inside the loop */)
    {
      if (i == 0)
        {
          ...
          gdk_color_state_convert_color (color_states[i], ics, (const float *) &stops[i].color, (float *) &real_stops[0].color);
      ...
      else
        {
          ...
          gdk_color_state_convert_color (color_states[i - 1], ics, (const float *) &stops[i - 1].color, (float *) &real_stops[0].color);
          real_stops[0].color.alpha *= 0;
        }
      for (j = 1; j < 6 && i < n_stops; j++)
        {
          real_stops[j].offset = stops[i].offset;
          gdk_color_state_convert_color (color_states[i], ics, (const float *) &stops[i].color, (float *) &real_stops[j].color);
      ...
      func (&other, node, ics, real_stops, j);
    }
                                           ...
                                           gsize                 n_stops)
{
  gsk_gpu_linear_gradient_op (self->frame,
                              gsk_gpu_clip_get_shader_clip (&self->clip, &self->offset, &node->bounds),
                              gsk_gpu_node_processor_color_states_explicit (self, ics, TRUE),
                                      ...
                                      GskRenderNode       *node)
{
  ...
  if (gsk_render_node_get_node_type (source_child) == GSK_COLOR_NODE &&
    ...
    {
      float color[4];
      const float *node_color = gsk_color_node_get_color2 (source_child);
      GdkColorState *node_color_state = gsk_color_node_get_color_state (source_child);
      GskGpuColorStates color_states;
...
gsk_fill_data_free (gpointer data)
{
  ...
  gdk_color_state_unref (fill->color_state);
                                  ...
                                  cairo_t  *cr)
{
  ...
  gdk_cairo_set_source_color_ccs (cr, GDK_COLOR_STATE_SRGB, fill->color_state, fill->color);
  cairo_fill (cr);
}
                                      ...
                                      GskRenderNode       *node)
{
  ...
  if (GSK_RENDER_NODE_TYPE (child) == GSK_COLOR_NODE)
    {
      color = gsk_color_node_get_color2 (child);
      color_state = gsk_color_node_get_color_state (child);
    }
  else
    {
      color = white;
  ...
  mask_image = gsk_gpu_upload_cairo_op (self->frame,
                                        ...
                                        g_memdup (&(FillData) {
                                            ...
                                            .color_state = gdk_color_state_ref (color_state),
...
gsk_stroke_data_free (gpointer data)
{
  ...
  gdk_color_state_unref (stroke->color_state);
                                    ...
                                    cairo_t  *cr)
{
  ...
  gdk_cairo_set_source_color_ccs (cr, GDK_COLOR_STATE_SRGB, stroke->color_state, stroke->color);
  cairo_stroke (cr);
}
