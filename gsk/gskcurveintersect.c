 ...
 */
static int
gsk_curve_find_intersections (const GskCurve   *curve1,
                              const GskCurve   *curve2,
                              float            *t1,
                              float            *t2,
                              graphene_point_t *p,
                     ...
                     int                  n)
{
  ...
  if (curve_near (curve1, curve2, 0.5))
    {
      t1[0] = 0; t1[1] = 1;
      t2[0] = 0; t2[1] = 1;
      p[0] = *gsk_curve_get_start_point (curve1);
  ...
  nn = gsk_curve_find_intersections (curve1, curve2, tt1, tt2, pp, 10);
  if (nn == 10)
    {
      float s1[2], s2[2];
      ...
      if (find_coincidence (curve1, curve2, s1, s2, q))
        {
          ...
          t2[num] = s2[0];
          p[num] = q[0];
          kind[num] = GSK_PATH_INTERSECTION_START;
          num++;

          t1[num] = s1[1];
          t2[num] = s2[1];
          p[num] = q[1];
          kind[num] = GSK_PATH_INTERSECTION_END;
          num++;
          ...
          if (op1 == GSK_PATH_CUBIC)
            {
              ...
              /* check for loop situation */

              if (s1[0] > 0 || s1[1] < 1)
                {
                  if (s1[0] > 0)
                    gsk_curve_split (curve1, s1[0], &c, NULL);
                  else
                    gsk_curve_split (curve1, s1[1], NULL, &c);

                  m = gsk_curve_find_intersections (&c, curve2, &ss1, &ss2, &qq, 1);
                  if (m > 0)
                    {
                      if (s1[0] > 0)
                        t1[num] = ss1 * s1[0];
                      else
                        t1[num] =  s1[1] + ss1 * (1 - s1[1]);
                      t2[num] = ss2;
                      p[num] = qq;
                      kind[num] = GSK_PATH_INTERSECTION_NORMAL;
                      num++;
              ...
              if (s2[0] > 0 || s2[1] < 1)
                {
                  if (s2[0] > 0)
                    gsk_curve_split (curve2, s2[0], &c, NULL);
                  else
                    gsk_curve_split (curve2, s2[1], NULL, &c);

                  m = gsk_curve_find_intersections (curve1, &c, &ss1, &ss2, &qq, 1);
                  if (m > 0)
                    {
                      t1[num] = ss1;
                      if (s2[0] > 0)
