                       ...
                       graphene_point_t        p[2])
{
  ...
  if (dis < 0)
    ...
    {
      int sdy = dy < 0 ? -1 : 1;
      p[0].x = c->x + (D*dy + sdy*dx*sqrtf (dis)) / dr2;
      p[0].y = c->y + (-D*dx + fabs (dy)*sqrtf (dis)) / dr2;
      p[1].x = c->x + (D*dy - sdy*dx*sqrtf (dis)) / dr2;
      p[1].y = c->y + (-D*dx - fabs (dy)*sqrtf (dis)) / dr2;
      return 2;
    }
}
...
cubic_is_simple (const GskCurve *curve)
{
  ...
  s = CLAMP (graphene_vec2_dot (&t1, &t2), -1, 1);

  if (fabs (acosf (s)) >= M_PI / 3.f)
    return FALSE;
...
conic_is_simple (const GskCurve *curve)
{
  ...
  s = CLAMP (graphene_vec2_dot (&n1, &n2), -1, 1);
      ...
      if (n == 3)
        {
          ...
          gsk_curve_split (curve, t[1], &c1, &c2);
          callback (&c1, data);
          callback (&c2, data);
        }
      else
            ...
            Segment    *offset)
{
  ...
  if (fabs (angle) < DEG_TO_RAD (5))
    ...
    line_join = offset_data->stroke->line_join;
      ...
      if ((angle > 0) == (offset_data->distance > 0))
      ...
      else
        {
          ...
          add_line_join (offset_data->offset,
                         ...
                         offset_data->stroke->miter_limit,
                         gsk_curve_get_start_point (c2),
                         o1,
                         o2,
                         angle);
                            ...
                            GskStroke        *stroke)
{
  ...
  offset_data_init (&offset_data, builder, distance, stroke);
