                        ...
                        GskCurve       *offset)
{
  ...
  if (coinc == 7)
  ...
  else if (coinc == 0)
    {
      ...
      if (graphene_point_near (&m1, &m2, 0.01) ||
          !line_intersection (&p[0], &m1, &m2, &m3, &p[1]))
        p[1] = m1;

      if (graphene_point_near (&m3, &m4, 0.01) ||
          !line_intersection (&m2, &m3, &m4, &p[3], &p[2]))
        p[2] = m4;
    }
 ...
 */
void
gsk_curve_raise (const GskCurve *curve,
                 GskCurve       *raised)
{
  if (curve->op == GSK_PATH_LINE)
    {
      ...
      gsk_quad_curve_init_from_points (&raised->quad, p);
    }
  else if (curve->op == GSK_PATH_QUAD)
    {
      ...
      gsk_cubic_curve_init_from_points (&raised->cubic, p);
    }
  else
    g_assert_not_reached ();
}
