                        ...
                        GskCurve       *offset_curve)
{
  ...
  gsk_curve_init (offset_curve, gsk_pathop_encode (GSK_PATH_CURVE, p));
}
 ...
 */
static void
cubic_approximation (GskCurve *curve,
                     GskCurve *cubic)
{
  GskConicCurve *self = &curve->conic;
  graphene_point_t p[4];
                                  ...
                                  gpointer              user_data)
{
  if (gsk_conic_is_close_to_cubic (curve, cubic, tolerance))
    ...
    return gsk_conic_curve_decompose_curve (curve, tolerance, add_curve_func, user_data);
}
                        ...
                        GskCurve       *offset_curve)
{
  ...
  gsk_curve_init (offset_curve, gsk_pathop_encode (GSK_PATH_CONIC, p));
}
