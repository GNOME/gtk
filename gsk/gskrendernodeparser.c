                       ...
                       GdkTexture *texture)
{
  ...
  fourcc = gdk_memory_format_get_dmabuf_rgb_fourcc (layout.format);
  default_color_state = GDK_COLOR_STATE_SRGB;
  if (fourcc == 0)
    {
      fourcc = gdk_memory_format_get_dmabuf_yuv_fourcc (layout.format);
  ...
  if (g_ascii_isalnum (fourcc_str[0]) &&
    ...
    {
      append_unsigned_param (p, "fourcc", fourcc);
    }
  append_unsigned_param (p, "width", layout.width);
  append_unsigned_param (p, "height", layout.height);
