...
static gboolean
parse_dmabuf_offset (GtkCssParser *parser,
                     Context      *context,
                     gpointer      data)
{
  GdkDmabuf *dmabuf = data;

  if (!parse_unsigned (parser, context, &dmabuf->planes[0].offset))
    return FALSE;
  if (!gtk_css_parser_try_token (parser, GTK_CSS_TOKEN_COMMA))
    return TRUE;
  if (!parse_unsigned (parser, context, &dmabuf->planes[1].offset))
    return FALSE;
  if (!gtk_css_parser_try_token (parser, GTK_CSS_TOKEN_COMMA))
    return TRUE;
  if (!parse_unsigned (parser, context, &dmabuf->planes[2].offset))
    return FALSE;
  if (!gtk_css_parser_try_token (parser, GTK_CSS_TOKEN_COMMA))
    return TRUE;
  if (!parse_unsigned (parser, context, &dmabuf->planes[3].offset))
    return FALSE;
...
static gboolean
parse_dmabuf_stride (GtkCssParser *parser,
                     Context      *context,
                     gpointer      data)
{
  GdkDmabuf *dmabuf = data;

  if (!parse_unsigned (parser, context, &dmabuf->planes[0].stride))
    return FALSE;
  if (!gtk_css_parser_try_token (parser, GTK_CSS_TOKEN_COMMA))
    return TRUE;
  if (!parse_unsigned (parser, context, &dmabuf->planes[1].stride))
    return FALSE;
  if (!gtk_css_parser_try_token (parser, GTK_CSS_TOKEN_COMMA))
    return TRUE;
  if (!parse_unsigned (parser, context, &dmabuf->planes[2].stride))
    return FALSE;
  if (!gtk_css_parser_try_token (parser, GTK_CSS_TOKEN_COMMA))
    return TRUE;
  if (!parse_unsigned (parser, context, &dmabuf->planes[3].stride))
    return FALSE;
                      ...
                      Context      *context)
{
  ...
  const Declaration declarations[] = {
    ...
    { "offset", parse_dmabuf_offset, NULL, &dmabuf },
    { "stride", parse_dmabuf_stride, NULL, &dmabuf },
  ...
  while (dmabuf.n_planes < GDK_DMABUF_MAX_PLANES && dmabuf.planes[dmabuf.n_planes].stride > 0)
    dmabuf.n_planes++;
                       ...
                       GdkTexture *texture)
{
  ...
  bytes = gdk_texture_download_data_buffer (texture, &buffer);
  if (bytes == NULL)
    return FALSE;
  ...
  fourcc_str[0] = fourcc >> 0;
  fourcc_str[1] = fourcc >> 8;
  fourcc_str[2] = fourcc >> 16;
  fourcc_str[3] = fourcc >> 24;
  fourcc_str[4] = 0;
  if (g_ascii_isalnum (fourcc_str[0]) &&
    ...
    {
      append_unsigned_param (p, "width", fourcc);
    }
  append_unsigned_param (p, "width", buffer.width);
  append_unsigned_param (p, "height", buffer.height);

  for (i = 0; i < n_planes; i++)
    {
      if (buffer.planes[i].data != data)
        break;
    }
  if (i < n_planes)
    {
      ...
      for (i = 0; i < n_planes; i++)
        {
          ...
          g_string_append_printf (p->str, "%zu", (gsize) (buffer.planes[i].data - data));
        }
      g_string_append (p->str, ";\n");
    }
  for (i = 0; i < n_planes; i++)
      ...
      for (i = 0; i < n_planes; i++)
        {
          if (i > 0)
            g_string_append_printf (p->str, ", ");
          g_string_append_printf (p->str, "%zu", buffer.planes[i].stride);
        }
      g_string_append (p->str, ";\n");
    }
                      ...
                      GdkTexture *texture)
{
  ...
  if (!append_dmabuf_texture (p, texture))
    {
