  ...
  else if (is_ident (p->cur, "transform"))
    {
      ...
      if (p->cur->type == TOK_OPEN_PAREN)
      ...
      else
        {
              ...
              if (is_ident (p->cur, "translate"))
                {
                  ...
                  transform = gtk_transform_translate (transform,
          ...
          gtk_transform_to_matrix (transform, &matrix);
        }

      child = parse_node (p);

      expect_skip (p, TOK_CLOSE_BRACE);
      result = gsk_transform_node_new (child, &matrix);
    ...
    case GSK_TRANSFORM_NODE:
      {
        ...
        append_matrix_param (p, "transform", gsk_transform_node_peek_transform (node));
